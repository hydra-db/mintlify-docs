...
<Note>
Delivered payloads carry `tenant_id` and `sub_tenant_id` alongside `database` and
`collection`. They are the pre-rename spellings, kept because receivers built
against them would stop finding the scope if they disappeared. Signature
verification is unaffected either way - the HMAC covers whatever bytes are
delivered. Read `database` and `collection` in new integrations, and see the
[glossary](/essentials/v2/glossary) for the full mapping.
</Note>
