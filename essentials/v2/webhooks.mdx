...
Webhooks let your application receive an HTTP callback when HydraDB finishes processing an ingested item.
...
| `id` | The item's `context_id`: the one you supplied at ingestion, or the generated one. For connector-synced content, the connector item's id. |
...
Delivery history calls the item's id `doc_id`. The outbound webhook payload calls it `id`.
...
| `id` is unexpected | It is the item's `context_id` (the one you supplied, or the generated one), or the connector item's id for synced content. |
