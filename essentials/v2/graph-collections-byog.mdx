---
title: "Cypher Graph Collections"
description: "Full Cypher access to property graphs you own end to end, stored and run by HydraDB."
---

Cypher Graph Collections give you full **Cypher** access to graph
...
their Cypher and their data model as they are.

<Note>
  Cypher Graph Collections are separate from ingested context. The endpoints live
  under the `/byog` path, but they are not [Bring Your Own Graph](/essentials/v2/bring-your-own-graph),
  which attaches your own entities and relations to a context at ingest
  with `graph_payload`.
</Note>

- **Databases** group your collections. A graph database created here appears in your
...
- **Isolation is structural.** Each collection is a completely separate graph
  owned by your organization. There is no other organization's data to reach:
...
# 2. Write data. Collections are created on first use.
...
A missing or invalid key returns `401`. Databases are scoped to the
...
Returns `{"database": "crm", "status": "ready", "cluster": "shared"}`, ready to
query. If your organization has a dedicated graph cluster, `status` is
`"provisioning"` and `cluster` is `"dedicated"` until that cluster is up; queries
meanwhile return `503` with a `Retry-After` header. Creating a name that already
exists returns `409`.
...
<Note>
  **Coverage in one line:** openCypher reads and writes are supported, except
  procedure calls and `LOAD CSV`, with the few dialect differences listed below.
</Note>
...
**Graph traversal and exploration**: walk relationships to arbitrary depth, expand a node's neighborhood, follow
...
- **`shortestPath`** goes in a `RETURN` or `WITH` clause (not `MATCH p = …`)
  and the traversal must be directed; see the shortest-path example below.
...
```json
{
  ...
  "error": null,
  "meta": { "request_id": "9be86a4e-…", "api_version": "2.0.1", "latency_ms": 12.4 }
}
```
...
| integer | JSON number (64-bit). Values beyond 2⁵³ lose precision in double-based parsers, so keep ids in range or return them as strings |
| float | JSON number |
...
The client is shown in Python and TypeScript, the patterns after it in Python;
the ideas port to any language.

### A minimal client

Wrap the endpoint once. Success puts rows in `data`; an error carries its code
and message in `error`, repeated under `detail`.
...
This also keeps your code stable as the schema grows: adding properties to nodes
...
Errors use the standard envelope, with the code and message repeated under
`detail`:
...
```json
{
  "success": false,
  "data": null,
  "error": { "code": "DATABASE_NOT_FOUND", "message": "…" },
  "meta": { "request_id": "9be86a4e-…", "api_version": "2.0.1", "latency_ms": 3.1 },
  "detail": { "success": false, "message": "…", "error_code": "DATABASE_NOT_FOUND" }
}
```

| Status | Meaning |
|---|---|
| `400` | Invalid request, unsupported construct, **Cypher error** (compiler message passed through) or **query timeout** |
| `401` | Missing or invalid API key |
| `403` | The API key's scope does not permit this operation |
...
| `503` | The database's dedicated cluster is still provisioning; retry after the `Retry-After` interval |
