...
def verify_processing(file_id: str, timeout: int = 120, interval: int = 3):
    ...
    while time.time() < deadline:
        ...
        status   = statuses[0].indexing_status if statuses else None
        if status == "completed":
            return True
        if status == "errored":
...
def recall_context(
    ...
    # Response shape:
    # result.chunks        - ranked context chunks with relevancy_score
    # result.graph_context - entity paths and chunk_relations
...
def handle_mention(event, client):
    ...
    answer = "\n\n".join(c.chunk_content for c in chunks[:3]) or "No results found."
    sources = result.sources or []

    if sources:
        links  = "\n".join(f"• {getattr(s, 'title', '')}" for s in sources[:3])
        answer += f"\n\n*Sources:*\n{links}"
