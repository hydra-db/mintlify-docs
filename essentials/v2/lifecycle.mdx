...
For `type: "knowledge"`, the response reports each ID independently in `results[]` with a per-ID `deleted` flag and a `deleted_count`, so a partial failure (one ID missing, others removed) is visible rather than silent. Memory deletes work the same way with `type: "memory"`, but the response reports an aggregate `user_memory_deleted` covering all listed IDs rather than per-ID results; to confirm a specific memory is gone, check [`POST /context/list`](/api-reference/v2/endpoint/list-documents).
