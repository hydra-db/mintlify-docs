...
| `INVALID_PARAMETERS` | 400 | Identifier exceeds length limit: `source_id` or `id` > 100 characters, `sub_tenant_id` > 128 characters, or `chunk_id` > 128 characters (raw embeddings only) |
...
### 400 or 422 errors on ingestion
...
- Identifier exceeds length limit: `source_id` / `file_id` / `id` > 100 characters, `sub_tenant_id` > 128 characters, or `chunk_id` > 128 characters (raw embeddings only)
