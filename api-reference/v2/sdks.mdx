...
| `/tenants/*` and `/tenants` | `client.tenants` | Create, list, delete, status, collections, stats |
...
A database is an isolated workspace. Most organizations create one database total, with collections for users or teams. See [Multi-Database](/essentials/v2/multi-database) for the full model.
...
Database creation is asynchronous. Poll [`tenants.status`](/api-reference/v2/endpoint/database-status) until provisioning completes:
