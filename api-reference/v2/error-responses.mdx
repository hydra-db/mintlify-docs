...
HydraDB core endpoints (`/databases`, `/context/*`, and `/query`) use the same top-level envelope for successful and failed requests. This page covers the **error** shape, status codes, and retry patterns. For the full success envelope and every `meta` field (`request_id`, `api_version`, scope echo, `deprecation[]`), see [Response Format](/api-reference/v2/response-format).
...
- [Response Format](/api-reference/v2/response-format) - success envelope and full `meta` reference
