...
<Accordion title="App sources - index text from your workspace or personal apps">
  ...
  <Note>
  `fields` is structured per `kind` (`email`, `message`, `ticket`, `knowledge_base`, `comment`, or `custom`)  -  the shape above is for `kind: "message"`. See [App Sources](/essentials/v2/app-sources#10-tldr-field-reference) for the full per-kind field reference, relation and attachment shapes, and per-provider examples (Slack, Jira, Linear, Notion, Gmail, Salesforce).
  </Note>
  ...
  | <Field name="title" type="string" recommended /> | Short title or subject shown in search results. |
  | <Field name="type" type="string" recommended /> | Source category for display/analytics, such as `slack`, `notion`, `gmail`, or `jira`. |
  | <Field name="kind" type="email, message, ticket, knowledge_base, comment, custom" recommended /> | App object kind. Selects the app parser and the shape expected inside `fields`. |
  | <Field name="provider" type="string" recommended /> | App namespace such as `slack`, `gmail`, `jira`, `notion`, `salesforce`, or `linear`. Prevents ID collisions across apps. |
  ...
  | <Field name="fields" type="object" required /> | Structured, kind-specific content. Primary app-native searchable payload  -  put the main text here (`body`, `description`, `title`, or `data`, depending on `kind`). |
  ...
  | <Field name="attachments" type="array" /> | Optional related attachments. Each attachment carries its own `content` object (`{ "text": "..." }` or `{ "markdown": "..." }`). (default=`[]`) |
  | <Field name="comments" type="array" /> | Optional embedded snapshot comments. (default=`[]`) |
  | <Field name="relations" type="array" /> | App-native relation array: `[{ "predicate": "linked_to", "target": { "external_id": "...", "provider": "..." } }]`. |
</Accordion>
