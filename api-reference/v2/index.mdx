...
|---|---|---|
| `database` | Your isolated workspace for data, metadata schema, and query. | Send it on every API call so HydraDB knows which workspace to read or write. |
| `collection` | Optional partition inside a database, often a user, team, account, or customer. | Use it when one database contains data for multiple users or customers. Read more about our [multi-database architecture](/essentials/v2/multi-tenant) |
...
|---|---|---|---|---|
| [`/databases`](/api-reference/v2/endpoint/create-database) | `POST` | `databases.create` | Create a database | You are setting up a new isolated workspace and optional metadata schema. |
...
| [`/databases`](/api-reference/v2/endpoint/list-databases) | `GET` | `databases.list` | List databases | You need to discover database IDs available to the current API key. |
| [`/databases`](/api-reference/v2/endpoint/delete-database) | `DELETE` | `databases.delete` | Delete a database | You need to permanently remove a workspace and its data. |
| [`/databases/status`](/api-reference/v2/endpoint/database-status) | `GET` | `databases.status` | Check provisioning readiness | You just created a database and need to wait before ingesting data. |
| [`/databases/collections`](/api-reference/v2/endpoint/list-collections) | `GET` | `databases.collections` | List active collections | You partition data by user, team, customer, or account and need to inspect those partitions. |
| [`/databases/stats`](/api-reference/v2/endpoint/database-stats) | `GET` | `databases.stats` | Get usage statistics | You want to monitor object counts and vector dimensions for a database. |
...
- **Database scoping.** Most database-scoped endpoints require a `database`. Many source and query endpoints also accept an optional `collection` for finer-grained scoping. If omitted, the default collection is used.
