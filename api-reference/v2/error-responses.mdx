...
Database creation is asynchronous. After `POST /tenants`, poll [`GET /tenants/status`](/api-reference/v2/endpoint/database-status) until `infra.scheduler_status`, `infra.graph_status`, `infra.vectorstore_status.knowledge`, and `infra.vectorstore_status.memories` are all `true`.
