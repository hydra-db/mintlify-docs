...
| <Field name="collection" type="string" /> | Logical partition inside the database. If omitted, defaults to the resolved primary tenant ID (the default collection). Formerly `sub_tenant_id`; the `sub_tenant_id` alias is still accepted (deprecated). (default=`""`  -  default collection) |
...
- **Collection defaulting.** Omitting `collection` defaults to the resolved primary tenant ID (the default collection). All endpoints (ingestion, search, delete) use this same rule. List available collections with [List Collections](/api-reference/v2/endpoint/list-sub-tenants).
