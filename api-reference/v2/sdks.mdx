...
<Info>
The Python SDK raises one of the typed exceptions exported from `hydra_db.errors`  -  `BadRequestError` (400), `ForbiddenError` (403), `NotFoundError` (404), `ConflictError` (409), `ContentTooLargeError` (413), `UnsupportedMediaTypeError` (415), `UnprocessableEntityError` (422), `InternalServerError` (500), `BadGatewayError` (502), and `ServiceUnavailableError` (503). Each subclasses `ApiError` and carries `status_code`, `headers`, and the parsed response `body` (from which you can read `body["error"]["code"]`).

**There is no typed class for 401 or 429.** The typed exceptions are generated from the OpenAPI spec, which does not yet declare either status, so catch the base `ApiError` and branch on `status_code` as above.
</Info>
```typescript TypeScript SDK
