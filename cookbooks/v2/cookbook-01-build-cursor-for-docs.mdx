...
```bash
python3 phase0/create_database.py
```
...
**Fix:** Run `python3 phase0/create_database.py` and wait for "✓ Database ready." before proceeding. Confirm `DATABASE_ID` in `config.py` exactly matches what you passed to `create_database`.
