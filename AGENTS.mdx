...
| Continuously sync external apps (Slack, GitHub, Linear, Notion, Gmail) | `/connectors*` (raw HTTP; see Connectors) |
...
Connectors continuously sync external app data (Slack, GitHub, Linear, Notion, Gmail) into the knowledge store, so you do not hand-build `app_knowledge` payloads. Authenticate once, pick which resources to sync, and HydraDB replicates new content on a schedule as searchable app sources.

- Connectors are a raw HTTP / REST feature. The Python and TypeScript SDKs do **not** yet expose connector methods; call these endpoints directly.
- Use the same auth headers as everything else: `Authorization: Bearer <api_key>` and `API-Version: 2`.
- Connector endpoints return their response object **directly** — they are not wrapped in the `{ success, data, error, meta }` envelope.

### Lifecycle

```text
POST /connectors                 -> create with credentials (returns connector_id)
GET  /connectors/:id/discover    -> list resources available to the credentials
POST /connectors/:id/configure   -> activate chosen resources, set lookback_days
POST /connectors/:id/sync        -> (optional) force a sync; scheduler runs hourly
GET  /connectors/:id/resources   -> poll until provider_cursor is non-empty
POST /query  (query_apps: true)  -> search the synced app data
```
...
|---|---|---|
| `/connectors` | POST | Create a connector |
| `/connectors` | GET | List connectors for the organization |
| `/connectors/:id` | GET | Get one connector |
| `/connectors/:id` | DELETE | Delete a connector (already-synced data is retained) |
| `/connectors/:id/discover` | GET | List resources available from the provider |
| `/connectors/:id/configure` | POST | Activate resources and set sync options |
| `/connectors/:id/sync` | POST | Trigger an on-demand sync |
| `/connectors/:id/resources` | GET | List configured resources and their sync state |
| `/connectors/:id/resources` | POST | Add a single resource row |
| `/connectors/:id/resources/:resource_id` | DELETE | Remove a resource |

### Create a connector

The create contract uses `database` (the tenant) and `collection` (the sub-tenant) — **not** `tenant_id` / `sub_tenant_id`. The response echoes them back as `tenant_id` and `sub_tenant_id`.
...
|---|---|---|
| `provider` | yes | One of `slack`, `github`, `linear`, `notion`, `gmail` |
| `name` | no | Human-readable label for this connector |
| `database` | yes | Tenant that receives the synced data |
| `collection` | no | Default sub-tenant partition (per-resource override available); default `""` |
| `provider_account_scope` | no | Stable external-account identifier; part of the deduplication key |
| `credentials` | yes | Provider token, e.g. `{ "api_token": "..." }` or `{ "access_token": "..." }` |

`provider_account_scope` must be distinct across connectors for the same provider — two connectors that share a scope (or both omit it) share a deduplication namespace and overwrite each other. Use the Slack workspace ID (`T...`), GitHub org/user login, or Linear/Notion workspace name; leave it empty for Gmail.

### Discover and configure

`GET /connectors/:id/discover` returns every syncable resource for the credentials under a provider-agnostic `resources` array, plus a per-type grouping key (`channels`, `repos`, `teams`, `projects`, `databases`, `pages`, `mailboxes`). **Always read from `resources`.** Each item has `id`, `name`, `resource_type`, and a provider-specific `metadata` object.

Activate resources with `POST /connectors/:id/configure`:
...
- `lookback_days` (default `30`) controls only how far back the **first** sync fetches; later syncs are incremental from the cursor.
- A per-resource `sub_tenant_id` overrides the connector-level `collection`.
- Reconfigure at any time to add resources or change routing — the cursor is preserved.
- `POST /connectors/:id/resources` adds a single resource without the full configure flow (use `sub_tenant_id_override` on that endpoint).

### Sync and confirm

Syncs run on a schedule (default hourly). Force one with `POST /connectors/:id/sync`; it returns `202` with a `run_id` — queued, not complete. Confirm a sync ran by polling `GET /connectors/:id/resources` until each resource's `provider_cursor` is non-empty.

### Metadata on synced objects

Each synced object carries the same two metadata layers as manual ingestion:

- **Tenant metadata (`metadata`)** — schema-declared and filterable. HydraDB always writes `provider`; your per-resource `metadata` is merged in, but `provider` wins on conflict. Only keys declared in `tenant_metadata_schema` are indexed for filtering.
- **Document metadata (`additional_metadata`)** — free-form. Auto-populated on every object with `connector_id`, `resource_id`, `provider_account_scope`, and provider-native IDs (Slack TS, GitHub issue number, Linear identifier, etc.). Your per-resource `additional_metadata` is merged in, but provider-generated fields win on conflict.

### Search synced data

Synced content is ordinary app-source knowledge. Query it with `type: "knowledge"` and `query_apps: true`, and scope to a connector, resource, or account via document-metadata filters:
...
| Scope to | `additional_metadata` key |
|---|---|
| One connector | `connector_id` |
| One channel / repo / label | `resource_id` |
| One external account | `provider_account_scope` |

---

## 8. Context status and lifecycle
...
## 10. Turning search results into LLM prompts
...
## 12. Browse, fetch, relations, delete
...
- [ ] Using `tenant_id` / `sub_tenant_id` in the `POST /connectors` body instead of `database` / `collection`.
- [ ] Reusing the same `provider_account_scope` across connectors for the same provider (dedup collision).
- [ ] Treating a `202` from `POST /connectors/:id/sync` as sync completion instead of polling `provider_cursor`.
- [ ] Hand-building `app_knowledge` for a source a connector already syncs.
