...
- **Auto-Capture** - After every AI turn, the last user/assistant exchange is sent to HydraDB (`POST /context/ingest`, `type=memory`) with `infer: true` and `upsert: true`, so HydraDB extracts the durable memory and builds the knowledge graph automatically.
