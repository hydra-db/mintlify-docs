...
Activates one or more resources and sets sync options. This is the step between discovery and the first sync. You can call configure again at any time to add resources, change `collection`, or update metadata  -  the cursor is preserved on reconfigure.
...
| <Field name="collection" type="string" /> | Collection partition for synced objects from this resource. Overrides the connector-level `collection`. (deprecated alias: `sub_tenant_id`) |
