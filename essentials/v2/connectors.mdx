...
Before you create a connector, you can:

1. Browse availability with [`GET /connector-catalog`](/api-reference/v2/endpoint/connector-catalog)
2. Confirm credential fields with [`GET /connectors/providers`](/api-reference/v2/endpoint/list-connector-providers)
3. Preview resources from temporary credentials with [`POST /connector-discovery`](/api-reference/v2/endpoint/connector-discovery)

After sync, search with `query_apps: true` on [`POST /query`](/api-reference/v2/endpoint/query).
...
| `database` | Which database receives the synced data (canonical v2 field; `tenant_id` is a deprecated alias) |
| `collection` | Which collection partition receives the data (canonical v2 field; `sub_tenant_id` is a deprecated alias) |
