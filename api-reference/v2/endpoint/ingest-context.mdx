...
Knowledge is data from apps, decision traces, agent logs, markdowns, PDFs, etc. Memory is user traits, preferences, and facts about their likes/dislikes.

import { Field } from "/snippets/field.jsx";

1. When context is of `type=knowledge:`
   - **Documents** - Use the `documents` field for binary uploads (PDF, DOCX, CSV, MD, TXT) that HydraDB should parse.
...
2. When context is of `type=memory`
