        ...
        app_knowledge=json.dumps([
            {
                ...
                "kind": "message",
                "provider": "slack",
                "external_id": "slack_thread_001",
                "fields": {"kind": "message", "body": "We agreed on three tiers..."},
...
| <Field name="upsert" type="string (boolean)" /> | Replace existing sources with the same ID. Pass `"true"` or `"false"` as strings — the API and SDK 2.1.1 type this parameter as a string. (default=`"true"`) |
...
<Accordion title="Knowledge documents - upload PDFs, DOCX, CSV, markdown, or text">
  ...
  with open("/path/to/policy.pdf", "rb") as f1:
      result = client.context.ingest(
          type="knowledge",
          database="acme_corp",
          documents=("policy.pdf", f1, "application/pdf"),
          document_metadata=json.dumps([
  ...
  ```typescript TypeScript SDK
  // SDK 2.1.1 accepts one Uploadable per call; repeat the call for more files.
...
<Accordion title="App sources - index text from your workspace or personal apps">
  ...
  | <Field name="kind" type="string" recommended /> | App object kind such as `message`, `email`, `ticket`, `knowledge_base`, `comment`, or `custom`. Use the kind-specific shape documented in [App Sources](/essentials/v2/app-sources). |
  | <Field name="provider" type="string" recommended /> | App namespace such as `slack`, `gmail`, `jira`, or `notion`. |
  | <Field name="external_id" type="string" recommended /> | Stable provider ID for exact lookup and relation resolution. |
  ...
  | <Field name="fields" type="object" required /> | Kind-specific searchable content. Include a matching `fields.kind` and the fields required for that app kind. |
  ...
  | <Field name="comments" type="array" /> | Optional embedded snapshot/backfill comments. (default=`[]`) |
  | <Field name="relations" type="array" /> | Typed app-source links. See [App Sources](/essentials/v2/app-sources) for relation targets and predicates. |
</Accordion>
...
<Accordion title="3. User memories - store notes or infer user preferences">
  ...
  | <Field name="additional_metadata" type="object" /> | Free-form per-memory fields. Keep this as an object inside each memory item; unlike `metadata`, do not JSON-stringify it again. (default=`{}`) |
