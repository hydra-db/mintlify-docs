...
> **SDK required**: Install the official Python SDK - `pip install "hydradb-sdk>=2,<3"`. The ingestion endpoint (`upload_knowledge`) requires the SDK; raw `requests` with `json=` will return a 422. Note: the import name differs from the package name.

<CodeGroup>
```python Python SDK
import os
from hydra_db import HydraDB        # pip install "hydradb-sdk>=2,<3"
