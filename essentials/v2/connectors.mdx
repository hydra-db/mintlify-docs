...
Attach free-text guidance that steers how a connector's documents are interpreted and indexed  -  for example, telling the pipeline that a rebranded product's old and new names are the same thing, or that a channel's threads are incident retros. Set a connector-wide default with `custom_instructions` on create or `PATCH`, and override it per resource where one connector spans different kinds of content. Purely additive: every existing connector supports it with a single `PATCH`, taking effect from the next sync.

See [Custom Ingestion Instructions](/essentials/v2/connector-instructions) for semantics, examples, and the resource-level override rules.
