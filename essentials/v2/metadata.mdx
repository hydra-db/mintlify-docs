...
| --- | --- | --- | --- |
| Database metadata | `metadata` | Top-level keys in `metadata_filters` | Stable, high-cardinality fields you filter on often. Declare these in `database_metadata_schema`, usually with `enable_match: true`. |
...
<Note>
The `database_metadata` request field (on `PATCH /context/{source_id}/metadata`) was formerly `tenant_metadata`, and `database_metadata_schema` (on `POST /databases`) was formerly `tenant_metadata_schema`; the old names still work as deprecated aliases. At `/query` and `/context/list`, the source metadata field is `metadata`, which still accepts `tenant_metadata` as a legacy alias.
</Note>
...
## 3. Define database metadata schema
...
- Up to 32 custom database metadata fields.
...
Use [`PATCH /context/{source_id}/metadata`](/api-reference/v2/endpoint/update-source-metadata) when you know the source ID and need to update metadata in place.
...
`/context/list` also accepts `tenant_metadata` as an alias for `metadata` and `document_metadata` for `additional_metadata`.
...
| Metadata edit returns 400 | Unknown key, wrong type, over-size value, too-deep nesting, reserved key, or missing `collection` | Check schema and validation limits; send only `database_metadata` / `additional_metadata`. |
...
- [Update Metadata](/api-reference/v2/endpoint/update-source-metadata)  -  point metadata edits
