...
```json
{
  ...
  "id": "<id>",
  "tenant_id": "<internal_database_id>",
  "database": "<database>",
...
| `database` | The name of the database you ingested into — the value you sent as `database` (or `tenant_id`) on the ingest request. Empty when HydraDB cannot determine it. |
...
| `tenant_id` | Deprecated. HydraDB's **internal** identifier for the database — not the name you ingested into. Always present. |
...
you ingested into (`marketing-docs`); `tenant_id` is HydraDB's internal identifier
for it (`kv3qz7mabx`). Route and filter on **`database`** — it is the only field
that matches what you sent.

`tenant_id` still carries the internal identifier it always has, so integrations
...
<Warning>
`database` is empty when HydraDB cannot determine the name — a malformed ingest
payload, or an item queued before this field was populated. `tenant_id` is always
set, so use it as a fallback if you need a guaranteed non-empty scope.
</Warning>
