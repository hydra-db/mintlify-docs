...
| <Field name="additional_metadata" type="object" /> | Context item's `additional_metadata` payload | Free-form per-document JSON. No schema declaration required. `document_metadata` is also accepted as a legacy alias; `additional_metadata` (canonical) wins if both are sent. |
