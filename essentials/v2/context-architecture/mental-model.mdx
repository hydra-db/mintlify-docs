...
**Architecture role:** Dynamic, user-scoped context. Ingest with `type=memory`. Query with `type: "memory"` or merge via `type: "all"`. Scope with `collection` so one user's preferences never leak into another's retrieval.
