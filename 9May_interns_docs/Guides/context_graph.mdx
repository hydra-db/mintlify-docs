...
**Triplets.** The unit of the graph. Each triplet is `source → relation → target`, where `source` and `target` are entity objects and `relation` describes the connection between them. Example: `billing_policy → governs → failed_payment_handling`.
