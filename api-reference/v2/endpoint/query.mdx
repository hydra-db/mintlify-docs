...
Use the same endpoint for Knowledge and Memories; the `type` field selects which store to search:

| If you need... | Set `type` to | Searches |
| --- | --- | --- |
| Shared documents, files, and app-source content | `"knowledge"` | Knowledge only |
| User-specific preferences, history, and inferred traits | `"memory"` | Memories only, scoped by `collection` |
| Personalized answers grounded in shared context | `"all"` | Knowledge and Memories together, merged and re-ranked |

`infer` does not appear on query requests. It only affects how a Memory was processed earlier during [`POST /context/ingest`](/api-reference/v2/endpoint/ingest-context).
...
| <Field name="type" type='"knowledge" or "memory" or "all"' /> | Which store to query. Use `"knowledge"` for shared documents/app sources, `"memory"` for user-specific Memories, and `"all"` when an answer needs both shared context and personalization. `"all"` runs Knowledge and Memories in parallel and merges by `relevancy_score`. (default=`"knowledge"`) |
...
| <Field name="metadata_filters" type="object or null" /> | Deterministic narrowing before ranking. See [Filters](#decision-matrix). Each list holds at most 500 values, and the whole object is capped at 64 KiB of compact JSON; over either returns `400`. (default=`null`) |
