...
| <Field name="database" type="string" required /> | Target tenant. |
| <Field name="collection" type="string" /> | Logical partition inside the tenant. (default=`""`  -  default sub-tenant) |
...
<Accordion title="App sources - index text from your workspace or personal apps">
  ...
  | <Field name="database" type="string" required /> | Target tenant. Must match the form-level `database`. |
  | <Field name="collection" type="string" required /> | Logical partition inside the tenant. Must match the form-level `collection`. |
...
- **Sub-tenant defaulting.** Omitting `collection` writes to the default sub-tenant. List available sub-tenants with [List Sub-Tenants](/api-reference/v2/endpoint/list-sub-tenants).
