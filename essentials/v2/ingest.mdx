...
`ids` are the `context_id`s of the related items. At query time, with `follow_forceful_relations` on (the default), a hit on one item pulls its declared relations into the response's `relations[]`, each with the `via` link that brought it in. See [Query](/essentials/v2/query#relations).
