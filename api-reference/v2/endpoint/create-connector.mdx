...
| <Field name="provider_account_scope" type="string" /> | Identifier for the external account (e.g. Slack workspace ID, GitHub org name). It is part of every synced context's ID, so use a distinct value for each connector of the same provider. |
| <Field name="credentials" type="object" required /> | Provider-specific credentials, matching the provider's `credential_schema` from [Get Connector Provider](/api-reference/v2/endpoint/get-connector-provider). Token-based providers typically take `{ "api_token": "..." }` or `{ "access_token": "..." }`. |
