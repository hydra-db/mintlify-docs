...
- `metadata`: schema-aligned, declared at database creation, fast filter path.
...
- `metadata_filters` top-level keys match `metadata`.
...
- `mode: "auto"` (default when `mode` is omitted) scores the query and routes to `"fast"` or `"thinking"` automatically, and overrides `graph_context` to match whichever it picks. Set `mode` explicitly for a deterministic pipeline.
...
- [ ] Using undeclared `metadata` keys in `metadata_filters`.
