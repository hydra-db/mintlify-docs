...
This is separate from the entity and relationship [context graph](/essentials/v2/context-graphs) HydraDB builds automatically. Graph extraction is _derived_ from content; forceful relations are _declared_ by you  -  useful when you know two documents belong together (a contract and its addendum, a runbook and its troubleshooting guide) but the text doesn't make the connection explicit. Forceful relations link whole **sources**; to declare the full entity/relation graph _within_ a single document (replacing extraction for it), use [Bring Your Own Graph](/essentials/v2/bring-your-own-graph).
