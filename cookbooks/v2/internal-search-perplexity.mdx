...
**Per-connector dependencies.** Each source below pulls in its own client library and its own credentials. Install and export only the ones for the sources you actually wire up  -  every snippet reads these directly, so a missing one fails immediately with a `KeyError`:

| Source | Install (Python) | Environment |
| --- | --- | --- |
| Slack | `pip install slack_sdk slack_bolt` | `SLACK_BOT_TOKEN` |
| Gmail | `pip install google-auth google-api-python-client` | an OAuth credentials JSON file, passed as `credentials_path` |
| Confluence | `pip install requests` | `CONFLUENCE_BASE_URL`, `CONFLUENCE_EMAIL`, `CONFLUENCE_API_TOKEN` |
| GitHub | `pip install PyGithub` | `GITHUB_TOKEN` |
| Answer synthesis | `pip install openai` | `OPENAI_API_KEY` |

The TypeScript variants use `@slack/web-api`, `@octokit/rest`, `axios` and `openai`, and read the same environment variables.
...
```mermaid
graph LR
    A["Slack · Gmail · Confluence<br/>GitHub · Linear · Notion"] -->|"context items"| B["Ingestion Layer<br/>connectors/slack.py<br/>connectors/gmail.py<br/>connectors/confluence.py<br/>connectors/github.py"]
...
async function ingestContext(collection: string, items: unknown[]) {
  ...
  return (json.results ?? []).map((r: any) => r.id as string);
}
...
def get_user_profile(user_id: str) -> str:
    """Retrieve a user's stored profile via /query against their user-<id> collection."""
    result = client.query(
        database=TENANT_ID,
        collection=f"user-{user_id}",
