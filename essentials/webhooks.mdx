...
`signing_secret` is optional here. You can supply one at registration as shown, or omit it and have HydraDB generate a strong one for you (see [Manage the signing secret](#manage-the-signing-secret)). A secret you supply must be at least 16 characters. Whichever you choose, your receiver should verify `X-HydraDB-Signature` on every delivery.
...
<Info>
This changed in July 2026. Previously, omitting `signing_secret` on an edit removed the stored secret and silently disabled signing. To disable signing now, call `DELETE /webhooks/indexing/signing-secret` explicitly.
</Info>
...
**Generate a secret.** Send no body and HydraDB creates a strong one for you. The plaintext is returned exactly once.
...
<Warning>
Rotating takes effect immediately, with no overlap period. Deliveries in flight during a rotation are signed with the new secret, so a receiver that only knows the old one will reject them. To rotate without a gap, teach your receiver to accept both secrets, rotate, then retire the old one.
</Warning>
...
| Signatures started failing after a rotation | Rotation applies immediately. Confirm your receiver has the new secret deployed. |
