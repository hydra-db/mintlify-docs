...
<Accordion title="User memories - store notes or infer user preferences">
  <CodeGroup>
  ...
  | <Field name="metadata" type="string (JSON object)" /> | Database-schema fields as a **JSON-stringified** object (e.g. `"{\"department\":\"legal\"}"`). Unlike `document_metadata` and `app_knowledge`, memory items take this as a string, not an object. (default=`""`) |
  | <Field name="additional_metadata" type="object" /> | Free-form per-memory fields for display or bookkeeping. Unlike `metadata`, send this as a plain object, not a JSON string. (default=`{}`) |
  | <Field name="relations" type="object" /> | Forceful relations within the Memories store. Shape: `{ "ids": ["...", "..."] }`. |
</Accordion>

<Accordion title="Chat/LLM conversation pairs - store chat history or support context">
  <CodeGroup>
