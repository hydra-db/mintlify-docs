...
description: "Model, load, and query your property graph in HydraDB."
---

Bring Your Own Graph (BYOG) lets you use **Cypher** with HydraDB: you read and
write [HydraDB's core graph database](https://github.com/hydra-db/hydradb)
directly, with your own schema and your own queries. We host it in the cloud
and it scales automatically, so there is nothing to provision or operate. It is
for teams that want to work with HydraDB in Cypher, including teams moving an
existing graph over from Neo4j or any other Cypher database.

<Tip>
  Moving an existing graph? Jump to
...
are invisible to you (they behave exactly like names that don't exist), and
`MATCH (n) RETURN n` only ever returns your own nodes.
...
- **One collection per query.** A collection is an independent graph inside
  the database; its data is isolated from every other collection.
...
`params` shown in its first comment. The **Response** tab is the `data` the
query returns.
...
- Internal node ids are not portable (true in most graph databases) - migrate using
...
one collection, verify, repeat. The example below reads from Neo4j with the
official driver; any source you can read nodes and relationships from works
the same way. It replays into HydraDB with the [`HydraGraph` client](#a-minimal-client) -
nodes first, then relationships matched on your own key. Relationship types
can't be parameters in Cypher, so write one batch per type:
...
If you only need your own entities and relations used for a document or memory
you are already ingesting - instead of HydraDB's LLM extraction - pass a
`graph_payload` on [`POST /context/ingest`](/api-reference/v2/endpoint/ingest-context).
It is a JSON map keyed by source id (a `document_metadata` `id`, an
`app_knowledge` `id`, or a memory `id`):
