...
def build_context_string(result):
    lines = []
    ...
    if chunks:
        ...
        for i, chunk in enumerate(chunks):
            ...
            source = chunk.get("source_title") or ""
            if source:
                lines.append(f"Source: {source}")
            lines.append(chunk.get("chunk_content") or "")

            # Graph relations for this chunk
            chunk_uuid = chunk.get("chunk_uuid") or ""
                ...
                if relevant_relations:
                    ...
                    for rel in relevant_relations:
                        for triplet in (rel.get("triplets") or []):
                            ...
                            pred = relation.get("canonical_predicate", "")
                            ctx = relation.get("context", "")
                            line = f"  [{src}] -> {pred} -> [{tgt}]: {ctx}"
                            temporal = relation.get("temporal_details")
                            if temporal:
                                line += f" [Time: {temporal}]"
                            lines.append(line)
            ...
            if extra_ids and additional_context:
                ...
                if extras:
                    ...
                    for extra in extras:
                        extra_source = extra.get("source_title") or ""
                        extra_content = extra.get("chunk_content") or ""
