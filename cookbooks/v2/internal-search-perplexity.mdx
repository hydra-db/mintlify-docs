...
async function getUserProfile(userId: string): Promise<string> {
  /** Retrieve a user's stored memory profile via `client.query(type="memory")`. */
...
async function explainDecision(
  ...
  /**
   * Answer 'why' / 'what led to' questions with full provenance.
   * Step 1: search chunks + graph relations from `client.query(type="knowledge")`.
   * Step 2: retrieve user profile from `client.query(type="memory")`.
   * Step 3: synthesize with citations via LLM.
   */
  // Step 1: Search with graph context
