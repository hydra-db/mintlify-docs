...
The deeper trade-offs  -  when to spin up a new database vs. a new collection, how scoping interacts with [metadata filters](/essentials/v2/attributes), and how this affects [Memories](/essentials/v2/memories)  -  are covered in [Multi-Tenant Support](/essentials/v2/databases-and-collections).
...
6. **Shape the response.** Return ranked `chunks`, graph paths in `graph`, declared `relations`, and `llm_prompt`, the same context as one prompt-ready string with citation labels.
