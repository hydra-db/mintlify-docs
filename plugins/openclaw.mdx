...
<Note>
  This plugin's own config keys are named `tenantId`/`subTenantId` (and `HYDRA_OPENCLAW_TENANT_ID`). These map 1:1 to the current API's `database` and `collection` fields (see [Multi-tenant](/essentials/v2/multi-tenant)) - set them to the same values you'd use for `database`/`collection` elsewhere.
</Note>
...
- **Auto-Recall** - Before every AI turn, queries HydraDB (`POST /query`, `type: "memory"`) for relevant memories and injects graph-enriched context: entity paths, chunk relations, and linked extra context.
- **Auto-Capture** - After every AI turn, the last user/assistant exchange is sent to HydraDB (`POST /context/ingest`, `type=memory`) with `infer: true` and `upsert: true`. The session ID is used as the memory `id` so HydraDB groups exchanges per session and builds the knowledge graph automatically.
