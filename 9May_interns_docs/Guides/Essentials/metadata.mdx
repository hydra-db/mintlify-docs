...
Use the top-level `metadata` field for stable attributes that you expect to filter on later, such as:
...
Multiple keys are treated as structured filter constraints; use semantic recall or downstream reranking for fuzzy, range, or preference-based matching. Use it when the filter value is known exactly. Operators such as range, contains, or fuzzy matching are not supported.
...
Use the metadata field names you planned in `tenant_metadata_schema`.
...
If a field is not declared with `enable_match: true`, HydraDB ignores that filter. Plan filterable fields before ingestion so your filters actually narrow recall.
...
`additional_metadata` is not matched against `metadata_filters`. If you want a field to be filterable, put it in `metadata` and declare it in `tenant_metadata_schema`.
