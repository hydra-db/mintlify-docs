...
Deletion is asynchronous. A `200` response means cleanup was scheduled, not that it finished. There is no collection-level completion endpoint, and a later `DELETE` `200` is also not a completion signal - the call is idempotent while cleanup is still running.

Do not reuse the collection name. Do not resume writes or queries against it after calling this API.
