...
<Tip>
To see the metadata actually attached to a source — including keys a connector added automatically — open the dashboard's [Context page](https://app.hydradb.com/context), select the source, and switch to the **Metadata** tab. Those keys are exactly what `metadata_filters` matches against.
</Tip>
