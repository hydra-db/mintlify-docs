...
| <Field name="collection" type="string or null" /> | Single collection scope. Required for per-user memory queries. If both `collection` and `collections` are omitted, defaults to the resolved primary tenant ID (the default collection). Formerly `sub_tenant_id`; the `sub_tenant_id` alias is still accepted (deprecated). (default=default collection) |
...
- **Common mistakes.** Check [Ingestion Status](/api-reference/v2/endpoint/source-status) for recently ingested documents before querying. If you omit `collection` and `collections`, HydraDB defaults to the resolved primary tenant ID (the default collection); use [List Collections](/api-reference/v2/endpoint/list-sub-tenants) to discover available IDs.
</Warning>
