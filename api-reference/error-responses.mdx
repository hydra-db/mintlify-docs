...
- `TENANT_NOT_FOUND`: Specified tenant does not exist. This error also occurs on ingestion endpoints (`POST /ingestion/upload_knowledge`, `POST /memories/add_memory`) when the tenant's vectorstore collection has not been provisioned yet. Use `GET /tenants/infra/status?tenant_id=<your_tenant_id>` to check provisioning progress before ingesting data.
