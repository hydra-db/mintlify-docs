...
Pass a list to accept **any one** of several values for a key. Lists are OR/ANY:
...
That matches sources in *either* department that are also `published`  -  separate keys still AND together. A single-element list behaves the same as the scalar form.

<Warning>
  **A list means ANY, not ALL.** There is no operator that requires a single key to hold every listed value; `["finance", "legal"]` also returns sources tagged only `finance`. To require multiple values, model the combination as its own field, split the constraint across separate keys, or intersect the results of several recalls client-side.
</Warning>

Lists are supported on `VARCHAR` fields only. A list passed for a field declared as `INT8`/`INT16`/`INT32`/`INT64`/`FLOAT`/`DOUBLE`/`BOOL` is rejected with `400 VALIDATION_ERROR` rather than silently returning nothing. Undeclared fields and everything under `additional_metadata` are treated as `VARCHAR`, so lists work there.
