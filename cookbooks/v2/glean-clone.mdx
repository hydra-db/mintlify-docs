...
def search(query: str, database: str, collection: str = None,
           ...
           metadata_filters: dict = None, query_apps: bool = True):
    return client.query(
        query=query,
        database=database,
        ...
        query_apps=query_apps,  # Required for connector / app_knowledge retrieval
...
def query_by_source_type(query: str, database: str, source_type: str):
    return search(query, database, metadata_filters={"source_type": source_type})

# Filter search by source title
def query_by_source_title(query: str, database: str, source_title: str):
    return search(query, database, metadata_filters={"source_title": source_title})
```
```typescript TypeScript SDK
import { HydraDBClient } from "@hydradb/sdk";
...
const search = async (query: string, database: string, options: SearchOptions = {}) => {
  ...
  return await client.query({
    ...
    queryApps,         // Required for connector / app_knowledge retrieval
...
const searchBySourceType = (query: string, database: string, sourceType: string) =>
  search(query, database, { metadataFilters: { source_type: sourceType } });

// Filter search by source title
const searchBySourceTitle = (query: string, database: string, sourceTitle: string) =>
  search(query, database, { metadataFilters: { source_title: sourceTitle } });
```
</CodeGroup>
...
const searchWithFilters = async (
  ...
  if (filters.sourceTypes) metadataFilters.source_type = filters.sourceTypes;
  if (filters.sourceTitles) metadataFilters.source_title = filters.sourceTitles;
...
const searchWithMemory = async (query: string, database: string, userId: string) => {
  ...
  const searchResults = await client.query({
    ...
    ...(userProfile.preferredSourceTypes?.length > 0 && {
      metadataFilters: { source_type: userProfile.preferredSourceTypes }
    })
  });
