...
// Put the app identity in kind/provider/externalId and the searchable text in
...
const normalizedData = {
  ...
  provider: 'slack',
  externalId: 'original_id', // Stable provider ID - enables exact lookup and relations
  ...
  if (event.type === "message") {
    const normalizedData = {
      ...
      fields: {
        ...
        createdAt: new Date(event.ts * 1000).toISOString(),
      },
      metadata: { channel: event.channel },
    };
