...
<Note>
  This CLI's own environment variables and `TENANT_ID` positional arguments map 1:1 to the current API's `database` field (see [Multi-tenant](/essentials/v2/multi-tenant)) - pass the same value you'd use for `database` elsewhere.
</Note>
