...
2. In-place [`PATCH /context/sources/{source_id}/metadata`](/api-reference/v2/endpoint/update-source-metadata) to merge values onto existing sources
...
| Attach or update field values on existing sources | Yes | [`PATCH .../sources/{source_id}/metadata`](/api-reference/v2/endpoint/update-source-metadata) (merge; omitted keys are preserved) |
...
Use Path B when the source content is already correct and you only need to attach or change metadata. Call [`PATCH /context/sources/{source_id}/metadata`](/api-reference/v2/endpoint/update-source-metadata).
...
3. Inspect the PATCH response before you query (see Verification).
...
<CodeGroup>
```bash cURL
curl -X PATCH 'https://api.hydradb.com/context/sources/auth-controls-001/metadata' \
  -H "Authorization: Bearer $HYDRA_DB_API_KEY" \
  -H "API-Version: 2" \
  -H "Content-Type: application/json" \
  -d '{
...
response = requests.patch(
    "https://api.hydradb.com/context/sources/auth-controls-001/metadata",
    headers={
        "Authorization": f"Bearer {os.environ['HYDRA_DB_API_KEY']}",
        "API-Version": "2",
        "Content-Type": "application/json",
...
If an edited `tenant_metadata` field has `enable_dense_embedding` or `enable_sparse_embedding`, HydraDB refreshes the relevant vector-store metadata search lane as part of the edit. Check the response:

- `vector_sync_required: false` — MongoDB-only (`enable_match`) edit; safe to query after a successful response
- `vector_sync_required: true` and `vector_synced: true` — dense/sparse lane refreshed; safe to query
- HTTP `500` with MongoDB written but vector sync failed — **retry the same idempotent PATCH** until `vector_synced` is `true` (or sync is no longer required). Do not treat the first `500` as final and do not verify with query yet

Deprecated aliases `milvus_sync_required` / `milvus_synced` may still appear; prefer `vector_*`.
...
1. Confirm the PATCH returned `success: true` (or a successful HTTP status with `updated: true`).
2. If `vector_sync_required` is `true`, wait until `vector_synced` is `true`. If the request returns `500` because MongoDB updated but vector sync failed, retry the **same** PATCH — the edit is idempotent and retries converge the vector lane.
3. Only then confirm filters with [`POST /query`](/api-reference/v2/endpoint/query).
...
- **Retry failed vector sync**  -  a `500` after MongoDB write means dense/sparse sync failed; retry the same PATCH before verifying with query.
