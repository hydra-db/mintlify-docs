...
- **Response format:** See [Response Format](/api-reference/v2/response-format)
...
**Response envelope:** Core v2 endpoints (`/databases`, `/context/*`, and `/query`) return a consistent envelope. Endpoint pages show the full envelope; the resource-specific payload lives under `data`. See [Response Format](/api-reference/v2/response-format) for the canonical success envelope and the full `meta` field reference (`request_id`, `api_version`, scope echo, `deprecation[]`).
...
Errors use the same envelope with `success: false`, `data: null`, and an `error` object containing `code` and `message`. See [Error Responses](/api-reference/v2/error-responses).

`meta` may also include a `deprecation` list when a request uses a legacy `/tenants` route or a deprecated field (`tenant_id`/`sub_tenant_id`, or `sub_tenant_ids` on `/query`); each entry carries `deprecated`, a `message`, and `deprecated_since` (field-level notices also add `deprecated_field` and `preferred_field`). It is a non-breaking migration nudge (the status code is unchanged) and is accompanied by a `Deprecation: true` response header. See [Response Format](/api-reference/v2/response-format#detecting-deprecations-at-runtime) and [Migrating from `tenant_id` and `sub_tenant_id`](/essentials/v2/multi-tenant#7-migrating-from-the-legacy-tenant-and-sub-tenant-fields).
