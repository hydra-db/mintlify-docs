...
result = client.query(
    database="your-tenant",
    collections=["your-sub-tenant"],
...
| <Field name="collections" type='string[] or { [collection]: positive number (max one decimal place) }' /> | Preferred query-time collection selector. Send a list for equal normalized weights, or an object to fan out with relative ranking weights with at most one decimal place. Maximum 100 collections. |
| <Field name="collection" type="string or null" /> | Single collection scope. Deprecated aliases `sub_tenant_id` / `sub_tenant_ids` are still accepted; prefer `collection` / `collections`. Note: ingest endpoints still use singular `sub_tenant_id`. |
...
- **Common mistakes.** Check [Ingestion Status](/api-reference/v2/endpoint/source-status) for recently ingested documents before querying. If you omit `collections` and deprecated `collection`, HydraDB queries the default sub-tenant. Use [List Sub-Tenants](/api-reference/v2/endpoint/list-sub-tenants) to discover available IDs.
</Warning>
