...
| <Field name="acl" type="string[]" /> | Who may retrieve this item. Emails, `group:<provider>:<id>`, `domain:<domain>`, or `__public__`. | Omit to leave it unrestricted; a query only sees it when the query carries a matching `acl`. See [Access Control](/essentials/v2/access-control). |
