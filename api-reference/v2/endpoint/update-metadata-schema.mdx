...
- Filter indexes for `enable_match` fields are created before the merged schema is persisted.

<Warning>
  This endpoint persists the updated schema and filter indexes. It does not yet alter or backfill existing vector-index collections for newly added dense/sparse metadata fields. Create the desired semantic metadata fields before ingesting, or migrate/re-ingest into a database with the final schema if those fields must participate in semantic/BM25 metadata search.
</Warning>
