...
| `collection` | Optional partition inside a database, often a user, team, account, or customer. | For a database holding data for many users or customers. Alias `sub_tenant_id` (deprecated). See [multi-tenancy](/essentials/v2/databases-and-collections) |
...
| [`/context/inspect`](/api-reference/v2/endpoint/fetch-content) | `GET` | `context.inspect` | Read a context's stored content | You need the full content behind a `context_id`. For title and attributes, use `POST /context/list` with `ids`. |
