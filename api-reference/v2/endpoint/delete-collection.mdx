...
Deletion is asynchronous. The `200` response means the request was accepted and background cleanup has started, not that every store has finished tearing down.

`GET /databases/collections` stops listing the collection once its source-of-record rows are gone. Vector, graph, and object-store cleanup can still be running after that. Do not reuse the same `collection` name, and do not resume writes or queries against it, until you have confirmed cleanup is finished (or waited long enough that a retry of this delete returns `200` against empty stores).
...
- **Stop in-flight work first:** Stop ingestion, polling, query, and background jobs targeting this collection before deleting. An in-flight ingest can write data after cleanup starts.
