...
Both file uploads and app knowledge uploads return the same response shape:
...
- **`results`**: One entry per uploaded file or app knowledge item
- **`success_count`**: Number of items successfully queued
- **`failed_count`**: Number of items that failed to upload (non-zero when some items fail)
