...
The REST API uses **snake_case** on the wire. The SDKs expose idiomatic names in each language and serialize to snake_case automatically.

| | Method naming | Request / response fields | Example |
|---|---|---|---|
...
| **TypeScript SDK** | camelCase for multi-word methods | camelCase for multi-word fields | method: `query()`, params: `database`, `queryBy`, `pageSize` |

Verified against `@hydradb/sdk@2.1.2`: TypeScript types use `queryBy`, `pageSize`, `graphContext`, `indexingStatus`, and `readyForIngestion`. Do **not** pass snake_case multi-word fields to the TypeScript client.

<Info>
**TypeScript:** use camelCase fields (e.g. `client.context.list({ database: "...", pageSize: 50 })`, `client.query({ queryBy: "hybrid" })`).

**Python:** use snake_case throughout (e.g. `client.context.list(database="...", page_size=50)`).

JSON strings you pass through as raw payloads (`memories`, `app_knowledge`, `document_metadata`) keep snake_case keys inside the string in both SDKs, because that content is wire JSON.
...
Your IDE's autocomplete and type checking work directly off the SDK types  -  if a field is optional in the API, it's optional in the SDK.
...
The method-reference tables above use the Python (snake_case) method names. TypeScript keeps the same method names but camelCases any that are multi-word (for example, the connector method `list_resources()` in Python is `listResources()` in TypeScript). Request and response fields are snake_case in Python and camelCase in TypeScript.
