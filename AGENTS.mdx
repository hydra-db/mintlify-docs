...
2. **`GET /databases/status`** (`client.databases.status()`): Poll until the scheduler, graph, and both vector stores report ready.
...
Many core v2 handlers use this success envelope:
...
- For an operation whose OpenAPI 2xx schema or SDK return type is a handler envelope, parse the resource payload from `data`. Core database, context, query, and webhook-management methods use this shape.
- Do not assume every v2 response has `data`. Connector catalog, discovery, and resource operations, plus database metadata-schema updates, can return direct objects. Follow the endpoint's 2xx schema or the SDK method's return type.
- On failure, SDK methods raise a typed exception rather than returning a success envelope with `error` set.
- Authentication or gateway middleware can reject a request before the handler and return a `detail` error shape. Branch on HTTP status before parsing a success envelope.
- Log a request or correlation ID when the response provides one.
- `meta` may also carry an optional `deprecation` list (with a `Deprecation: true` response header) when a request uses a legacy tenant route or a deprecated field (`tenant_id`/`sub_tenant_id`, or `sub_tenant_ids` on `/query`). Each entry has `deprecated`, `deprecated_field`, `preferred_field`, `message`, and `deprecated_since`. It is a non-breaking nudge. The status code is unchanged. Prefer the `/databases` routes and `database`/`collection`/`collections` fields to avoid it. Treat `meta` as an open object: ignore keys you do not recognize.
...
- `metadata`: schema-aligned source metadata, declared in the database schema, fast filter path.
...
- `metadata_filters` top-level keys match schema-backed `metadata`.
...
- For methods whose generated return type is a `{ success, data, error, meta }` envelope, read the payload under `.data` (e.g. `response.data.infra`, `response.data.statuses`, `response.data.results`). Connector catalog, discovery, and resource methods, plus database metadata-schema updates, can return direct objects instead.
...
const ingestId = ingest.data?.results?.find((item) => item.id && !item.error)?.id;
if (!ingestId) throw new Error("Ingestion returned no context ID");
...
while (true) {
  ...
  const status = statusEnvelope.data?.statuses?.[0];
  if (!status) throw new Error("Status response contained no item");
...
- Plan `database_metadata_schema` up front. You can add exact-match fields later with [Update Metadata Schema](/api-reference/v2/endpoint/update-metadata-schema); new semantic/BM25 metadata lanes for existing data may still require migration or re-ingestion.
...
Ready when all four infrastructure signals are true:
...
Python uses the snake_case spellings above; TypeScript uses `schedulerStatus`, `graphStatus`, and `vectorstoreStatus`.
...
| `metadata` | schema-aligned filterable fields |
  ...
  appKnowledge: JSON.stringify([
    {
      ...
      fields: {
        kind: "message",
        body: "We agreed on three tiers: Starter, Pro, and Enterprise.",
        author: "alice",
...
| `kind`, `provider`, `external_id` | app-native identity and provider namespace |
| `fields` | primary searchable content in the kind-specific shape |
| `metadata` | fast filters such as channel, project, region, workspace |
...
| `relations`, `attachments`, `comments` | optional app-native connected context |

On retrieval, the v2 OpenAPI exposes three app-source identity fields:

- `app_kind`: free-form string; the OpenAPI does not define an enum.
- `app_provider`: provider name such as `slack` or `github`.
- `app_external_id`: provider-assigned source identifier.

The v2 OpenAPI types `app_knowledge` only as a JSON string, so it cannot validate the nested item schema. For ingestion, follow the [App Sources](/essentials/v2/app-sources) guide's `kind` / `provider` / `external_id` / `fields` shape. On retrieval, treat `app_kind` as free-form rather than deriving a closed enum from the response schema.
...
| `metadata` | schema-aligned fields; for memory items, encode as a JSON string inside the JSON-stringified `memories` field |
...
```ts
while (true) {
  const envelope = await client.context.status({
    database: "acme_corp",
    ids: ["policy_v2"],
  });
  const status = envelope.data?.statuses?.[0];
  if (!status) throw new Error("Status response contained no item");
...
| `mode` | `fast`, `thinking`, `auto` | applies to hybrid; thinking improves quality but adds latency; auto lets the service choose |
...
- TypeScript: `buildString(result)` from `@hydradb/sdk/helpers`
...
|---|---|---|
| Hot-path exact filters: department, region, environment, status | `metadata` | top-level `metadata_filters` |
| Free-form display/bookkeeping: author, provider IDs, version | `additional_metadata` | `metadata_filters.additional_metadata` |
| Provider-specific values for UI/citations | `additional_metadata` | usually not filtered unless needed |
...
- For source metadata-only edits, use [Update Source Metadata](/api-reference/v2/endpoint/update-source-metadata); re-ingest with the same ID and `upsert: "true"` when content changes.
- Treat a metadata-key rename as a migration: add the schema field, update or re-ingest affected sources, then update queries.
...
- `filters.metadata` / TypeScript `filters.metadata`
- `filters.additional_metadata` / TypeScript `filters.additionalMetadata`
- `filters.source_fields` / TypeScript `filters.sourceFields`
...
| `502` | upstream provider/dependency error | Yes with backoff for idempotent operations |
...
Retry only `429` and transient `5xx` responses such as `500`, `502`, and `503`; retry writes only when they are idempotent. Use bounded exponential backoff with jitter.
...
try:
    ...
    if exc.status_code != 429 and exc.status_code < 500:
        raise
    # Retryable: back off with jitter, keep attempts bounded, then retry.
```
...
try {
  ...
  if (error instanceof HydraDBError) {
    ...
    console.error({ code: body?.detail?.errorCode, statusCode: error.statusCode });
  }
  throw error;
}
```
...
- [ ] Reading an SDK response payload from the top level instead of the envelope's `.data` field.
...
- [ ] Using undeclared schema-backed `metadata` keys in `metadata_filters`.
...
# 2. Check readiness. Poll until scheduler, graph, and both vector stores are true.
