...
**From the API:** include the `linear_workspace` resource in your `configure` call. To put the documents in their own sub-tenant partition, set `sub_tenant_id` on it, the same as any other resource:

```json
{
  "resource_id": "linear_workspace",
  "resource_type": "linear_workspace",
  "sub_tenant_id": "linear-docs"
}
```
...
- [Metadata](/essentials/v2/attributes)  -  tenant metadata vs document metadata in depth
- [Multi-Tenant](/essentials/v2/databases-and-collections)  -  routing resources to tenants and sub-tenants
