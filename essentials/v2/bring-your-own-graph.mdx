...
description: "Model, load, and query your own property graph in HydraDB with full Cypher."
---
...
<Tip>
  Already have entities and relations for a document and only want them used
  instead of LLM extraction? See [Without Cypher](#2-without-cypher-attach-a-graph-at-ingest).
</Tip>
...
official driver and replays into HydraDB with the `HydraGraph` client above -
...
Create an index on the key (`CREATE INDEX FOR (n:Person) ON (n.ext_id)`)
before step 2 so the relationship `MATCH` stays fast.

## 2. Without Cypher: attach a graph at ingest
