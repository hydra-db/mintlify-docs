...
This action is irreversible. Deleting a collection removes all context, embeddings, and graph data stored under that collection. The parent database and its other collections are not affected. There is no soft-delete and no recovery window.
...
| Name | Description |
| --- | --- |
| <Field name="database" type="string" required /> | Database that owns the collection. Alias `tenant_id` (deprecated). |
| <Field name="collection" type="string" required /> | Collection to delete. No default. Alias `sub_tenant_id` (deprecated). |
...
- The collection disappears from [List Collections](/api-reference/v2/endpoint/list-sub-tenants) as soon as the delete is accepted, before the vector, graph, and object-store cleanup completes.
...
Common codes: `400 INVALID_INPUT` (missing `database` or `collection`), `404 DATABASE_NOT_FOUND`, `401 UNAUTHORIZED`, `503 SERVICE_UNAVAILABLE` (a failed cleanup is still releasing the collection; retry shortly). See [Error Responses](/api-reference/v2/error-responses) for the full list.
...
- **Alternative:** [Delete Context](/api-reference/v2/endpoint/delete-source): remove specific context by ID without deleting the collection
