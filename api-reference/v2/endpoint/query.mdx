...
A list gives every sub-tenant equal normalized weight. An object treats values as positive relative ranking weights with at most one decimal place and normalizes them server-side. You can send at most 100 sub-tenants. When `max_results` is omitted, HydraDB uses up to 10 results per sub-tenant, capped at 1000 fanout candidates before the final ranked response is shaped. When `max_results` is set, it is the final global response cap across the merged fanout result set.

> **Caching tip:** `sub_tenant_ids` list order is not semantically significant for fanout selection. Sort list values before constructing cache keys; for weighted objects, sort keys and keep weights at the documented one-decimal precision so equivalent calls share the same cache entry.
...
| <Field name="sub_tenant_ids" type='string[] or { [sub_tenant_id]: positive number (max one decimal place) }' /> | Preferred query-time sub-tenant selector. Send a list for equal normalized weights, or an object to fan out with relative ranking weights with at most one decimal place. Maximum 100 sub-tenants. |
| <Field name="sub_tenant_id" type="string or null" /> | Deprecated query-time single sub-tenant selector. Use `sub_tenant_ids: ["your-sub-tenant"]` instead. If both selectors are omitted, defaults to the resolved primary tenant ID. Ingest endpoints still use singular `sub_tenant_id`. |
...
- **Common mistakes.** Check [Ingestion Status](/api-reference/v2/endpoint/source-status) for recently ingested documents before querying. If you omit `sub_tenant_ids` and deprecated `sub_tenant_id`, HydraDB defaults to the resolved primary tenant ID. Use [List Sub-Tenants](/api-reference/v2/endpoint/list-sub-tenants) to discover available IDs.
</Warning>
