...
`data.llm_prompt` is the whole context block as one markdown document: `# Query results`, then `## Results` (cited `[1]`), `## Forceful relations` (`[R1]`), `## Related facts` (`[P1]`, one per path in `graph[]`), `## Temporal facts` and `## Sources`. Inject it directly:
