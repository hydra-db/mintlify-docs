...
<Note>
  **Chunk metadata field name.** On the wire, each chunk carries its free-form per-source fields under `additional_metadata` (as shown above). The Python and TypeScript SDK chunk objects expose that same data as `.metadata` (e.g. `chunk.metadata`), not `.additional_metadata`. Only the SDK attribute name differs; the raw JSON key stays `additional_metadata`.
</Note>
