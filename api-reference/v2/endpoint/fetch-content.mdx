...
| <Field name="collection" type="string or null" /> | Collection scope. If omitted, defaults to the resolved primary tenant ID (the default collection). Formerly `sub_tenant_id`; the `sub_tenant_id` alias is still accepted (deprecated). (default=`null`) |
