---
title: "Build an AI Financial Analyst with Memory"
...
# SDK client — required for /ingestion/upload_knowledge (multipart/form-data)
...
TENANT_ID = "financial-analyst"
BASE_URL  = "https://api.hydradb.com"
HEADERS   = {"Authorization": f"Bearer {os.environ['HYDRADB_API_KEY']}", "Content-Type": "application/json"}
...
print(answer)
# → Returns a table: Q1 71.2% → Q2 72.8% → Q3 71.5% → Q4 73.1%
# → Cites each quarter's source document
# → Notes the Q3 dip and the recovery context
```
...
print(arc)
# → Q1 2023: Bullish ("We're hiring aggressively across all functions")
# → Q2 2023: Neutral ("We continue to invest in the right roles")
# → Q3 2023: Cautious ("We're being more selective with headcount")
# → Q4 2023: Defensive ("We've right-sized the org for the current environment")
# → Narrative arc: Clear pivot from growth-mode hiring to efficiency focus mid-year.
```
...
def financial_qna(
    ...
    user_id: str,
    ticker: str = None,
    doc_type: str = None,       # "earnings_transcript" | "internal_metrics" | "board_memo"
...
r3 = financial_qna(
    "Does the Q3 2023 board memo address the churn increase we saw in the metrics?",
    user_id="priya",
    ticker="ACME",
    fiscal_year=2023,
    fiscal_quarter=3,
...
def extract_quarter_info(text: str) -> tuple[int | None, int | None]:
