...
<Info>
  **Backward Compatibility**: The legacy field name `app_sources` is still accepted as an alias for `app_knowledge` but is deprecated — migrate to `app_knowledge`.
</Info>
...
<Warning>
  **Tenant not provisioned (HTTP 404)**: If you receive a `404 TENANT_NOT_FOUND` error, the tenant's vector store collection has not finished provisioning. Poll `GET /tenants/infra/status?tenant_id=<your_tenant_id>` until the status is `ready` before retrying the upload. See [Monitor & infra status](/api-reference/endpoint/monitor-tenant#check-tenant-infra-status) for details.
</Warning>
