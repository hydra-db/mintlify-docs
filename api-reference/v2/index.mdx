...
| `database_metadata_schema` | Database-level fields you define up front so metadata can be filtered or queried consistently. | Use it for stable fields like department, customer, region, plan, category, or compliance label. (`tenant_metadata_schema` is a deprecated alias.) |
...
| [`/context/sources/{source_id}/metadata`](/api-reference/v2/endpoint/update-source-metadata) | `PATCH` | REST | Update metadata | You need to merge `metadata` or `additional_metadata` onto one existing source without re-ingesting. |
