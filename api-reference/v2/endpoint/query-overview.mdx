...
| `graph[]` | Paths through the context graph: `origin` (`query_path` or `chunk_relation`), `triplets[]` and a `path_summary`. Each hop's `relation.chunk_id` names the chunk it came from. |
| `forceful_relations[]` | Chunks linked with `forceful_relations` at ingest, each with the `via` that brought it in. |
| `llm_prompt` | A server-built markdown string, ready to inject into a model call: results cited `[1]`, forceful relations `[R1]`, related facts labelled `[P1]` in `graph[]` order, then temporal facts and sources. |
