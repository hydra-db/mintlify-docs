...
See [Metadata](/essentials/v2/metadata) for schema design, filter examples, and the `metadata` vs `additional_metadata` decision rule. See [Create Tenant](/api-reference/v2/endpoint/create-database) for the schema field configuration (`enable_match`, `enable_dense_embedding`, `enable_sparse_embedding`).
...
| Using `additional_metadata` for hot filterable fields | Filters work only with nested syntax and are less efficient than schema-backed metadata filters | Put hot filter keys in `metadata` and declare them in the [tenant schema](/api-reference/v2/endpoint/create-database), or filter under `metadata_filters: {"additional_metadata": {...}}` for occasional/free-form fields  -  see [Metadata](/essentials/v2/metadata). |
