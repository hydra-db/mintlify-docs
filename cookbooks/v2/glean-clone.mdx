...
// Put the app identity in kind/provider/external_id and the searchable text in
// fields (fields.body for messages/emails, fields.description for tickets, etc.) -
// that's what unlocks app-aware search (thread expansion, actor lookup, exact ID match).
// NOTE: this whole object gets JSON.stringify'd into appKnowledge, so these keys
// are NOT auto-converted by the SDK - use the API's snake_case names here, even
// though the rest of this file uses camelCase for normal typed SDK parameters.
// See https://docs.hydradb.com/essentials/v2/app-sources for the full per-kind reference.
const normalizedData = {
  id: 'unique_id',
  database: 'your_database',
  collection: 'your_collection',
  title: 'Document/Message Title',
  type: 'slack', // Provider category: gmail, slack, notion, jira, etc.
  kind: 'message', // App object kind: email, message, ticket, knowledge_base, comment, custom
  provider: 'slack',
  external_id: 'original_id', // Stable provider ID - enables exact lookup and relations
