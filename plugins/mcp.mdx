...
Both ways to run HydraDB expose the exact same tools - pick whichever fits:

- **Remote MCP server** - point your client at the hosted URL `https://mcp.hydradb.com`. Nothing to install or update; you authenticate per request with headers.
- **Local MCP server** - your client spawns the `@hydradb/mcp` binary (via `npx`) on your own machine over stdio.

<Steps>
  <Step title="Get your credentials">
    Both methods use the same two values:
  ...
  <Step title="Connect your client">
    <Tabs>
      <Tab title="Remote MCP server">
        ...
        credentials as headers - nothing to install. The key names differ by
        client: VS Code uses the `servers` schema with an explicit `type`, the
        others use `mcpServers`. See [authentication](#authenticating-to-the-remote-server)
        for every header.
  ...
  <Step title="Replace the placeholders and restart">
    ...
    values from Step 1, then restart your editor or client so it picks up the
    new MCP server.
  </Step>
</Steps>

<Info>
  **Local MCP server only:** the canonical tool names and `HYDRADB_*` variables need
  **@hydradb/mcp 1.1.0 or later**. If your config pins `@0.0.1` or `@1.0.0`,
  change it to `@^1.1.0` and restart. Nothing else changes - every name you were
  using before still works, and the caret keeps you on the 1.x line this page
  documents.
</Info>
...
Every request to the hosted URL carries its own credentials, so one server
serves any number of independent users. Send them as headers:
...
<Tip>
  **Per-call tenant switching:** Multi-tenant agents serving multiple companies
  ...
  When omitted in tool arguments, calls fall back to the default scope configured
  via the `X-HydraDB-Database` / `X-HydraDB-Collection` headers.
</Tip>
