...
<Warning>
This plugin page documents the current OpenClaw packaging, which still uses the v1 tenant / recall surface. For HydraDB **API v2** integrations, use `database` / `collection` and `POST /query` — see the [Agent Integration Guide](/AGENTS) and [Quickstart](/get-started/v2/quickstart).
</Warning>
