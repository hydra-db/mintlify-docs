...
Start by creating a connector — this is always the first step. All subsequent endpoints operate on the connector ID returned by `POST /connectors`.

```text
POST /connectors                     -> create connector with credentials
GET  /connectors/:id                 -> get connector status and metadata
GET  /connectors                     -> list all connectors for the organization
