...
App sources are pre-parsed records from business applications: emails, chat messages, tickets, wiki pages, CRM objects, comments, and attachments. Use them when your connector already extracted the item's text and structured fields. Not sure whether app sources are the right path for your content? See [Choosing Your Ingestion Path](/essentials/v2/ingestion-decision-guide).
...
- [Choosing Your Ingestion Path](/essentials/v2/ingestion-decision-guide) - decision trees for when to use app sources vs files vs connectors
