...
- `mode: "auto"` (or omit `mode`) to let HydraDB choose.
...
| `mode` | `fast`, `thinking`, `auto` | applies to hybrid; omit or set `auto` to let HydraDB choose; `thinking` improves quality but adds latency |
...
- [ ] Reading SDK payloads from the top level instead of `.data` (SDKs return the full `{ success, data, error, meta }` envelope).
