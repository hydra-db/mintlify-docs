...
You do not format the graph yourself. The `llm_prompt` returned by the same query already contains a `=== GRAPH ===` section, one entry per path, labelled `[P1]`, `[P2]` and so on, with each step citing the label of the chunk it was extracted from when that chunk is in the response:
