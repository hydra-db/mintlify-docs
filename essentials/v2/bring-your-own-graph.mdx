...
write HydraDB's core graph database directly, with your own schema and your
own queries. It is for teams that want to work with HydraDB in Cypher,
including teams moving an existing graph over from Neo4j.

- **Databases** group your collections. A BYOG database is created only with
  [`POST /byog/databases`](#endpoints).
- **Collections** are independent graphs inside a database. Each query runs
  against exactly one collection, and each collection's data is isolated from
  every other collection.
- **Full Cypher** for reads and writes, plus multi-hop traversal and path
  finding - see [Supported Cypher](#supported-cypher).
- **Isolated per organization.** `MATCH (n) RETURN n` returns *your* nodes and
  nothing else.

<Tip>
  Coming from Neo4j? Jump to [Migrating from Neo4j](#migrating-from-neo4j).
</Tip>
...
one collection, verify, repeat. A minimal port reads from Neo4j with the
official driver and replays into HydraDB with the [`HydraGraph` client](#a-minimal-client) -
