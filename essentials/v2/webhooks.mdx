...
<Note>
**Signing scheme:** when a signing secret is configured, every delivery is signed with **HMAC-SHA256 over the raw request body**, sent as `X-HydraDB-Signature: sha256=<hex digest>`. Verify against the raw bytes before parsing JSON — see [Receiver examples](#5-receiver-examples).
</Note>
