...
| List values (OR / ANY) | A list means **match ANY** of the listed values. `tags: ["alpha", "beta"]` matches a source tagged `alpha`, a source tagged `beta`, or a source tagged both. Supported on `VARCHAR` fields only. |
| List values on non-`VARCHAR` fields | Rejected with `400 VALIDATION_ERROR`. Pass a single scalar instead. |
| ALL / AND within one key | **Not supported.** See [Matching several values on one field](#matching-several-values-on-one-field). |
...
<Note>
  **A list is OR, not AND.** Passing several values for one key widens the result set  -  it never narrows it. Different keys still AND together, so `{ "department": ["legal", "finance"], "status": "published" }` reads as *(department is legal OR finance) AND status is published*.
</Note>
...
### Filtering on several values at once (ANY)

Give a key a list to match a source holding **any one** of those values. This is the only list behavior HydraDB implements.

Two sources, ingested separately  -  one with `department: "finance"`, one with `department: "legal"`:

```json
{
  "database": "acme",
  "query": "What is our Q3 spending policy?",
  "metadata_filters": {
...
Both sources are candidates. A single-element list behaves the same as the scalar form, so `["finance"]` and `"finance"` are interchangeable.

The same applies to free-form fields nested under `additional_metadata`:
...
If a source stores a multi-valued field as a comma-joined string (for example `tags: "alpha,beta"`), a list filter matches when **any** requested value is present in that set  -  `["alpha"]`, `["beta"]`, and `["beta", "gamma"]` all match it.

#### Matching several values on one field

There is **no ALL / AND operator for a single key**. You cannot ask for "sources tagged both `alpha` and `beta`" through `metadata_filters`  -  a list always means ANY.

If you need AND across values, pick one of these:

- **Model the combination as its own field.** Declare a dedicated key (for example `tier: "alpha_beta"`) and set it at ingest. This keeps the fast pre-filter path.
- **Split across different keys.** Separate keys AND together, so `{ "language": "en", "status": "published" }` requires both.
- **Intersect client-side.** Query once per value and keep the source IDs present in every result set.

<Warning>
  A list filter never means ALL. `tags: ["alpha", "beta"]` returns sources tagged only `alpha` as well  -  it does not restrict results to sources carrying both values.
</Warning>

#### Lists are `VARCHAR`-only

List values are implemented for `VARCHAR` fields. A list against a field declared as `INT8`, `INT16`, `INT32`, `INT64`, `FLOAT`, `DOUBLE`, or `BOOL` is rejected outright rather than silently returning nothing:

```json
{
  "error": {
    "code": "VALIDATION_ERROR",
    "message": "metadata_filters.priority: list/array values are not supported for INT64 fields (only VARCHAR); pass a single scalar value instead"
  }
}
```

Fields you never declared in `database_metadata_schema`, and everything under `additional_metadata`, are treated as `VARCHAR`  -  lists work there as normal.
...
- **Use equality and ANY-of-a-list only.** A scalar is exact equality; a list on a `VARCHAR` field matches any one of its values. Range, contains, fuzzy matching, and ALL-of-a-list belong in the query, query mode, or downstream reranking  -  not here.
...
| Range / contains / fuzzy scope doesn't work | `metadata_filters` supports equality and ANY-of-a-list only | Move that constraint into the `query` text, use a different `query_by`, or apply it in your own post-scoping. |
| A list filter returns *more* sources than expected | Lists are ANY (OR), not ALL | Expected  -  `["alpha", "beta"]` matches sources holding either value. There is no ALL operator; see [Matching several values on one field](#matching-several-values-on-one-field). |
| `400 VALIDATION_ERROR` naming a field and its type | A list was passed for a non-`VARCHAR` declared field | Lists are `VARCHAR`-only. Pass a single scalar for `INT*`/`FLOAT`/`DOUBLE`/`BOOL` fields. |
