...
## 5b. Undeclared ACL metadata filter

<AccordionGroup>
  <Accordion title="Problem">
    “Filtered” shared-collection query still returns every channel’s content.
  </Accordion>
  <Accordion title="Why">
    `channel_id` (or similar) was never declared with `enable_match: true`. Undeclared filter keys are **silently ignored**.
  </Accordion>
  <Accordion title="Fix">
    Declare the field before ingest; verify a wrong value returns empty; prefer collection partitions for ACL. [ACL Partitions → Pattern 2](/essentials/v2/scope-composition/acl-partitions#pattern-2--parallel-equality-filters--union).
  </Accordion>
</AccordionGroup>
