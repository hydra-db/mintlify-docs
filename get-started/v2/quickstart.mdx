...
This guide walks through the full HydraDB loop: create a database, ingest two contexts, wait for indexing, and run a query, using the [API](/api-reference/v2). By the end you have a working personalized-RAG flow you can plug into your own LLM prompt.
...
Create a database, ingest a company policy into a shared collection and a customer conversation into that customer's own collection, wait for both to index, then ask a question that needs both.
...
#    The SDK sends the list in the `context` form field.
...
# 4. Wait until both contexts are indexed.
...
```mermaid
flowchart LR
  A([1. Create Database]) --> B([2. Ingest Context])
...
Creating a database and indexing are **asynchronous**: HydraDB provisions infrastructure and indexes your content in the background, so each is followed by a short polling loop. The ingest call returns `202` as soon as the contexts are queued, and querying runs in real time. One ingest request takes up to 100 contexts in its `context` list; send more requests for more.
...
| If you want to... | Read... |
|---|---|
| See every context field, conversations and enrichment | [Ingest context](/essentials/v2/ingest) |
