...
`POST /query` returns retrieved context, not an answer: ranked `chunks`, graph paths in `graph`, declared `relations`, and `llm_prompt`, the same context as one prompt-ready string. A typical application flow is:
...
4. Read `chunks`, `graph` and `relations` when you need structure, and `GET /context/inspect` for a chunk's source details.
