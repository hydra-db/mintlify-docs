...
| `metadata` | Key-value pairs merged into the attributes of every synced object from this resource. Undeclared keys are accepted but only keys in `database_metadata_schema` are indexed for filtering. |
