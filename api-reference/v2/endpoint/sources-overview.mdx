...
| Read a context's stored content | `GET /context/inspect` |
| Delete context | `DELETE /context` |
| Inspect graph relations | `GET /context/relations` |
| Walk everything connected to one context | `GET /context/{id}/subgraph` |
| Update a context's metadata without re-ingesting | `PATCH /context/{id}/metadata` |

Ingest is text only: to add a file, extract its text and send it as a context. Content from connected apps arrives through [connectors](/essentials/v2/connectors) rather than this endpoint.
