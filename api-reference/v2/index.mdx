...
| [`/connector-catalog`](/api-reference/v2/endpoint/connector-catalog) | `GET` | `catalog` | List connector catalog | You need provider availability/maturity for a catalog UI before creating connectors. |
| [`/connector-discovery`](/api-reference/v2/endpoint/connector-discovery) | `POST` | `connectors.discover_preview` | Preview provider resources | You want to list resources from credentials before creating a connector. |
| [`/connectors/providers`](/api-reference/v2/endpoint/list-connector-providers) | `GET` | REST | List connector providers | You need the connectable provider list for the authenticated org. |
| [`/connectors`](/api-reference/v2/endpoint/create-connector) | `POST` | `connectors.create` | Create a connector | You are connecting an external account to sync into a database. |
| [`/connectors`](/api-reference/v2/endpoint/list-connectors) | `GET` | `connectors.list` | List connectors | You need every connector for the current organization. |
| [`/connectors/{id}`](/api-reference/v2/endpoint/get-connector) | `GET` | `connectors.get` | Get a connector | You need status for one connector. |
| [`/connectors/{id}`](/api-reference/v2/endpoint/delete-connector) | `DELETE` | `connectors.delete` | Delete a connector | You need to stop sync and remove a connector. |
| [`/connectors/{id}/discover`](/api-reference/v2/endpoint/discover-connector-resources) | `GET` | `connectors.discover` | Discover resources | You need resources available after connector creation. |
| [`/connectors/{id}/configure`](/api-reference/v2/endpoint/configure-connector) | `POST` | `connectors.configure` | Configure resources | You are activating resources and sync options. |
| [`/connectors/{id}/sync`](/api-reference/v2/endpoint/sync-connector) | `POST` | `connectors.sync` | Trigger sync | You want an on-demand sync instead of waiting for the scheduler. |
| [`/connectors/{id}/resources`](/api-reference/v2/endpoint/connector-resources) | `GET` | `connectors.list_resources` | List resources | You need configured resource sync state. |
