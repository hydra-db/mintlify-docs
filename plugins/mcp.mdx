...
- **Remote (hosted URL)** - point your client at `https://mcp.hydradb.com/mcp`. Nothing to install or update; you authenticate per request with headers.
- **Local (npx)** - your client spawns the `@hydradb/mcp` binary on your own machine over stdio.
...
<Info>
  **Local (npx) only:** the canonical tool names and `HYDRADB_*` variables need
