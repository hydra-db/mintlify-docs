...
A query that **failed** still issues a `request_id`, and feedback on it is especially valuable  -  send it too. The SDKs raise on failure rather than returning an envelope, so read the id off the exception body:

```python
from hydra_db.errors import ApiError

try:
    result = client.query(database="acme_corp", query=question)
    request_id = result.meta.request_id
except ApiError as exc:
    # exc.body is the parsed error envelope; meta.request_id is present on it
    request_id = (exc.body or {}).get("meta", {}).get("request_id")
    if request_id:
        client.feedback.submit(
            request_id=request_id,
            source="agent",
            feedback=f"query failed: {(exc.body or {}).get('error', {}).get('code')}",
        )
    raise
```
...
try {
  ...
  var requestId = result.meta.request_id;
} catch (error) {
...
Guard on the id being present  -  a transport-level failure (timeout, DNS) never reached the API and has no `request_id` to report against.
