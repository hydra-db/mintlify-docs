...
<Warning>
This plugin page documents the current MCP packaging, which still uses the v1 tenant / recall surface (`HYDRA_DB_TENANT_ID`, `/recall/*`). For HydraDB **API v2** integrations, use `database` / `collection` and `POST /query` — see the [Agent Integration Guide](/AGENTS) and [Quickstart](/get-started/v2/quickstart).
</Warning>
