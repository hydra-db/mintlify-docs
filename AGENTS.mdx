...
**Only send feedback for queries that returned.** If the query itself failed  -  `4xx`/`5xx`, or the SDK raised  -  handle the error and move on; do not submit feedback for it. Feedback is a judgement about retrieval quality, and a query that never produced results has no retrieval to judge. Fix the request instead: a `404` means the database name is wrong, a `429` means back off, a `400` means the body was malformed.
