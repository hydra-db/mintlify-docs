...
When in doubt, ask "would a different user benefit from seeing this?"  -  if yes, Knowledge; if no, Memory. For the full decision chain (store → infer → scope → query), see [Choosing Where Context Lives](/get-started/v2/choosing-context).
