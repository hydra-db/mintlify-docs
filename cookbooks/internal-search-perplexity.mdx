...
{/*
  ...
  The DX audit found 4 broken code blocks on this page: `google`/`googleapiclient` are never listed as install steps, `CONFLUENCE_BASE_URL` and `GITHUB_TOKEN` are read but never documented, and `full_recall()` is passed a `search_apps` argument it does not accept.

  They are deliberately left broken. v1 is being deprecated and the decision
  (PRO-1268) is to signal that rather than repair v1 content — the equivalent v2
  page has been fixed instead. Since 2026-06-01 an unpinned `pip install
  hydradb-sdk` serves the v2 SDK, which is the root cause of most of these.

  Per-block list: FAILING-SNIPPETS.md, attached to PRO-1259.
*/}
...
All connectors use the same endpoint: `POST /ingestion/upload_knowledge`. This endpoint uses **multipart form-data** - not a raw JSON request. For app connectors, send `app_knowledge` as a JSON-stringified form field. Each app-source item includes its own `id`, `tenant_id`, `sub_tenant_id`, `kind`, `provider`, `external_id`, `fields`, and optional `metadata`, `relations`, `attachments`, and `comments`.

> **Important**: Do not set `Content-Type: application/json`. Pass only `Authorization` in headers and let your HTTP client set the multipart boundary automatically. `app_knowledge` must be serialized with `JSON.stringify(...)` / `json.dumps(...)` before upload.
...
Ingest each Slack message as a typed `message` app source. Use `thread_id` for the root message timestamp and `parent_id` for replies so HydraDB can expand same-thread context during Recall.
...
async function ingestSlackChannel(
  ...
  while (true) {
    ...
    for (const msg of messages) {
      ...
      const thread = (msg.reply_count ?? 0) > 0
        ? (await slack.conversations.replies({ channel: channelId, ts: msg.ts! })).messages ?? [msg]
        : [msg];

      const appKnowledge = thread
        .filter((m) => m.text)
        .map((m) => ({
          id: `slack_${channelId}_${m.ts}`,
          tenant_id: TENANT_ID,
          sub_tenant_id: "slack",
          kind: "message",
          provider: "slack",
          ...
          fields: {
            ...
            author: m.user,
            thread_id: msg.ts,
            parent_id: m.ts !== msg.ts ? msg.ts : undefined,
            created_at: new Date(Number(m.ts) * 1000).toISOString(),
            url: `https://slack.com/archives/${channelId}/p${String(m.ts).replace(".", "")}`,
          },
          metadata: {
      ...
      for (let i = 0; i < appKnowledge.length; i += 20) {
        const result = await client.upload.knowledge({
          tenant_id: TENANT_ID,
          app_knowledge: JSON.stringify(appKnowledge.slice(i, i + 20)),
        });
        for (const item of ((result as any).results ?? [])) {
          if (item?.source_id) allIds.push(item.source_id);
        }
      }
    }
...
def ingest_slack_channel(channel_id: str, channel_name: str, days_back: int = 365):
    ...
    Each Slack message is a typed app source linked by thread_id/parent_id.
    """
    ...
    while True:
        ...
        for msg in messages:
            ...
            if msg.get("reply_count", 0) > 0:
                thread = slack.conversations_replies(channel=channel_id, ts=msg["ts"])["messages"]

            for m in thread:
                if not m.get("text"):
                    continue
                batch.append({
                    "id": f"slack_{channel_id}_{m['ts']}",
                    "tenant_id": TENANT_ID,
                    "sub_tenant_id": "slack",
                    "kind": "message",
                    "provider": "slack",
                    ...
                    "fields": {
                        ...
                        "author": m.get("user"),
                        "thread_id": msg["ts"],
                        "parent_id": msg["ts"] if m["ts"] != msg["ts"] else None,
                        "created_at": datetime.fromtimestamp(float(m["ts"]), tz=timezone.utc).isoformat(),
                        "url": f"https://slack.com/archives/{channel_id}/p{m['ts'].replace('.', '')}",
                    },
                    "metadata": {
                        "container_type": "channel",
...
def _upload_batch(batch: list, sub_tenant: str) -> list:
    """Upload a batch of typed app-source objects as multipart form-data."""
    result = client.upload.knowledge(
        tenant_id=TENANT_ID,
        app_knowledge=json.dumps(batch),
    )
    return [item.source_id for item in (result.results or []) if item.source_id]
```
</CodeGroup>

### 2.2 Gmail / Email Threads

Email threads contain decisions that never make it to Confluence. Ingest each Gmail message as a typed `email` app source with `thread_id` and `reply_to_id` when available.
...
async function ingestGmailThreads(
  ...
  for (const thread of threads) {
      ...
      if (body) {
        appKnowledge.push({
          ...
          fields: {
            ...
            thread_id: thread.id,
            reply_to_id: previousMessageId,
            created_at: msg.internalDate ? new Date(Number(msg.internalDate)).toISOString() : undefined,
          },
    ...
    if (!appKnowledge.length) continue;
...
def ingest_gmail_threads(credentials_path: str, query: str, max_threads: int = 200):
    ...
    Each email message is a typed app source; messages share thread_id.
    """
    ...
    for thread in threads:
            ...
            if body:
                app_sources.append({
                    ...
                    "fields": {
                        ...
                        "created_at": datetime.fromtimestamp(int(msg.get("internalDate", "0")) / 1000, tz=timezone.utc).isoformat() if msg.get("internalDate") else None,
                    },
                    "metadata": {"container_type": "mailbox", "source": "gmail"},
                })
                previous_id = msg["id"]
        ...
        for i in range(0, len(app_sources), 20):
            result = client.upload.knowledge(
                tenant_id=TENANT_ID,
                app_knowledge=json.dumps(app_sources[i:i + 20]),
            )
            all_ids.extend(item.source_id for item in (result.results or []) if item.source_id)
...
Confluence and Notion pages are structured knowledge-base app sources. Upload each page with `kind: "knowledge_base"`, a provider-scoped `external_id`, and space/database metadata so HydraDB can preserve hierarchy and citations.
    ...
    for (const page of pages) {
      ...
      const appKnowledge = [{
        ...
        fields: {
          ...
          url: `${CONFLUENCE_URL}/wiki/spaces/${spaceKey}/pages/${page.id}`,
        },
        metadata: {
          container_type: "space",
          container_id: spaceKey,
          container_name: spaceKey,
          source: "confluence",
        },
      }];
...
def ingest_confluence_space(space_key: str):
    ...
    while True:
        ...
        for page in pages:
            ...
            result = client.upload.knowledge(
                tenant_id=TENANT_ID,
                app_knowledge=json.dumps([app_source]),
            )
            all_ids.extend(item.source_id for item in (result.results or []) if item.source_id)
            time.sleep(0.1)
