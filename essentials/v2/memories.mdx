...
<Frame>
  <img src="/images/memories-diagram.png" alt="Memories ingestion flow: text or user_assistant_pairs into an infer decision, branching to infer: true (extraction) or infer: false (stored as written), into POST /context/ingest type: memory, polled via /context/status, then retrieved with POST /query type: memory." />
</Frame>

Try it yourself: add a memory below, then switch "Viewing as" to a different person and watch it disappear  -  while anything you add as Knowledge stays visible to everyone.
...
Memories don't always need to be stored word-for-word  -  HydraDB can infer the useful preference or trait from raw signal instead. See [Choosing infer: true vs false](#use-infer-true-when-the-input-is-raw-signal) below.
...
flowchart LR
    subgraph Memory Ingestion Pipeline [" "]
      direction LR
      A([Parse input]) --> B{infer?}
      B -- true --> C([Extract preference / trait / fact])
      B -- false --> D([Store as written])
      C --> E([Process & index])
      D --> E
      E --> F([Queryable])
    end
    ...
    style B fill:#FF571A,stroke:#334155,stroke-width:2px,color:#f8fafc
    ...
    style E fill:#1e293b,stroke:#334155,stroke-width:2px,color:#f8fafc
    style F fill:#0f172a,stroke:#334155,stroke-width:2px,color:#f8fafc
```

The most important choice is [`infer`](#choose-whether-hydradb-should-infer-the-memory), highlighted above. It decides whether HydraDB should extract the useful memory from raw signal, or store exactly what you send.
...
|---|---|---|---|
| <Field name="type" required /> | `"memory"` |  -  | Selects the Memories store. Use singular `memory`. |
| <Field name="database" required /> | `string` |  -  | Target database. The `database` field was formerly `tenant_id`. |
| <Field name="collection" recommended /> | `string` | default collection | User, workspace, or session scope for the memory. The `collection` field was formerly `sub_tenant_id`. |
| <Field name="upsert" /> | `boolean` | `true` | Replace existing memories with the same `id`. |
| <Field name="memories" required /> | `string (JSON array)` |  -  | JSON-stringified array of memory items. |

### `memories` item fields

| Field | Type | Description |
|---|---|---|
| <Field name="id" /> | `string` | Optional stable ID. Acts as the upsert key and relation target. |
| <Field name="title" /> | `string` | Short label for display in `/context/list` and query results. |
| <Field name="text" recommended /> | `string` | Raw text or Markdown content. Required unless `user_assistant_pairs` is provided. |
| <Field name="user_assistant_pairs" recommended /> | `array` | Conversation pairs in the shape `{ "user": "...", "assistant": "..." }`. Required unless `text` is provided. |
| <Field name="is_markdown" /> | `boolean` | Treat `text` as Markdown for chunking and indexing. |
| <Field name="infer" /> | `boolean` | When `true`, HydraDB extracts the underlying preference, trait, or fact. When `false`, it stores the input as written. |
| <Field name="custom_instructions" /> | `string` | Guides extraction when `infer: true`. Ignored when `infer: false`. |
| <Field name="user_name" /> | `string` | User name used during inference. |
| <Field name="expiry_time" /> | `integer` | TTL in seconds. The memory stops surfacing after expiry. |
| <Field name="metadata" /> | `string (JSON object)` | Database-schema fields for filtering and query. For memory ingestion, encode this nested object as a JSON string inside each `memories[]` item, for example `"metadata": "{\"department\":\"support\"}"`. |
| <Field name="additional_metadata" /> | `object` | Canonical free-form per-memory fields for display or bookkeeping. Send this as an object. |
| <Field name="relations" /> | `object` | Forcefully connect this memory to other memories with `{ "ids": [...] }`. |
...
Forceful relations only work reliably with stable, controlled `id`s on the memories you ingest  -  that's what makes linking them together possible.
...
With `infer: true`, you give HydraDB messy or indirect evidence instead  -  dialogue, logs, behavior, feedback, observations. HydraDB extracts the useful preference, trait, or fact from it.

For example, instead of writing custom logic to detect a dark-mode preference, you can send a stream of raw UI events: switched to dark mode, kept it for a month, briefly toggled back to light, then reverted. HydraDB can infer the useful memory from that pattern: `"user prefers dark mode"`.
...
Later, a [`POST /query`](/api-reference/v2/endpoint/query) call with `type: "memory"` and a query like `"what UI settings does the user prefer?"` returns the inferred preference  -  `"prefers dark mode"`  -  instead of the raw event log.
...
| Mistake | Symptom | Fix |
|---|---|---|
| Send `"memory": "..."` instead of `"memories": [...]` | Validation fails | Always an array: `"memories": [{ "text": "..." }]` |
| Omit `database` | Validation error | Required on every call |
| Omit `collection` for per-user data | Lands in the default collection, wrong scope | Use the user's ID as `collection` |
| Set `custom_instructions` with `infer: false` | Instructions are ignored | Only affects extraction when `infer: true` |
| Extract preferences yourself before sending raw logs | Duplicates work HydraDB already does | Send raw signal with `infer: true` |
| Send pre-extracted facts with `infer: true` | HydraDB may re-derive and reword the fact | Use `infer: false` for already-structured facts |
| Treat `user_name` as cosmetic | Wrong name can bias inference | Pass the actual user's name |
| Store shared docs as memories | Won't surface under `type: "knowledge"` | Use [Knowledge ingestion](/essentials/v2/knowledge) instead |
| Expect immediate searchability | Ingestion is asynchronous | Poll [status](/api-reference/v2/endpoint/source-status) until indexing completes |
| Forget `infer` on conversational input | Raw dialogue stored with no extraction | Set `infer: true` for conversations |
