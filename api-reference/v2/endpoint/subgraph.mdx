...
- **`id`** is the item to start from: any `id` returned by Query, List Context or Ingest. This query-string form takes any id, including one that contains `/`. `GET /context/{id}/subgraph` is the same read with the id as a URL-encoded path segment; it cannot carry an id containing a literal `/`.
