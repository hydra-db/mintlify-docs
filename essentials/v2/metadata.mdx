...
- **Plan scoping fields before first ingest.** Field definitions are immutable and undeclared scope keys are silently ignored at query time. Additive fields are supported, but new semantic/BM25 lanes are not backfilled automatically.
...
- **Keep keys stable.** Renaming means adding a new schema field, updating or re-ingesting affected sources, and changing every query. Existing field types and flags cannot be mutated.
- **Use the metadata edit endpoint for value-only changes.** Re-ingest with `upsert: true` and the same ID when the source content also changes.
...
- `collection` is required by the metadata-edit handler; send it explicitly even though the generated SDK request type currently marks it optional.
...
- `document_metadata` is an accepted deprecated alias for `additional_metadata`; use `additional_metadata` in new integrations.
- `enable_match`-only database metadata updates are MongoDB-only and take effect for filters/listing.
- If an edited database metadata field has `enable_dense_embedding` or `enable_sparse_embedding`, HydraDB synchronously syncs the relevant Milvus lane and reports `milvus_sync_required` / `milvus_synced` in the response.
...
| Existing schema field changes don't take effect | Existing definitions are immutable | Add new fields through the schema update endpoint; migrate when an existing field's type or flags must change. |
| Metadata values on a source don't update | Wrong source ID/scope or edit shape | Use the source metadata edit endpoint; re-ingest only when content also changes. |
...
| Metadata edit returns 400 | Missing collection, unknown key, wrong type, over-size value, too-deep nesting, or reserved key | Send the source's collection, check schema and validation limits, and use `database_metadata` and/or `additional_metadata`. |
...
**Schema-as-product.** Treat `database_metadata_schema` as a data contract and review additions like database migrations. Existing definitions remain immutable even though additive fields are supported.
