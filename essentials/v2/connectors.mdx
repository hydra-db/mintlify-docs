...
| `metadata` | Key-value pairs merged into database metadata on every synced object from this resource. Undeclared keys are accepted but only keys in `database_metadata_schema` are indexed for filtering. |
...
Database metadata is the **schema-declared** layer. Fields here are defined once per tenant via `database_metadata_schema` and are indexed for fast, exact-match filtering. This is what you use for stable high-cardinality fields you filter on often  -  `department`, `region`, `status`, `priority`. (`database_metadata_schema` was formerly `tenant_metadata_schema`; the old name still works as a deprecated alias.)

HydraDB always writes `provider` into database metadata for every synced object. You can extend this with your own fields by passing `metadata` on each resource in `POST /connectors/:id/configure`. User-supplied fields are merged first; `provider` always takes precedence.
...
- [Metadata](/essentials/v2/metadata)  -  database metadata vs document metadata in depth
