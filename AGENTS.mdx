...
The v2 OpenAPI does not enumerate app-source kinds. In search results, `app_kind` is a free-form string (for example `slack`), alongside `app_provider` and `app_external_id`; choose consistent kind values for your own integration and filter on them via metadata rather than expecting a server-enforced enum.

At minimum provide stable IDs, content, source type, metadata, and relations so app-aware search can group, cite, and expand results.
