...
<Info>
**Chunks carry no source details.** No title, url, collection, timestamps or attributes. `llm_prompt` prints the title, url, collection and last-updated date for the model; call [`GET /context/inspect`](/api-reference/v2/endpoint/fetch-content) with `id=<context_id>` to show them yourself.
</Info>
