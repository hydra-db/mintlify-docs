...
<Note>
  **Async Processing**: This endpoint returns HTTP **202 Accepted** immediately after validating and queuing your upload. Content is processed asynchronously — use [Verify Processing](/api-reference/endpoint/verify-processing) to check status.
</Note>

<Note>
  **Tenant must be provisioned first**: Before uploading knowledge, ensure the tenant's vectorstore collection has been provisioned. If the collection does not exist, this endpoint returns HTTP 404 with error code `TENANT_NOT_FOUND`. Use `GET /tenants/infra/status?tenant_id=<your_tenant_id>` to check provisioning progress after creating a tenant.
</Note>
