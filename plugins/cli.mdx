...
<Warning>
This CLI page documents the current CLI packaging, which still uses the v1 tenant / recall command surface (`hydradb tenant`, `hydradb recall`). For HydraDB **API v2** integrations, use `database` / `collection` and `POST /query` — see the [Agent Integration Guide](/AGENTS) and [Quickstart](/get-started/v2/quickstart).
</Warning>
