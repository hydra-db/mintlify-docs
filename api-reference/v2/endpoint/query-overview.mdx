...
| <Field name="follow_forceful_relations" /> | boolean | Pull items linked with `forceful_relations` at ingest into `relations[]`. On by default. |
...
| Key | Contents |
| --- | --- |
| `chunks[]` | Ranked matches: `chunk_id`, `context_id`, `score`, `content`, optional `enrichment` and `temporal`. No source details; call `GET /context/inspect` with the `context_id` for those. |
| `graph[]` | Paths through the context graph: `triplets[]` and a `path_summary`. |
| `relations[]` | Chunks linked with `forceful_relations` at ingest, each with the `via` that brought it in. |
| `llm_prompt` | A server-built string with citation labels (`[1]`, `[R1]`, `[P1]`), ready to inject into a model call. |
