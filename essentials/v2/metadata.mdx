...
<Note>
The `database_metadata` request field (on `PATCH /context/sources/{source_id}/metadata`) was formerly `tenant_metadata`, and `database_metadata_schema` (on `POST /databases`) was formerly `tenant_metadata_schema`; the old names still work as deprecated aliases. At `/query` and `/context/list`, the source metadata field is `metadata`, which still accepts `tenant_metadata` as a legacy alias.
</Note>
