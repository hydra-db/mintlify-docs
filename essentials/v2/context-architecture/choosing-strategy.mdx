...
<Tabs>
  ...
  <Tab title="Customer Support">
    **Shape:** Shared help center + per-customer history.
    ...
    | Query | Dual query: Knowledge (shared collection) + Memory (`customer_id`); or `type: "all"` with `collections` covering both |
  ...
  <Tab title="Internal Knowledge Assistant">
    **Shape:** Company "second brain" across Slack, Notion, Drive — Glean-like.
    ...
    | Query | Dual query or `type: "all"` with `collections` = [shared Knowledge partition, employee]; always ACL `metadata_filters` on Knowledge |
...
Use two separate [`POST /query`](/api-reference/v2/endpoint/query) calls when Knowledge and Memories live in **different collections** (the usual support / coding pattern: shared Knowledge in default/team, Memories under `user_id`) or when prompt formatting differs. A single `type: "all"` call only works if every store you need is reachable from the `collection` / `collections` you pass — see [Multi-tenant](/essentials/v2/multi-tenant). Details: [Query](/essentials/v2/query) · [API Results](/essentials/v2/api-results).
