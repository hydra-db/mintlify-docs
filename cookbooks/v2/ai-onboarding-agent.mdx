...
| `422 field required: memories` on `context.ingest(type="memory")` | Wrong field structure | Pass `memories=json.dumps([{"text": "...", "infer": True}])`. `memories` is a multipart form field, so it takes a JSON **string**, not a Python list. |
