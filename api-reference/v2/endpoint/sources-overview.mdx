...
- **Items**: everything you ingest is a piece of context, a `text` or a `conversation`. One database holds all of them; collections partition them per user, team or project. See [Ingest context](/essentials/v2/ingest).
- **IDs**: each item has a `context_id`, yours or generated. The ingest response reports it as `results[].id`. Use it for polling status, inspecting content, deleting, and inspecting relations.
...
- **Declared relations**: any item can name the items it relates to with `forceful_relations`, so they surface together in `forceful_relations[]` on a `thinking` query.
...
Declared attributes, sent on the same item, decide which items an `attributes` filter lets a query return.
