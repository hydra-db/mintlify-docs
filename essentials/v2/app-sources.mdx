...
<Accordion title="Slack message app-source payload (Python / TypeScript / cURL)">
<CodeGroup>
```python Python SDK 
import json
...
Each section shows one or more correct `app_knowledge` items you can adapt. Click to expand the JSON.
...
<Accordion title="Slack thread root + reply">
  ```json expand
  {
    "id": "slack_C01_1716213600_000100",
    "tenant_id": "acme_corp",
...
<Accordion title="Jira ticket payload">
  ```json
  {
    "id": "jira_AUTH-123",
    "tenant_id": "acme_corp",
...
<Accordion title="Linear comment payload">
  ```json
  {
...
<Accordion title="Notion page payload">
  ```json
  {
    "id": "notion_page_abc",
    "tenant_id": "acme_corp",
...
<Accordion title="Gmail email payload">
  ```json
  {
...
<Accordion title="Salesforce opportunity payload">
  ```json
  {
