...
| <Field name="collection" type="string" /> | Logical partition inside the database. Formerly `sub_tenant_id`; the `sub_tenant_id` alias is still accepted (deprecated). Maximum 128 characters. (default=`""`  -  default collection) |
...
<Accordion title="Knowledge documents - upload PDFs, DOCX, CSV, markdown, or text">
  ...
  | Field | Description |
  | --- | --- |
  | <Field name="id" type="string" /> | Optional context ID. If set, becomes the `id` for this document (use your app's document ID for parity). Maximum 100 characters. Must not contain a comma (`,`) - it is reserved as the id separator on `/context/status?ids=`. |
...
<Accordion title="App sources - index text from your workspace or personal apps">
  ...
  | Field | Description |
  | --- | --- |
  | <Field name="id" type="string" required /> | Context ID. Treated as the upsert key. Send an empty string to have one generated upstream. Maximum 100 characters. Must not contain a comma (`,`) - it is reserved as the id separator on `/context/status?ids=`. |
  ...
  | <Field name="collection" type="string" required /> | Logical partition inside the database. Must match the form-level `collection`. Formerly `sub_tenant_id`; the `sub_tenant_id` alias is still accepted (deprecated). Maximum 128 characters. |
...
<Accordion title="3. User memories - store notes or infer user preferences">
  ...
  | Field | Description |
  | --- | --- |
  | <Field name="id" type="string" /> | Optional unique ID. Acts as the upsert key. Maximum 100 characters. Must not contain a comma (`,`) - it is reserved as the id separator on `/context/status?ids=`. (default=auto-generated) |
