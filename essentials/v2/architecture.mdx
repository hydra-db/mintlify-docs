...
|---|---|---|
| **Control** | API authentication, tenant lifecycle, provisioning, and status | [`/tenants`](/api-reference/v2/endpoint/databases-overview) family |
...
1. **Create a tenant** with [`POST /tenants`](/api-reference/v2/endpoint/create-database)  -  your isolated workspace, optionally with a [metadata schema](/essentials/v2/metadata) declared up front.
2. **Wait for provisioning** by polling [`GET /tenants/status`](/api-reference/v2/endpoint/database-status) until `vectorstore_status.knowledge`, `vectorstore_status.memories`, and `graph_status` are all `true`.
...
A `tenant_id` is the top-level isolation boundary  -  no tenant can read another tenant's data. Within a tenant, sub-tenant IDs carve out logical partitions: users, teams, workspaces, or projects. Writes use singular `sub_tenant_id`; query should use `sub_tenant_ids` and can fan out across multiple sub-tenants. Omitting query scope resolves to the tenant's default sub-tenant, which is auto-created with the tenant itself.
...
1. **Authenticate and scope.** Validate `tenant_id`, resolve the tenant, and apply the requested `sub_tenant_ids` fanout scope (or legacy `sub_tenant_id`).
...
| `sub_tenant_id` | Ingest + legacy query | Narrows writes to a user, team, or workspace inside the tenant. For new query calls, prefer `sub_tenant_ids`. |
| `sub_tenant_ids` | Query | Preferred query-time sub-tenant selector. Use a single-item list, a multi-scope list with equal weights, or a weighted object for fanout ranking. |
...
2. **Scope second.** Is the `tenant_id` correct? Did you write under one `sub_tenant_id` and query without including it in `sub_tenant_ids`? See [Multi-Tenant](/essentials/v2/multi-tenant).
