...
<AccordionGroup>
  ...
  <Accordion title="Better solution">
    Ask: *Would another user benefit from this exact content?* If yes → [Knowledge](/essentials/v2/knowledge). If no → [Memories](/essentials/v2/memories). Retrieve both with dual `POST /query` calls, or one `type: "all"` whose `collections` cover **every** partition you wrote to — do not collapse the stores into one bucket.
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  ...
  <Accordion title="Better solution">
    Per feature, decide `type`, `collection`/`collections`, and `metadata_filters` explicitly. When shared Knowledge lives in the default/team collection and Memories live under a user collection, use **two queries** (or one `type: "all"` with `collections` listing **both** partitions) — a lone user `collection` on `type: "all"` will miss shared Knowledge. Document-only tools → `type: "knowledge"`. See [Query](/essentials/v2/query) · [Multi-tenant](/essentials/v2/multi-tenant).
  </Accordion>
</AccordionGroup>
