...
|---|---|---|
| **Control** | API authentication, database lifecycle, provisioning, and status | [`/databases`](/api-reference/v2/endpoint/databases-overview) family |
...
1. **Create a database** with [`POST /databases`](/api-reference/v2/endpoint/create-database)  -  your isolated workspace, optionally with a [metadata schema](/essentials/v2/metadata) declared up front.
2. **Wait for provisioning** by polling [`GET /databases/status`](/api-reference/v2/endpoint/database-status) until `vectorstore_status.knowledge`, `vectorstore_status.memories`, and `graph_status` are all `true`.
