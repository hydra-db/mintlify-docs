...
<Note>
**One name, three meanings.** You will meet `document_metadata` in three places; only the first is something to use in new code:

1. **Ingest parameter** — the per-file array on multipart upload (`document_metadata=[…]`, one entry per file). Current, documented on [Knowledge](/essentials/v2/knowledge#file-metadata-document_metadata-array-items).
2. **Filter alias** — a legacy alias for the `additional_metadata` namespace in `metadata_filters` / listing filters. Accepted, deprecated — write `additional_metadata` instead.
3. Anything else you see calling source metadata "document metadata" is informal prose for the two above.

And remember: top-level `metadata_filters` keys match the **schema-defined `metadata`** layer; `additional_metadata` values are matched only when nested under the `additional_metadata` key.
</Note>
...
1. **Document-store source-id prefilter.** Safe scalar tenant/additional metadata filters are resolved to matching `source_id`s in the document store.
...
<Warning>
  Adding fields **to a database that already has data** updates the stored schema and filter indexes, but existing vector-store collections are not altered/backfilled for the newly added dense/sparse metadata lanes yet. Fields declared at database creation are unaffected. If you need a new metadata field to participate in semantic/BM25 metadata search for existing data, create a new database/schema and re-ingest, or confirm the current platform migration path with support.
</Warning>
...
- `enable_match`-only tenant metadata updates touch only the document store and take effect for filters/listing.
...
`source_fields` filters on **system-populated source attributes** rather than user metadata — here `type`, the source's provider/kind (`slack`, `notion`, `file_upload`, …), which connectors and the ingestion pipeline set automatically.
...
| New schema field does not participate in semantic metadata search | Additive schema update does not alter/backfill existing vector-store fields yet | Create the desired schema before ingesting, or migrate/re-ingest into a database with the final schema. |
