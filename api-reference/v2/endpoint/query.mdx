...
- **Common mistakes.** Check [Ingestion Status](/api-reference/v2/endpoint/source-status) for recently ingested documents before querying. If you omit `collection`, HydraDB defaults to the resolved primary tenant ID (the default collection); use [List Collections](/api-reference/v2/endpoint/list-sub-tenants) to discover available IDs.
</Warning>
