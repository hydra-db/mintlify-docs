...
### Read the declared schema

You can read back the fields a database has declared with [`GET /databases/{database}/metadata-schema`](/api-reference/v2/endpoint/get-metadata-schema). The response uses the same field shape the create and update endpoints accept, so you can reconcile the schema your application expects against what the database already has.
...
- [Get Metadata Schema](/api-reference/v2/endpoint/get-metadata-schema)  -  read the declared fields
