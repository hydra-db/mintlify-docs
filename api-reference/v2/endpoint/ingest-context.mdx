...
| <Field name="sub_tenant_id" type="string" /> | Logical partition inside the tenant. If omitted, defaults to the resolved primary tenant ID. |
...
- **Sub-tenant defaulting.** Omitting `sub_tenant_id` defaults to the resolved primary tenant ID. All endpoints (ingestion, search, delete) use this same rule. List available sub-tenants with [List Sub-Tenants](/api-reference/v2/endpoint/list-sub-tenants).
