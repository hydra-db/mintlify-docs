...
flowchart TB
  ...
  subgraph H["HydraDB database: product_acme"]
    K[Knowledge: ADRs · runbooks · service docs]
  ...
  Dev --> Agent
  Agent -->|POST /query type=all graph_context=true| H
  H --> Agent --> LLM --> Agent --> Dev
  Agent -->|ingest memory infer=true| M
...
| Collections | `dev_id` for Memories; shared docs in team/default |
| Knowledge | ADRs, runbooks; upsert on merge |
| Memories | Style prefs + decisions; debug notes with `expiry_time` |
| Query | `type: "all"`, `mode: "thinking"` for hard bugs |
| Graph | On for ownership / dependency questions |
...
flowchart LR
  ...
  Q[POST /query type=all query_apps=true]
...
| Knowledge | Accounts, opportunities, notes as app sources |
| Memories | Rep preferences, "don't pitch X", last strategy |
| Query | `query_apps: true` for CRM object traversal behavior |
...
flowchart TB
  ...
  Q[type=all + filters on segment/region]
  LLM[LLM]
...
| Layer | Choice |
| --- | --- |
| Knowledge | Battle cards (upsert on enablement publish) + account corpus |
| Memories | Rep collection |
| Metadata | `segment`, `region`, `product_line`, `stage` |
| Query | Filter by account segment; personalize with Memories |
...
sequenceDiagram
  ...
  U->>A: task
  A->>A: authorize → database + collection
  A->>H: POST /query (type, filters, graph)
  H-->>A: chunks + sources + optional graph
  A->>L: formatted context
  L-->>A: completion
  A-->>U: answer + citations
  opt learning
    A->>H: POST /context/ingest type=memory
  end
```
