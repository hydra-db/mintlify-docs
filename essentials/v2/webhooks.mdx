...
<Info>
Successful webhook management responses use the same standard v2 `{ success, data, error, meta }` envelope as `/databases`, `/context/*`, and `/query` - the fields shown below (`registered`, `url`, `event_types`, etc.) live under `data`. Validation errors (`4xx`) instead return a `{ "detail": { ... } }` body, so read the error from `detail`, not `error`.
</Info>
...
curl -X POST 'https://api.hydradb.com/webhooks/indexing' \
  ...
  -d '{
    ...
    "signing_secret": "replace-with-at-least-32-characters"
  }'
```
...
`signing_secret` is optional but must be at least 32 characters if provided. If you include it, your receiver should verify `X-HydraDB-Signature`.
...
curl -X POST 'https://api.hydradb.com/webhooks/indexing' \
  ...
  -d '{
    ...
    "event_types": ["indexing.status_changed"],
    "signing_secret": "new-secret-at-least-32-characters"
  }'
```
...
| `tenant_id` | Deprecated alias for `database`, carrying the same value. |
...
<Note>
`tenant_id` and `sub_tenant_id` are sent alongside `database` and `collection`
for backward compatibility. Read `database` and `collection` in new integrations.
</Note>
