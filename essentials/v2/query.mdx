...
Surface it to your agent verbatim, and let the model cite the labels. When you need structured output instead, read `chunks[].content`, `chunks[].enrichment`, `chunks[].enrichment_kind`, `graph[].path_summary` and `forceful_relations[]`. See [How to Use API Results](/essentials/v2/api-results).
