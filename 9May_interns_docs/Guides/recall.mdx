...
For request/response schemas, see the API reference: [Full Recall](/api-reference/endpoint/full-recall), [Memory Recall](/api-reference/endpoint/recall-preferences), [Boolean Recall](/api-reference/endpoint/keyword).
...
HydraDB stores two kinds of context, and they live in separate stores:

- **Knowledge** — ingested documents, files, and app sources. Shared reference material.
...
- You want HydraDB to write the answer for you → use `/recall/qna`.

---

## 3. Types of Recall

HydraDB exposes four endpoints under `/recall/*`. Pick one based on what you need back.
...
| `POST /recall/qna` | Knowledge or memories (`search_mode`) | `QnASearchResponse` | You want a grounded LLM-generated answer |

`full_recall` and `recall_preferences` share the same request and response shape (`RecallSearchRequest` → `RetrievalResult`). `boolean_recall` accepts a smaller set of parameters and runs deterministic lexical matching — no graph traversal, no metadata filters. `qna` runs a recall pipeline and then composes a grounded answer in a single call.
...
The semantic recall endpoints (`full_recall`, `recall_preferences`, `qna`) accept a `mode`:
...
If you'd rather skip the merge step entirely, swap both calls for a single `/recall/qna` call and return its `answer` directly.
...
- **Pass `additional_context`** with known session state (page, feature, role). It can sharpen retrieval without requiring additional retrieval calls.
...
| `metadata_filters` does not narrow results | Filter key is undeclared or not match-enabled | Declare filterable keys in `tenant_metadata_schema` with matching enabled before ingestion. |
...
**Skip the merge with `qna`.** When you only need an answer rendered to the user (no agent loop, no custom prompt engineering), `/recall/qna` runs the same pipeline and returns a grounded answer in one round-trip.
