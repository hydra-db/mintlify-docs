...
<Accordion title="Knowledge documents - upload PDFs, DOCX, CSV, markdown, or text">
  ...
  In particular, an uploaded document's **title cannot be set**. It is not accepted at ingest, and [`PATCH /context/{id}/metadata`](/api-reference/v2/endpoint/update-source-metadata) only merges `additional_metadata` and `database_metadata`. Use `additional_metadata` for your own display fields.
  </Note>
</Accordion>
