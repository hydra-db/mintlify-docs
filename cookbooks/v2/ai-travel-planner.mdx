...
class TravelAssistant {
  ...
  async processResponse(response) {
    ...
    // Response fields stay snake_case on the wire, even in TypeScript.
    return {
      chunks: response.data?.chunks,
      graphRelations: response.data?.graph_context?.chunk_relations,
      queryPaths: response.data?.graph_context?.query_paths
    };
  }
}
```
