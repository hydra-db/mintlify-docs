...
HydraDB never talks to the LLM, and the LLM never talks to HydraDB - your application sits between both, every time. It asks HydraDB one question - *"what does this user need to know, right now, to answer this?"* - and HydraDB answers using everything it has stored: what this specific user has told you, what your organization knows, and how those things connect. Your application then hands that answer to the LLM as context, gets a response back, and relays it to the user. The LLM never touches storage directly, and your application never touches embeddings, chunking, or ranking directly.

Everything past this point in HydraDB is in service of making that one arrow - `App → HydraDB` - return the right thing. The `HydraDB → App` and `App → LLM` arrows are two separate calls your code makes; HydraDB never makes the second one for you.
...
flowchart TD
  ...
  Retrieval -->|"retrieved context"| Dev
  Dev --> LLM([LLM])
  LLM -->|"answer"| Dev
...
Same rule as §2, drawn at the pipeline level: HydraDB returns context to your application, and your application - never HydraDB - makes the separate call to the LLM.
...
with open("refund-policy.pdf", "rb") as f:
    doc = client.context.ingest(
        type="knowledge",
        database=database,
        documents=[("refund-policy.pdf", f, "application/pdf")],
    )

doc_id = doc.data.results[0].id
```
...
Ingestion is asynchronous (§6) - poll the same [`GET /context/status`](/api-reference/v2/endpoint/source-status) check used throughout these docs until the document is `completed`, before querying it.

```python
while client.context.status(database=database, ids=[doc_id]).data.statuses[0].indexing_status != "completed":
    time.sleep(2)
```
...
result = client.query(
    ...
    mode="fast",  # start here; upgrade to "thinking" only where retrieval quality demands it (see §9)
)
```
