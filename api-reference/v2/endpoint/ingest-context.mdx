...
| <Field name="collection" type="string" /> | Logical partition inside the database. (default=`""`  -  default collection) |
...
<Accordion title="App sources - index text from your workspace or personal apps">
  ...
  | <Field name="database" type="string" required /> | Target database. Must match the form-level `database`. |
  | <Field name="collection" type="string" required /> | Logical partition inside the database. Must match the form-level `collection`. |
