...
`POST /context/ingest` writes either Knowledge or Memories. The `type` form field chooses the store:

| If you are ingesting... | Set `type` to | Send content in | Use `infer`? |
| --- | --- | --- | --- |
| Shared documents, files, or app-source text | `knowledge` | `documents`, `document_metadata`, and/or `app_knowledge` | No. Knowledge is parsed, chunked, and indexed as sent. |
| User-specific preferences, history, observations, or session context | `memory` | `memories` | Yes, on each memory item. Set `infer: true` for raw signal and `infer: false` for already-written facts. |

`infer` is not a third ingestion mode and not a separate endpoint. It only applies inside items in the `memories` array when `type=memory`.
...
text_memory_result = client.context.ingest(
    ...
    memories=json.dumps([
        {
            # This is already the memory, so store it as written.
...
# OR: already-written memory text.
curl -X POST 'https://api.hydradb.com/context/ingest' \
  -H "Authorization: Bearer <your_api_key>" \
  -H "API-Version: 2" \
  -F "type=memory" \
...
| Name | Description |
| --- | --- |
| <Field name="type" type="&#x22;knowledge&#x22; or &#x22;memory&#x22;" /> | Selects which store receives this request. Use `"knowledge"` for shared documents/app sources and singular `"memory"` for user-specific memories. (default=`"knowledge"`) |
...
<Accordion title="3. User memories - store notes or infer user preferences">
  ...
  | <Field name="infer" type="boolean" /> | Memory items only. When `true`, HydraDB extracts the underlying preference, trait, or fact from raw signal. When `false`, HydraDB stores the item as written. Always set this explicitly. (default=`false`) |
