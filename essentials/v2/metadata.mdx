...
If a field will be scoped on **every** query, it belongs in `metadata` and the [database schema](/api-reference/v2/endpoint/create-database). If it's ad-hoc or unique to one document, use `additional_metadata` instead.
...
- [Create Database  -  API Reference](/api-reference/v2/endpoint/create-database)  -  full `database_metadata_schema` reference
