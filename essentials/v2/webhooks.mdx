...
`collection`. They are the pre-rename spellings, kept so existing receivers keep
working - the signature is computed over the delivered bytes, so they cannot be
dropped without breaking verification for those receivers. Read `database` and
`collection` in new integrations, and see the [glossary](/essentials/v2/glossary)
