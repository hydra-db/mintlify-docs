...
- `collection` defaults to the default collection when omitted.
...
| Metadata edit returns 400 | Unknown key, wrong type, over-size value, too-deep nesting, or reserved key | Check schema and validation limits; send `database_metadata` and/or `additional_metadata`. |
