...
<Field name="feedback" type="string" required>
...
| Status | When |
| --- | --- |
| `400` | `request_id` missing or not a UUID; `feedback` blank or too long; unknown `rating`/`source`; `collection` without `database` |
