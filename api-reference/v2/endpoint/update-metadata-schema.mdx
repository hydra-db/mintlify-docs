...
| <Field name="data_type" type="string" /> | `VARCHAR`, `BOOL`, `INT8` to `INT64`, `FLOAT`, `DOUBLE`, `JSON`, or aliases like `string`. Default `VARCHAR`. `ARRAY` is a `400`; comma-join multiple values. |
...
| --- | --- |
| `400` | Invalid body or field; empty `add_fields`; `ARRAY` type; over 32 fields; or embedding flags on a new field. |
| `404` | Database not found. |
| `409` | A field with this name exists with a different definition (or repeats in `add_fields`), or the database changed mid-request. |
