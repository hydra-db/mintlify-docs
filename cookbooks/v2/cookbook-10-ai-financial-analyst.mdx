...
> **SDK required for all API calls.** Install: `pip install "hydradb-sdk>=2,<3"`. Import as `from hydra_db import HydraDB` (the import name differs from the package name).
...
| Raw `requests` for PDF upload | 422 error on `/context/ingest` | Use the SDK: `pip install "hydradb-sdk>=2,<3"` |
