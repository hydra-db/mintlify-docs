...
If an edited `tenant_metadata` field has `enable_dense_embedding` or `enable_sparse_embedding`, HydraDB refreshes the relevant vector-store metadata search lane as part of the edit. For `enable_match`-only fields, the update stays on the MongoDB filter path.
...
After Path B, confirm filters immediately with [`POST /query`](/api-reference/v2/endpoint/query).
