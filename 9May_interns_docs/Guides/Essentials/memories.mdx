...
The trade: `infer: false` is faster and deterministic; `infer: true` is more useful when your input is raw and unstructured.
...
A realistic example. The user has been toggling dark mode in your app:

```python
# Raw behavior log — let HydraDB do the extraction
...
The same input with `infer: false` would index the raw text verbatim — useful only if you actually want to retrieve the event log.
...
- [Knowledge ingestion](/api-reference/endpoint/upload-knowledge) — for shared, tenant-wide content
