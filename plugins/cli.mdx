...
<Steps>
  ...
  <Step title="Recall the indexed knowledge">
    ...
    `--mode fast` is the default and uses a single query for lower latency.
    `--mode thinking` expands the query and reranks the combined results for higher
    quality at higher latency. Both `recall full` and `recall preferences` accept these
    modes; `recall keyword` does not. See [Recall modes](/essentials/recall#4-how-recall-works).
  </Step>
</Steps>
...
if ! upload_json="$(
  hydradb -o json knowledge upload ./runbook.md --upsert
)"; then
  printf '%s\n' "Knowledge upload failed" >&2
  printf '%s\n' "$upload_json" >&2
  exit 1
fi
...
' >/dev/null || {
  printf '%s\n' "Upload response validation failed" >&2
  printf '%s\n' "$upload_json" >&2
  exit 1
}
...
if ! source_id="$(
  ...
  printf '%s\n' "Upload response did not contain a source ID" >&2
  printf '%s\n' "$upload_json" >&2
  exit 1
fi
...
for attempt in $(seq 1 60); do
  if ! status_json="$(
    hydradb -o json knowledge verify "$source_id"
  )"; then
    printf '%s\n' "Status check failed for $source_id" >&2
    printf '%s\n' "$status_json" >&2
    exit 1
  fi
  ...
  if ! status="$(
    printf '%s\n' "$status_json" |
      jq -er '(.statuses // .results // [])[0]
        | .indexing_status // .status // "unknown"'
  )"; then
    printf '%s\n' "Could not parse status for $source_id" >&2
    printf '%s\n' "$status_json" >&2
    exit 1
  fi
  ...
  case "$status" in
    ...
    failed|errored|error)
      printf '%s\n' "Indexing failed for $source_id" >&2
      printf '%s\n' "$status_json" >&2
      exit 1
      ;;
    *)
...
' >/dev/null || {
  printf '%s\n' "Recall response validation failed" >&2
  printf '%s\n' "$recall_json" >&2
  exit 1
}
```
...
| Command | Purpose |
|---|---|
| `hydradb memories add --text TEXT [--user-name NAME]` | Add a memory; optional `--user-name` identifies the user for personalization; also accepts piped stdin |
...
| Command | Purpose |
|---|---|
| `hydradb recall full QUERY [--mode fast\|thinking]` | Semantic search over knowledge; `fast` is lower latency, while `thinking` reranks multiple queries |
| `hydradb recall preferences QUERY [--mode fast\|thinking]` | Semantic search over memories with the same mode choices |
