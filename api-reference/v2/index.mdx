...
**Response envelope:** Successful v2 endpoint-handler responses use a consistent envelope. Endpoint pages show the full envelope; the resource-specific payload, including webhook management responses, lives under `data`. Authentication or gateway middleware may reject a request earlier with a `detail` error shape, so branch on HTTP status before parsing the success envelope.
