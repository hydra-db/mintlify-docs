...
| Name | Description |
| --- | --- |
| <Field name="ids" type="string[]" required /> | Context ids, including connector-synced ones. Repeat the param (`ids=a&ids=b`) or comma-join (`ids=a,b`). |
...
| <Field name="collection" type="string or null" /> | Collection scope; the default collection when omitted. Alias `sub_tenant_id` (deprecated). (default=`null`) |
...
| `message` | string | Result of the lookup, such as "ID not found". For the ingestion outcome, read `indexing_status` and `error_code`. |
...
| *ingestion-pipeline codes* | A processing failure as an `E####` code, such as `E1001` parse failed. | Act on the code; see [Ingestion error codes](/api-reference/v2/error-responses#ingestion-error-codes). |
