...
# Store a document with an ID, a title, a date, a filterable attribute and a category
hydradb ingest --text "Refund policy: 30-day window." --context-id policy-1 \
  --title "Refund policy" --happened-at 2026-07-29 \
  --attributes '{"team": "support"}' --category business_knowledge --database my-db
...
| `--category` | `context_category` label: `auto` (default, no label), `user_preference`, `business_knowledge` or `decision_trace` |
