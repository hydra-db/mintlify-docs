...
| <Field name="forceful_relations" type="object" /> | Relations you declare to other items: `{ "ids": ["<context_id>", ...], "properties": {} }`. Followed on `/query` with `follow_forceful_relations` and returned in `relations[]`. `relations` is accepted as an alias for the field; `context_ids` or `source_ids` for the `ids` key. |
