...
| `type: "all"` or dual query covering **both** shared Knowledge and user Memory collections | Query Knowledge-only and hope the LLM "knows" the user; or `type: "all"` with only the user collection when Knowledge is shared |
...
- Shared Knowledge stays in a well-known collection (often default) and is queried with that scope — either a dedicated `type: "knowledge"` call or `type: "all"` whose `collections` also include the user partition
