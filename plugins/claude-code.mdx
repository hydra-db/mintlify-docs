...
<Note>
  This plugin's own environment variables and config keys are named `tenantId`/`subTenantId` (and `HYDRADB_TENANT_ID`/`HYDRADB_SUB_TENANT_ID`). These map 1:1 to the current API's `database` and `collection` fields (see [Multi-tenant](/essentials/v2/multi-tenant)) - set them to the same values you'd use for `database`/`collection` elsewhere.
</Note>
...
<Steps>
  ...
  <Step title="Get credentials">
    ...
    - Create or copy your database ID from the [HydraDB dashboard](https://app.hydradb.com/tenants)
  </Step>
