...
| `tenant_id` | An identifier for the database — not the name you ingested into. Always present. |
...
<Note>
Deliveries also carry `database` and `collection` alongside the fields above.
`database` is **the name you ingested into** and is what you should route on;
`tenant_id` is an identifier for that database and is not the name you sent.
`database` can be empty for items that were already being processed when it was
introduced; `tenant_id` is always set, so fall back to it if you need a guaranteed
non-empty scope. See the [v2 webhooks payload](/essentials/v2/webhooks#payload) for
the full field list.
</Note>
