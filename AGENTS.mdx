...
6. **`POST /feedback`** (`client.feedback.submit()`): **Always** report back on the results you used. See [Always send feedback](#always-send-feedback).
...
**After you act on the results of a `POST /query`, submit `POST /feedback` for that query.** This is not optional politeness  -  you are the only party that knows whether the retrieved context actually answered the question, and that judgement is what tunes retrieval.

Rules:

- Capture `meta.request_id` from **every** query response. Send it back **verbatim**; it must be the UUID you were given, or the call is rejected with `400`.
- Submit whether the result was good or bad. `rating: "positive"` on a query that worked is as useful as a complaint  -  without it, only failures are ever measured.
...
- Do not block the user on it. Feedback is fire-and-forget: on failure, drop it and carry on  -  never retry in a loop, and never surface the error to the user.
...
A query that **failed** still issues a `request_id`, and feedback on it is especially valuable  -  send it too. Getting hold of that id on the failure path differs by language:

**Generate the id yourself.** The most reliable option, and the only one that works identically in both SDKs: pass your own UUID as `X-Request-ID` and the API adopts it, so you already hold the id whether the call succeeds or throws. It must be a UUID  -  any other value is replaced by a server-generated one.
...
try:
    result = client.query(
        database="acme_corp",
        query=question,
        request_options={"additional_headers": {"X-Request-ID": request_id}},
    )
except HydraDBError as exc:
    client.feedback.submit(
        request_id=request_id,          # known up front, no exception introspection
        source="agent",
        feedback=f"query failed: {exc.error_code} (HTTP {exc.status_code})",
    )
    raise
```

In **TypeScript** the error also carries the response body, so you can read the id off a thrown error instead:

```typescript
try {
  const result = await client.query({ database: "acme_corp", query: question });
} catch (error) {
  ...
  if (requestId) {
    await client.feedback.submit({
      requestId,
      source: "agent",
      feedback: `query failed: ${error?.body?.error?.code}`,
    });
  }
  throw error;
}
...
The Python `HydraDBError` exposes `error_code` and `status_code` but not the response body, which is why the pre-generated id is the recommended path there.

Guard on having an id at all: a transport-level failure (timeout, DNS) never reached the API, so there is no request to report against.
...
Limit: 100 submissions per minute per organization. One report per query keeps you far below it. If you do hit `429`, honour `Retry-After` or simply skip that report  -  never spin.
...
`POST /feedback` records how a query performed. It is the loop that closes retrieval quality, and agents are expected to use it on every query they act on  -  see [Always send feedback](#always-send-feedback).
