...
For AI coding agents and IDE assistants, use the [HydraDB Agent Integration Guide](/AGENTS) and the [v2 OpenAPI spec](/api-reference/v2/openapi.json). When wiring retrieval, agents select the store with `type: "knowledge" | "memory" | "all"` on [`/query`](/essentials/v2/query).