...
<Note>
  **Looking for "Infer"?** It isn't a separate store or endpoint - it's the `infer` field you set on a Memory item at ingestion (see [Choose whether HydraDB should infer the memory](#choose-whether-hydradb-should-infer-the-memory) below). If you're unsure whether your content belongs here at all, see [Knowledge vs. Memories vs. Infer](/get-started/v2/core-concepts#knowledge-vs-memories-vs-infer).
</Note>
...
Once content belongs in Memories, the next choice is whether HydraDB should [infer the memory](#choose-whether-hydradb-should-infer-the-memory) from raw signal or store it exactly as written.
...
| <Field name="infer" type="boolean" /> | Defaults to `false`. When `true`, HydraDB extracts the underlying preference, trait, or fact. When `false`, it stores the input as written. Always set it explicitly. |
...
```json
{
  ...
  "data": {
    ...
    "results": [
      { "id": "mem_abc123", "status": "queued", "infer": false }
    ],
    "success_count": 1,
    "failed_count": 0
  },
