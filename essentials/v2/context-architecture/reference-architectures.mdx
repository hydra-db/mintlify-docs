---
title: "Reference Architectures"
description: "Six production HydraDB context architectures with Mermaid diagrams — Slack RAG, support, coding agents, enterprise SaaS, research, and internal knowledge."
---

These are **complete system shapes**, not endpoint tutorials. Each architecture shows how sources flow through ingestion, stores, graph, query, and the LLM — using only real HydraDB surfaces: [`POST /context/ingest`](/api-reference/v2/endpoint/ingest-context), [`POST /query`](/api-reference/v2/endpoint/query), databases/collections, metadata, and graph context.

---

## 1. Slack → Knowledge → Graph → Query → LLM

Canonical team-knowledge loop for channels that hold decisions.

```mermaid
flowchart TB
  Slack["Slack channels / threads"]
  AuthZ["Your app: resolve caller ACLs<br/>allowed channels / workspaces"]
  Conn["Connector sync or App Source ingest"]
  Ingest["POST /context/ingest<br/>type=knowledge<br/>+ visibility metadata / collection"]
  Know["Knowledge store"]
  Graph["Context Graph"]
  Q["POST /query type=knowledge<br/>query_apps=true<br/>+ ACL metadata_filters or collections"]
  LLM["Your LLM"]
  Resp["Answer + citations"]

  Slack --> Conn --> Ingest
  Ingest --> Know
  Ingest --> Graph
  AuthZ --> Q
  Know --> Q
  Graph --> Q
  Q --> LLM --> Resp

  style Graph fill:#FF571A,stroke:#FF571A,color:#f8fafc
  style Ingest fill:#1e293b,stroke:#334155,color:#f8fafc
  style Q fill:#1e293b,stroke:#334155,color:#f8fafc
  style AuthZ fill:#0f172a,stroke:#ef4444,color:#f8fafc
```

| Concern | Design |
| --- | --- |
| IDs | Stable `id` / `external_id` per message or thread snapshot |
| Ingest scope | Stamp every source with visibility you can filter later — e.g. schema `metadata.channel_id` / `visibility`, or a workspace `collection` that matches Slack ACLs |
| Query scope | **Required:** derive `metadata_filters` and/or `collection`/`collections` from the caller's allowed channels. Never run an unfiltered company-wide `query_apps: true` over mixed public + private channels |
| Query extras | `query_apps: true`, `mode: "thinking"` for thread reconstruction — only *after* ACL filters are applied |
| Refresh | Re-ingest upserts on edit; [`DELETE /context`](/api-reference/v2/endpoint/delete-source) on delete events |

<Warning>
  HydraDB does not infer Slack channel membership. If the connector indexes private channels into a shared company `database`, an unscoped Knowledge query can surface them. Enforce ACLs in **your app** via collections and/or `metadata_filters` on every request.
</Warning>

Related: [App Sources](/essentials/v2/app-sources) · [Connectors](/essentials/v2/connectors) · [Metadata](/essentials/v2/metadata)

---

## 2. Personalized Support Desk

Help Knowledge is shared; customer state is Memories.

```mermaid
flowchart LR
  Ticket["Incoming ticket"]
  Agent["Support agent service"]
  QK["POST /query type=knowledge<br/>shared / default collection"]
  QM["POST /query type=memory<br/>collection=customer_id"]
  Merge["Merge context"]
  LLM["LLM reply"]
  WB["POST /context/ingest<br/>type=memory infer=true<br/>collection=customer_id"]

  Ticket --> Agent
  Agent --> QK
  Agent --> QM
  QK --> Merge
  QM --> Merge
  Merge --> LLM
  LLM --> Agent
  Agent --> WB
```

**Why two queries (default):** Shared help articles live in the default (or team) Knowledge collection; customer Memories live under `collection = customer_id`. A single `type: "all"` call with only the customer collection **does not** retrieve Knowledge written under a different collection — see [Multi-tenant](/essentials/v2/multi-tenant).

**One-call alternative:** If you want a single [`POST /query`](/api-reference/v2/endpoint/query), pass `type: "all"` with `collections` covering **both** partitions (e.g. shared Knowledge collection + `customer_id`), or ingest shared macros into every customer collection (usually worse). Prefer dual query when Knowledge and Memories need different prompt formatting — see [Query](/essentials/v2/query).

Cookbook: [Customer Support Agent](/cookbooks/v2/customer-support-agent)

---

## 3. Enterprise SaaS Multi-Tenant Brain

Hard isolation per customer org; collections for workspaces and seats.

```mermaid
flowchart TB
  subgraph Control["Control plane"]
    Prov["POST /databases per customer"]
    Status["GET /databases/status"]
  end

  subgraph CustA["database: customer_a"]
    KA["Knowledge: their Notion/Slack"]
    WA["collection: workspace_*"]
    UA["collection: user_* Memories"]
  end

  subgraph CustB["database: customer_b"]
    KB["Knowledge"]
    UB["Memories"]
  end

  App["Your SaaS backend"]
  Prov --> Status
  Status --> CustA
  Status --> CustB
  App --> CustA
  App --> CustB
  CustA -.->|no read path| CustB

  style CustA fill:#1e293b,stroke:#334155,color:#f8fafc
  style CustB fill:#0f172a,stroke:#334155,color:#f8fafc
```

| Concern | Design |
| --- | --- |
| Isolation | Never put two customers in one `database` |
| Env | `customer_a_prod` vs `customer_a_staging` as separate databases |
| Metadata | Per-customer schema fields: `department`, `region`, `classification` |
| AuthZ | Resolve `database` + `collection` from your session before any HydraDB call |

Related: [Multi-tenant](/essentials/v2/multi-tenant) · [Create Database](/api-reference/v2/endpoint/create-tenant)

---

## 4. Coding Agent with Repo Knowledge + Dev Memory

```mermaid
sequenceDiagram
  participant Dev as Developer
  participant Agent as Coding agent
  participant H as HydraDB
  participant LLM as LLM

  Dev->>Agent: "Why is checkout failing?"
  Agent->>H: POST /query type=knowledge<br/>team/default collection graph_context=true
  Agent->>H: POST /query type=memory<br/>collection=dev_id
  H-->>Agent: runbooks + ADRs
  H-->>Agent: prefs + prior decisions
  Agent->>LLM: prompt with chunks + relations
  LLM-->>Agent: diagnosis
  Agent-->>Dev: answer
  Agent->>H: POST /context/ingest type=memory<br/>collection=dev_id infer=true
```

| Store | Content |
| --- | --- |
| Knowledge | ADRs, runbooks, service docs in the **shared** team/default collection (upsert on merge to main) |
| Memories | Style prefs, prior incident conclusions, tool constraints under `collection = dev_id` |
| Query | Dual query (recommended) or one `type: "all"` with `collections` listing **both** the shared Knowledge collection and `dev_id` |
| Graph | Service ownership and dependency questions on the Knowledge query |
| Ephemeral | Debug paste memories with `expiry_time` |

---

## 5. Research Corpus Synthesizer

```mermaid
flowchart TB
  Papers["PDFs · HTML clips · notes"]
  Ingest["ingest type=knowledge<br/>metadata: trust_tier, year"]
  Know["Knowledge corpus"]
  Graph["Citation / entity graph"]
  Mem["Memories: hypotheses, steering"]
  Q["POST /query mode=thinking<br/>dual or type=all with both collections<br/>graph_context=true"]
  Synth["Synthesis agent"]

  Papers --> Ingest --> Know
  Ingest --> Graph
  Mem --> Q
  Know --> Q
  Graph --> Q
  Q --> Synth
```

**Design notes:** Prefer `metadata_filters` for `trust_tier` / year gates. Keep corpus Knowledge and steering Memories in explicit collections — dual query or `collections` fanout when they differ. Use `recency_bias` when newer literature should win. Archive dead projects by isolating or deleting the project `collection`'s sources.

---

## 6. Internal Knowledge Assistant (Notion + Slack + Drive)

```mermaid
flowchart TB
  subgraph Sources
    N[Notion]
    S[Slack]
    D[Drive / PDFs]
  end

  subgraph HydraDB["Company database"]
    Conn[Connectors / App Sources]
    K[Knowledge + visibility metadata]
    G[Context Graph]
    M[Employee Memories per collection]
  end

  AuthZ["Session → allowed depts / channels"]
  QK["POST /query type=knowledge<br/>query_apps=true + ACL filters"]
  QM["POST /query type=memory<br/>collection=employee_id"]
  UI["Search / chat UI"]
  LLM["LLM"]

  N --> Conn
  S --> Conn
  D --> Conn
  Conn --> K
  Conn --> G
  AuthZ --> QK
  AuthZ --> QM
  K --> QK
  G --> QK
  M --> QM
  UI --> QK --> LLM
  UI --> QM --> LLM
  LLM --> UI

  style AuthZ fill:#0f172a,stroke:#ef4444,color:#f8fafc
```

| Concern | Design |
| --- | --- |
| ACL-shaped data | Encode visibility at ingest (`department`, `confidentiality`, `channel_id`) and **always** apply matching `metadata_filters` (and/or allowed `collections`) from the session — never an unfiltered company-wide query |
| Personalization | Employee Memories under `collection = employee_id`; query Memories separately (or `type: "all"` with `collections` that include both the shared Knowledge partition and the employee collection) |
| Sync | Connector-driven refresh; treat `external_id` as upsert key |

Related: [Glean Clone](/cookbooks/v2/glean-clone) · [Connectors](/essentials/v2/connectors) · [Metadata](/essentials/v2/metadata)

---

## Architecture selection matrix

| If you need… | Start from architecture |
| --- | --- |
| Channel decision search | #1 Slack → Knowledge → Graph |
| Ticket personalization | #2 Support Desk |
| Multi-customer SaaS | #3 Enterprise Multi-Tenant |
| IDE / coding agent | #4 Coding Agent |
| Long-form synthesis | #5 Research Corpus |
| Company-wide search | #6 Internal Assistant |

---

## Shared production spine

Every architecture above shares the same operational spine from [Architecture](/essentials/v2/architecture):

```mermaid
stateDiagram-v2
  [*] --> Provisioned: POST /databases + status ready
  Provisioned --> Ingesting: POST /context/ingest
  Ingesting --> Indexing: queued → processing → graph_creation
  Indexing --> Queryable: completed
  Queryable --> Serving: POST /query
  Serving --> Learning: memory write-back
  Learning --> Ingesting
  Queryable --> Refreshing: upsert same id
  Refreshing --> Indexing
  Queryable --> Deleted: DELETE /context
  Deleted --> [*]
```

Poll [`GET /context/status`](/api-reference/v2/endpoint/source-status) (or use [webhooks](/essentials/v2/webhooks)) before treating new sources as fully graph-ready.

---

## Next

- [Context Lifecycle](/essentials/v2/context-architecture/lifecycle) — months-long evolution
- [Anti-Patterns](/essentials/v2/context-architecture/anti-patterns) — how these architectures go wrong
- [Production Examples](/essentials/v2/context-architecture/production-examples) — domain walkthroughs
