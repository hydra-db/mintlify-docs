...
{/*
  ...
  The DX audit found 2 broken code blocks on this page: one block uses module-level `await` (SyntaxError before any call), and one imports the nonexistent `HydraDBError`.
