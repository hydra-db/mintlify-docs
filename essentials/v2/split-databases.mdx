...
Relations, subgraph and chunks each page a cursor over one graph, and two cursors cannot be merged, so `all` reads knowledge there. Every response carries `meta.source_type`, which reports the corpus that answered, not the `type` you sent.

<Warning>
`DELETE /context` with `type: "all"` deletes from **both** corpora. The two lanes run against two stores with no transaction between them: if the knowledge lane succeeds and the memory lane then fails, the knowledge deletions have already happened. To delete from one corpus only, send `knowledge` or `memory`.
</Warning>
...
On a unified database every chunk carries `is_memory`, and it is filterable in `attributes`. On a split database it is not filterable, because rows written before the column existed do not have it.
...
- On a unified database, `GET /databases/stats` reports the database's rows in `knowledge_collection.row_count`, and `memory_collection` repeats the same number.
