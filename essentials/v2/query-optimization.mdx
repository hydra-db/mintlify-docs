...
6. **Check security**
   - Wrong user's data appearing? → Select an authorized per-user `database` or `collection` server-side; do not rely on request-supplied metadata for isolation
   - Wrong environment data? → Add `metadata_filters: {env: "production"}` within the authorized database or collection
