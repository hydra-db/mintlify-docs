...
| `## Results` | One block per entry of `chunks[]`, in ranked order, separated by `---`: a `### 1. title` heading; a line with `**Relevance:**` (the `score`), `**Collection:**`, `**Type:**` and `**Category:**` (the `enrichment.kind`); a line with `**Id:**` (the `context_id`) and `**Last updated:**`; the chunk's `content`; then `**Enrichment:**` with the `enrichment.text`. |
...
| `## Related facts` | One line per path in `graph[]`, such as `- [P1] **A** -pred→ **B** (query path) [1]`: the path's label, its chain of hops, `(query path)` or `(chunk relation)` for its `origin`, and the results its hops were extracted from. The `path_summary` is indented on the line under it. |
| `## Temporal facts` | One line per dated fact the query engaged (the facts behind `chunks[].temporal`): subject, relation and object, then the resolved window, precision, status and evidence phrase, citing its result. |
| `## Sources` | Each context once, in order of first appearance: title, type, id, url and last-updated date. |
...
Surface it to your agent verbatim, and let the model cite the labels. The SDK `build_string` / `buildString` helpers return `llm_prompt` verbatim on a unified database. When you need structured output instead, read `chunks[].content`, `chunks[].enrichment.text`, `graph[].path_summary` and `forceful_relations[]`. See [How to Use API Results](/essentials/v2/api-results).
