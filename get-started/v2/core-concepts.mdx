...
## Knowledge vs. Memories vs. Infer

These three terms get confused because they sound like a family of related features. They aren't parallel concepts: **Knowledge and Memories are the two stores**, and **Infer is a setting on Memories**, not a store of its own.

Use this to decide where a piece of context belongs:

```mermaid
flowchart TD
    A["I have context to ingest"] --> B{"Does it belong to<br/>everyone, or one user?"}
    B -->|"Everyone in the database"| C["Store as Knowledge<br/>(type=knowledge)"]
    B -->|"One user or session"| D["Store as a Memory<br/>(type=memory)"]
    D --> E{"Is the input already<br/>a clean fact, or raw signal?"}
    E -->|"Already a fact, e.g. Plan tier is Pro"| F["infer=false<br/>stored verbatim"]
    E -->|"Raw signal: chat logs, UI events, feedback"| G["infer=true<br/>HydraDB extracts the preference"]
```

| | Knowledge | Memories | `infer` |
| --- | --- | --- | --- |
| **What it is** | A store | A store | A field on a Memory item |
| **Use it for** | Shared documents, app sources, and facts every user can retrieve | User-specific preferences, history, traits, and session context | Deciding whether HydraDB should derive a memory from raw signal |
| **Write with** | `POST /context/ingest` and `type=knowledge` | `POST /context/ingest` and `type=memory` | Inside each item in the `memories` array |
| **Read with** | `POST /query` and `type: "knowledge"` | `POST /query` and `type: "memory"` | Not queried directly; it only affects ingestion |
| **Scope** | Shared across a database | Scoped to one `collection` (user, workspace, or session) | N/A - only affects how that memory item is processed |
| **Question it answers** | "What content should every user be able to retrieve?" | "What context is specific to this user or session?" | "Is this input already the fact, or should HydraDB extract the fact?" |

For personalized answers grounded in shared documents, call `POST /query` with `type: "all"` to search both stores and merge the results. See [How to Use API Results](/essentials/v2/api-results).

Read More: [Usage → Knowledge](/essentials/v2/knowledge) · [Usage → Memories](/essentials/v2/memories)
...
| **Inference** | `infer` flag (per memory item) extracts the memory from raw signal; exists only on Memories | Not applicable - content is parsed and chunked as sent |
