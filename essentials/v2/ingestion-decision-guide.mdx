...
while (true) {
  ...
  if (["graph_creation", "completed"].includes(status.indexingStatus)) break;
...
# Stop with an error if processing fails.
while true; do
  response=$(curl -s -w '\n%{http_code}' -G 'https://api.hydradb.com/context/status' \
    -H "Authorization: Bearer $HYDRA_DB_API_KEY" \
    -H "API-Version: 2" \
  ...
  if [ "$http_code" != "200" ]; then
    echo "Status request failed (HTTP $http_code): $body" >&2
    exit 1
  fi

  status=$(echo "$body" | jq -r '.data.statuses[0].indexing_status')
  case "$status" in
    graph_creation|completed) break ;;
    errored)
      echo "Ingestion failed: $(echo "$body" | jq -r '.data.statuses[0].error_message')" >&2
      exit 1 ;;
  esac
  sleep 2
done
