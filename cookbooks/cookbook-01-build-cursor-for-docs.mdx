...
def _poll_until_ready(timeout=180, interval=4):
    ...
    while time.time() < deadline:
        ...
        s = getattr(status, "infra_status", None) or getattr(status, "status", "") or ""
...
def upload_batch(batch: list) -> list:
    ...
    ids = [r.source_id for r in (result.results or [])]
...
def recall_context(query: str, max_results: int = 10) -> dict:
    ...
    Returns a plain dict with "chunks", "sources", and "graph_context" keys.
    """
    raw = full_recall(
        query=query,
        max_results=max_results,
...
def build_context_block(recall_payload: dict) -> str:
    ...
    for source in sources:
        title     = getattr(source, "source_title", None) or "Untitled"
...
async def chat(body: dict):
    ...
    async def stream():
        ...
        serialized_sources = [
            {"title": getattr(s, "source_title", "") or "", "source_id": getattr(s, "source_id", "") or ""}
            for s in recall_payload.get("sources", [])
        ]
        yield json.dumps({"sources": serialized_sources}) + "\n"
...
def ask(body: dict):
    ...
    return {
        ...
        "chunks": [
            {"chunk_content": c.chunk_content or "", "relevancy_score": c.relevancy_score or 0}
            for c in recall_payload.get("chunks", [])
        ],
...
4. **Append the sources list** from `recall_payload.get("sources", [])` — typed SDK source objects accessed via `getattr` — in `- title (id)` format so the model can cite them.
