...
# Ingestion is async - wait until the memory is indexed.
while True:
    ...
    if status.indexing_status in ("completed", "errored"):
        break
    time.sleep(2)
...
while (true) {
  ...
  if (["completed", "errored"].includes(status.indexingStatus)) break;
...
# Ingestion is async - poll until indexed (repeat until completed).
