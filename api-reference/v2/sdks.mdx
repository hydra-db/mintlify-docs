...
|---|---|---|---|
| **Raw HTTP / cURL** |  -  | snake_case | `database`, `collection` |
...
| `/databases/*` and `/databases` | `client.databases` | Create, list, delete, status, collections, stats |
...
A database is an isolated workspace. Most organizations create one database total, with collections for users or teams. See [Multi-Database](/essentials/v2/multi-tenant) for the full model.
...
Database creation is asynchronous. Poll [`databases.status`](/api-reference/v2/endpoint/database-status) until provisioning completes:
