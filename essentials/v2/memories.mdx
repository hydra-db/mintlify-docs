...
<Warning>
**`thinking` mode only.** Forceful-relation context is fetched only when `mode` resolves to `"thinking"` on the query request. In `fast` mode, `query_forceful_relations` is silently ignored even if set to `true` - the server does not error, and `additional_context` will simply be empty. Under `mode: "auto"`, whether it takes effect depends on the automatic routing decision.
</Warning>
---
