...
| [Webhooks](/api-reference/v2/endpoint/webhooks-overview) | Register a callback for indexing events and manage delivery history | When you want push notifications for `completed`/`errored` items instead of polling `/context/status` |
...
| [`/webhooks/indexing`](/api-reference/v2/endpoint/register-webhook) | `POST` | REST | Register or replace the indexing webhook | You want an HTTP callback when ingested items reach a terminal status. |
| [`/webhooks/indexing`](/api-reference/v2/endpoint/get-webhook) | `GET` | REST | Get the current webhook registration | You need to confirm the active URL, events, and signing configuration. |
| [`/webhooks/indexing`](/api-reference/v2/endpoint/delete-webhook) | `DELETE` | REST | Unregister the webhook | You want to stop receiving indexing events. |
| [`/webhooks/indexing/test`](/api-reference/v2/endpoint/test-webhook) | `POST` | REST | Send a synthetic test delivery | You just registered and want to verify your endpoint end to end. |
| [`/webhooks/indexing/deliveries`](/api-reference/v2/endpoint/list-deliveries) | `GET` | REST | List delivery attempts | You need to audit deliveries or find failures to retry. |
| [`/webhooks/indexing/deliveries/{delivery_id}`](/api-reference/v2/endpoint/get-delivery) | `GET` | REST | Get one delivery attempt | You are debugging a specific event your receiver logged. |
| [`/webhooks/indexing/deliveries/{delivery_id}/retry`](/api-reference/v2/endpoint/retry-delivery) | `POST` | REST | Retry a failed delivery | A delivery is `failed` or `permanently_failed` and your receiver is fixed. |
...
**Response envelope:** Core v2 endpoints (`/databases`, `/context/*`, and `/query`) return a consistent envelope. Endpoint pages show the full envelope; the resource-specific payload lives under `data`. [Webhook management endpoints](/api-reference/v2/endpoint/webhooks-overview) (`/webhooks/indexing*`) are the exception: they return their documented response object directly.
