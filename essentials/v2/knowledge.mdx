...
Both are queried through the same [`POST /query`](/api-reference/v2/endpoint/query) endpoint; `type` just picks the store.
...
| <Field name="collection" /> | `string` | none | Scope this knowledge to a collection. If omitted, knowledge is available to all collections. |
