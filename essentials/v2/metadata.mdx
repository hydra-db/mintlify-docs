...
<Tip>
To see the metadata actually attached to a source — including keys a connector added automatically — open the dashboard's [Context page](https://app.hydradb.com/context), select the source, and switch to the **Metadata** tab. To filter on what you see there, follow the rules below: schema-declared `metadata` keys need `enable_match: true`, and `additional_metadata` keys must be nested under `additional_metadata` in `metadata_filters`.
</Tip>
