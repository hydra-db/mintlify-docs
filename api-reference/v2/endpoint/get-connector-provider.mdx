...
| `setup_guide` | Connect-time steps the credential schema cannot express, in reading order. Present only for providers that need them. |
| `token_scopes` / `token_scopes_note` | Permissions the provider token should carry (`id`, `required`, `reason`), or a note when the provider has no scope strings. Omitted when unknown. |
