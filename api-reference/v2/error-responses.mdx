...
|---|---|---|
| `INVALID_PARAMETERS` | `400` | A required parameter is missing, malformed, mutually incompatible with another parameter, or exceeds length limits (e.g., `source_id` > 100 characters, `sub_tenant_id` > 128 characters). |
...
- Identifier exceeds length limit: `source_id` or `id` > 100 characters, `sub_tenant_id` > 128 characters.
