...
> **Note**: `/query` has no top-level `metadata` parameter - deterministic filtering goes through `metadata_filters`, and only keys declared in your database's metadata schema (with `enable_match: true`) are filterable. `source_type`/`source_title` below are illustrative; declare the fields you actually need at database creation. See [Metadata](/essentials/v2/metadata).
...
# Filter search by a declared "document_type" schema field
def query_by_document_type(query: str, database: str, document_type: str):
    return search(query, database, metadata_filters={"document_type": document_type})

# Filter search by a declared "department" schema field
def query_by_department(query: str, database: str, department: str):
    return search(query, database, metadata_filters={"department": department})
```
```typescript TypeScript SDK
import { HydraDBClient } from "@hydradb/sdk";
...
// Filter search by a declared "document_type" schema field
const queryByDocumentType = (query: string, database: string, documentType: string) =>
  search(query, database, { metadataFilters: { document_type: documentType } });

// Filter search by a declared "department" schema field
const queryByDepartment = (query: string, database: string, department: string) =>
  search(query, database, { metadataFilters: { department: department } });
```
</CodeGroup>
...
# Search with optional metadata_filters on declared schema fields
def search_with_filters(query: str, database: str, document_type: str = None, department: str = None):
    metadata_filters = {}
    if document_type:
...
const searchWithFilters = async (
  query: string,
  database: string,
  filters: { documentType?: string; department?: string } = {}
) => {
  const metadataFilters: Record<string, any> = {};
  if (filters.documentType) metadataFilters.document_type = filters.documentType;
  if (filters.department) metadataFilters.department = filters.department;
...
def search_with_memory(query: str, database: str, user_id: str):
    ...
    if profile["preferred_source_types"]:
        # metadata_filters only matches keys declared in your database's metadata schema;
        # this assumes a "document_type" field was declared with enable_match: true.
        kwargs["metadata_filters"] = {"document_type": profile["preferred_source_types"][0]}
...
const searchWithMemory = async (query: string, database: string, userId: string) => {
  ...
  const searchResults = await client.query({
    ...
    alpha: 0.5,
    recencyBias: 0.3,
    // metadataFilters only matches keys declared in your database's metadata schema;
    // this assumes a "documentType" field was declared with enable_match: true.
    ...(userProfile.preferredSourceTypes?.length > 0 && {
      metadataFilters: { document_type: userProfile.preferredSourceTypes[0] }
    })
  });
...
const SearchResults = ({ results }) => {
  // graph_context stays snake_case on the response wire, even in TypeScript/JavaScript.
  const { chunks, graph_context: graphContext } = results.data ?? {};
  ...
  return (
    <div className="search-results">
      ...
      {graphContext && graphContext.query_paths && (
        <div className="graph-context">
          <h3>Related Knowledge Graph Paths</h3>
          <pre>{JSON.stringify(graphContext.query_paths, null, 2)}</pre>
        </div>
      )}
    </div>
  );
...
class DataPrivacyManager {
  ...
  async deleteOldData(database, memoryId, collection = null) {
    ...
    // Raw fetch to the REST API: DELETE /context takes type, database, ids
    // (and optional collection) as flat sibling keys - see client.context.delete().
    const body = { type: 'memory', database: database, ids: [memoryId] };
    if (collection) {
