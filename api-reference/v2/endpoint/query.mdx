...
<AccordionGroup>
  ...
  <Accordion title="Filters">
    ...
    Multiple keys are ANDed  -  a source must satisfy every one of them.

    A **list value means ANY (OR)** of the listed values, on `VARCHAR` fields only:
    ...
    That reads as *(department is finance OR legal) AND status is published*. A single-element list is equivalent to the scalar form.

    | Case | Behavior |
    |---|---|
    | Scalar value | Exact equality. |
    | List on a `VARCHAR` field (declared, undeclared, or under `additional_metadata`) | Matches a source holding **any one** of the values. |
    | List on a declared `INT8`/`INT16`/`INT32`/`INT64`/`FLOAT`/`DOUBLE`/`BOOL` field | `400 VALIDATION_ERROR`  -  lists are `VARCHAR`-only; pass a single scalar. |

    **There is no ALL / AND operator within a single key.** `["finance", "legal"]` never restricts results to sources carrying both values. To require multiple values, model the combination as its own field, split the constraint across different keys, or intersect the results of several queries client-side.

    Range, contains, and fuzzy operators are not supported in user-provided `metadata_filters`; run multiple queries or post-process client-side for those cases.
  </Accordion>
</AccordionGroup>
