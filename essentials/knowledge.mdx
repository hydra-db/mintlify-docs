...
<Note>
  For incremental app-source webhooks, use the same `upload_knowledge` endpoint. Re-ingest the same `id` only when the same source changed. Ingest new Slack/email messages, new knowledge-base pages, and new ticket comments as new app sources. Ticket comments should use `kind: "comment"` with `fields.parent_id` pointing to the parent ticket's `external_id`.
</Note>
