...
Memories are the atoms of HydraDB. Any piece of unstructured content that carries ding includes documents, PDFs, emails, messages, webpages, presentations, reports, user preferences, past conversations, and CSVs. becomes a memory.

HydraDB parses, chunks, embeds, and links each memory into a shared context graph.

HydraDB treats memory and knowledge as **distinct primitives**, which is one of the things that sets it apart from other tools. Memory is a dynamic, interaction-level state that evolves with every interaction; knowledge is a static, document-level context that gets versioned and replaced. Same retrieval pipeline, different storage layers.
...
HydraDB's recall is not a vector search. It's a multi-stage pipeline that combines metadata filtering, semantic and keyword retrieval, graph traversal, and personalized ranking based on the user, agent, and task.
All of this, in a single API call.
...
- **Tenant metadata** - organizational fields defined at tenant creation. These are immutable schemas.
- **Document metadata** - per-document fields attached at ingestion of memories. Fully flexible.
