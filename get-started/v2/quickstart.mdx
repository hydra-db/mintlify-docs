...
#    The SDK sends the item list in the `context` form field.
...
Creating a database and indexing are **asynchronous**: HydraDB provisions infrastructure and indexes your content in the background, so each is followed by a short polling loop. The ingest call returns `202` as soon as the items are queued, and querying runs in real time. One ingest request takes up to 100 items; send more requests for more.
