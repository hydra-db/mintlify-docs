...
Webhooks let your application receive an HTTP callback when HydraDB finishes processing an ingested context.
...
When an ingested context reaches a terminal state, HydraDB creates a delivery record and sends a `POST` request to your webhook URL.
...
| Event | When it fires |
|---|---|
| `indexing.status_changed` | When a context reaches `completed`, `errored`, or `success` |
...
<Info>
Webhook management endpoints return the standard v2 `{ success, data, error, meta }` envelope. The responses on this page show the `data` object.
</Info>
...
| `X-HydraDB-Signature` | `sha256=<hex>` HMAC-SHA256 of the raw body, keyed by your signing secret. Only when signing is configured. See [Verifying signatures](#5-verifying-signatures) |
...
| `id` | The context's `context_id`: the one you supplied at ingestion, or the generated one. For connector-synced content, the connector context's id. |
| `database` | The database you ingested into, as sent on the ingest request. Empty only for context ingested before this field existed. |
| `collection` | Collection scope for the indexed context. |
...
<Note>
Older examples may refer to this identifier as `doc_id`. New webhook payloads use `id`.
</Note>
...
<Note>
`database` is empty only for context ingested before this field existed. Read
...
These wire the verifier above into a real handler. Both read the **raw** body before parsing.
  ...
  "deliveries": [
    {
      ...
      "webhook_url": "https://api.example.com/webhooks/hydradb",
      "attempts": 1,
      "error_code": null,
      "error_message": null,
...
Delivery history calls the context's id `doc_id`. The outbound webhook payload calls it `id`.
...
| Signature check fails | Compute the HMAC over the raw body, not parsed JSON, and compare lowercase hex against the whole header, including `sha256=`. |
| Signature header is missing | Signing is not configured. Call `POST /webhooks/indexing/signing-secret` to enable it. |
| Signatures started failing after a rotation | Rotation is immediate; deploy the new secret. See [Zero-downtime key rotation](#zero-downtime-key-rotation). |
...
| `id` is unexpected | It is the context's `context_id` (the one you supplied, or the generated one), or the connector context's id for synced content. |
