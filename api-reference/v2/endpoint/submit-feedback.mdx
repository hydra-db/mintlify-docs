...
| Status | When |
| --- | --- |
| `400` | `request_id` missing or not a UUID; no usable signal (`feedback` blank or absent **and** `ground_truth` absent, empty, or blank); `feedback`, `ground_truth` or `metadata` over its limits; unknown `rating`/`source`; `collection` without `database` |
