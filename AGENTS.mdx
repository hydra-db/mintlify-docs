...
try:
    client.feedback.submit(
        ...
        feedback=(
            ...
            "Chunks 1 and 3 (Q3 handbook) carried the answer. Chunk 2 was the "
            "2023 policy and was discarded as superseded. No outside sources needed."
        ),
        database="acme_corp",
    )
except Exception:
    pass  # fire-and-forget, exactly as above
```
...
`POST /feedback` records how a query performed. It is the loop that closes retrieval quality, and it is open to agents whenever a query falls short  -  and, once a task is finished, for a short report on how the context held up  -  see [Sending feedback](#sending-feedback).
