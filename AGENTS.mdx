...
- Separate keys AND together. A scalar value is an exact match.
- A **list value means ANY (OR)** of the listed values: `{"department": ["finance", "legal"]}` matches a source in either department. A single-element list equals the scalar form.
- **There is no ALL/AND operator within a single key.** A list never restricts results to sources holding every listed value. To require multiple values, model the combination as its own field, split across separate keys, or intersect several queries client-side.
- Lists are `VARCHAR`-only. A list on a declared `INT8`/`INT16`/`INT32`/`INT64`/`FLOAT`/`DOUBLE`/`BOOL` field returns `400 VALIDATION_ERROR`. Undeclared fields and `additional_metadata` are treated as `VARCHAR`.
- Range, contains, and fuzzy operators are not supported.
