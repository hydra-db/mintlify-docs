...
3. **`query_by`** ⚡ **Medium impact**
   - Choose: `hybrid` (default), `text` (keyword-only), or `vector` (semantic-only)
...
| `vector` | Semantic-only | Conceptual similarity, cross-lingual | "authentication methods" |

**Default:** `hybrid`  
**Performance:** Similar latency across all modes
...
**Use `vector` when:**
- Pure semantic similarity ("What's similar to OAuth?")
- Cross-lingual search (query in English, match Spanish docs)
...
**Range:** `0.0` to `1.0`, or `"auto"` (recommended default)

**What it controls:**
- `0.0` = Pure keyword matching (like `query_by: "text"`)
- `0.5` = Balanced semantic and keyword
- `1.0` = Pure semantic matching (like `query_by: "vector"`)
- `"auto"` = HydraDB chooses based on query
...
result = client.query(
    ...
    database="cars",
    alpha="auto"  # Returns generic EV comparisons
)
...
Apply filters when you need hard access control or data segmentation.

**Use for:**
- **Access control**: User can only see their own data
- **Data segmentation**: Filter by environment (production/staging)
- **Category filtering**: Only billing docs, not technical docs
- **App source filtering**: Only Slack messages, not emails
...
<CodeGroup>
```python Python
# Multi-tenant app - security constraint
result = client.query(
    query="my recent orders",
    database="commerce",
    metadata={
        "user_id": current_user.id,     # Security: only this user's data
        "status": ["completed", "shipped"]  # Business logic
    }
)

# Support bot - filter by product line
result = client.query(
...
const result = await client.query({
  ...
  database: "commerce",
  metadata: {
    user_id: currentUser.id,     // Security: only this user's data
    status: ["completed", "shipped"]  // Business logic
  }
});
...
> **Learn more**: See [Metadata Filtering](/essentials/v2/metadata) for advanced filtering patterns.
...
|---------|---------|----------|
| **Wrong `type`** | Irrelevant results (memories mixed with docs) | Set `type: "knowledge"` or `"memory"` explicitly |
...
| **No metadata filters** | Security leak, wrong data returned | Add `metadata` filters for access control |
| **Using `hybrid` for exact match** | Fuzzy results, missing exact strings | Switch to `query_by: "text"` + `operator: "and"` |
| **No filters on multi-tenant** | Users see other users' data | Always filter by `user_id` in metadata |
