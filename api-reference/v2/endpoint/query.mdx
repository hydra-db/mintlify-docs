...
<Info>
  **Fewer results than expected?** Ingestion is asynchronous, so content that is still indexing won't appear in results yet. Every query response carries a `meta.note` when results may be incomplete — call [`GET /context/status/pending`](/api-reference/v2/endpoint/pending-sources) to list the source IDs that have not finished indexing for your `database`/`collection`.
</Info>
