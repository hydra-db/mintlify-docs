...
| `sub_tenant_id` | string | No | the `tenant_id` value | Sub-tenant scope. When omitted, content is stored under the default sub-tenant, which is the `tenant_id` value - **not** the literal string `"default"`. Maximum 128 characters. |
...
|---|---|---|
| `file_id` | string | Optional. Server-generated if omitted. Use this to reference the file later (e.g., for `verify_processing`). Maximum 100 characters (also called `source_id` internally). |
...
| `sub_tenant_id` | string | **Required.** Sub-tenant scope. If not partitioning by user, use the same value as `tenant_id` - that is the default sub-tenant an omitted `sub_tenant_id` resolves to on queries. Do **not** use the literal string `"default"`; it creates a separate scope that omitted-scope queries will never match. Maximum 128 characters. |
| `id` | string | **Required.** Unique identifier for the source. Maximum 100 characters. |
