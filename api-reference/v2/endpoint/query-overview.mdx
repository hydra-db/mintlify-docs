...
To accept **any one of several values** for a key, pass a list. Lists are OR/ANY and are supported on `VARCHAR` fields only:
...
That matches sources in *either* department that are also `published`. A list never means ALL  -  there is no way to require a single key to hold every listed value. A list against a declared `INT*`/`FLOAT`/`DOUBLE`/`BOOL` field returns `400 VALIDATION_ERROR`. See [Metadata  -  filter semantics](/essentials/v2/metadata#filter-semantics).
