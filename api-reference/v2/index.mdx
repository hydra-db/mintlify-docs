...
|---|---|---|
| [Databases](/api-reference/v2/endpoint/databases-overview) | Create, monitor, and manage isolated workspaces | First step in any integration  -  and any time you need usage stats, provisioning status, or to tear down a workspace |
...
| `sub_tenant_id` | Optional partition inside a database, often a user, team, account, or customer. | Use it when one database contains data for multiple users or customers. Read more about our [multi-database architecture](/essentials/v2/multi-database) |
...
|---|---|---|---|---|
| [`/tenants`](/api-reference/v2/endpoint/create-database) | `POST` | `tenants.create` | Create a database | You are setting up a new isolated workspace and optional metadata schema. |
| [`/tenants/{tenant_id}/metadata-schema`](/api-reference/v2/endpoint/update-metadata-schema) | `PATCH` | REST | Add metadata schema fields | You need to add filterable metadata fields after database creation. |
| [`/tenants`](/api-reference/v2/endpoint/list-databases) | `GET` | `tenants.list` | List databases | You need to discover database IDs available to the current API key. |
| [`/tenants`](/api-reference/v2/endpoint/delete-database) | `DELETE` | `tenants.delete` | Delete a database | You need to permanently remove a workspace and its data. |
| [`/tenants/status`](/api-reference/v2/endpoint/database-status) | `GET` | `tenants.status` | Check provisioning readiness | You just created a database and need to wait before ingesting data. |
| [`/tenants/sub-tenants`](/api-reference/v2/endpoint/list-collections) | `GET` | `tenants.subTenants` / `tenants.sub_tenants` | List active collections | You partition data by user, team, customer, or account and need to inspect those partitions. |
| [`/tenants/stats`](/api-reference/v2/endpoint/database-stats) | `GET` | `tenants.stats` | Get usage statistics | You want to monitor object counts and vector dimensions for a database. |
...
- **Database scoping.** Most database-scoped endpoints require a `tenant_id`. Many source and query endpoints also accept an optional `sub_tenant_id` for finer-grained scoping. If omitted, the default collection is used.
