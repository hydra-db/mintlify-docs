...
1. **Source-id prefilter.** Safe scalar tenant/additional metadata filters are resolved to matching `source_id`s in the document store.
...
<Warning>
  Adding fields updates the stored database schema and filter indexes. Existing vector-index collections are not altered/backfilled for newly added dense/sparse metadata lanes yet. If you need a new metadata field to participate in semantic/BM25 metadata search for existing data, create a new database/schema and re-ingest, or confirm the current platform migration path with support.
</Warning>
...
- `enable_match`-only tenant metadata updates are filter-index-only and take effect for filters/listing.
...
| New schema field does not participate in semantic metadata search | Additive schema update does not alter/backfill existing vector-index fields yet | Create the desired schema before ingesting, or migrate/re-ingest into a database with the final schema. |
