...
| `SimilarityPostprocessor` drops all nodes | `score=None` on all `NodeWithScore` objects | Return `score=float(raw_score) if raw_score is not None else 1.0` in `_chunk_to_node` |
...
class AsyncHydraDBRetriever(HydraDBRetriever):
    ...
    async def _aretrieve(self, query_bundle: QueryBundle):
        # Create a fresh async client per call to avoid event-loop issues.
        # For production: use a connection pool or inject a shared async client.
        async_client = AsyncHydraDB(token=os.environ["HYDRA_DB_API_KEY"])
        result = await async_client.query(
            database=self._database,
            collection=self._collection,
