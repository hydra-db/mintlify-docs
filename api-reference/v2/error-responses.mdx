...
- The request exceeds the limits: 100 items, 1 MiB of text per item, 8 MiB of text per request, 1,024 bytes per `title`, 4,000 characters of `instructions`, 16 KiB of `attributes` or 1 KiB of `custom_attributes` per item.
...
- An `attributes` filter may be too restrictive, or the items may not carry the attribute values you filter on. See [Attributes](/essentials/v2/attributes).
