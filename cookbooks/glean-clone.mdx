  ...
  if (uploadResult.results?.length) {
    for (const item of uploadResult.results) {
      const response = await client.upload.verifyProcessing({
        tenant_id: tenantId,
        file_ids: [item.source_id]
      });
      if (response.statuses?.[0]?.indexing_status === "errored") {
        throw new Error(`Processing failed for source ${item.source_id}`);
      }
    }
  }
    ...
    if upload_result.results:
        for item in upload_result.results:
            ...
            if statuses and statuses[0].indexing_status == "errored":
                raise Exception(f"Processing failed for source {item.source_id}")
...
> **Note**: HydraDB supports filtering by `source_title` and `source_type` using the `metadata_filters` parameter. Use these for targeted searches across specific data sources.
...
const searchBySourceType = (query: string, tenantId: string, sourceType: string) =>
  search(query, tenantId, { metadata_filters: { source_type: sourceType } });

// Filter search by source title
const searchBySourceTitle = (query: string, tenantId: string, sourceTitle: string) =>
  search(query, tenantId, { metadata_filters: { source_title: sourceTitle } });
```
```python Python SDK
import os
from hydra_db import HydraDB
...
def search(query: str, tenant_id: str, sub_tenant_id: str = None,
           ...
           metadata_filters: dict = None, additional_context: str = None):
...
def search_by_source_type(query: str, tenant_id: str, source_type: str):
    return search(query, tenant_id, metadata_filters={"source_type": source_type})

# Filter search by source title
def search_by_source_title(query: str, tenant_id: str, source_title: str):
    return search(query, tenant_id, metadata_filters={"source_title": source_title})
```
</CodeGroup>
...
const searchWithFilters = async (
  ...
  if (filters.sourceTypes) metadata_filters.source_type = filters.sourceTypes;
  if (filters.sourceTitles) metadata_filters.source_title = filters.sourceTitles;
...
def search_with_filters(query: str, tenant_id: str, source_types: str = None, source_titles: str = None):
    metadata_filters = {}
    if source_types:
        metadata_filters["source_type"] = source_types
    if source_titles:
        metadata_filters["source_title"] = source_titles
...
const searchWithMemory = async (query: string, tenantId: string, userId: string) => {
  ...
  const searchResults = await client.recall.fullRecall({
    ...
    ...(userProfile.preferredSourceTypes?.length > 0 && {
      metadata_filters: { source_type: userProfile.preferredSourceTypes }
    })
  });
...
def search_with_memory(query: str, tenant_id: str, user_id: str):
    ...
    if profile["preferred_source_types"]:
        kwargs["metadata_filters"] = {"source_type": profile["preferred_source_types"]}
...
class TenantManager {
  ...
  async searchWithTenantContext(query, tenantId, subTenantId = null, userId = null) {
    ...
    if (subTenantId) {
      searchOptions.metadata_filters.sub_tenant_id = subTenantId;
    }
...
class DataPrivacyManager {
  ...
  async deleteOldData(tenantId, memoryId, subTenantId = null) {
    // Use HydraDB SDK's delete memory method
    const params = { tenant_id: tenantId, memory_id: memoryId };
    if (subTenantId) {
      params.sub_tenant_id = subTenantId;
    }
    return this.client.memories.deleteMemory(params);
  }
