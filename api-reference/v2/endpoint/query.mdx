...
- **Common mistakes.** Check [Ingestion Status](/api-reference/v2/endpoint/source-status) for recently ingested documents before querying. If you omit `collections` and deprecated `collection`, HydraDB queries the default collection. Use [List Collections](/api-reference/v2/endpoint/list-collections) to discover available IDs.
</Warning>
