...
Narrows results to documents matching specific metadata fields before semantic ranking.

- Top-level keys are matched against `tenant_metadata` and must correspond to fields defined in `tenant_metadata_schema` during tenant creation.
- To filter on `document_metadata`, nest filters under a `document_metadata` key.
- Both filter types support exact match filtering and can be combined.

```json
// Filter by tenant metadata
{ "category": "engineering", "department": "R&D" }

// Filter by document metadata
{ "document_metadata": { "source": "account_plan" } }

// Combined
{ "category": "engineering", "document_metadata": { "status": "approved" } }
```
