...
- **App knowledge** – structured content from connected apps (Slack messages, Notion pages, Gmail emails, ticket comments, tickets, CRM records, etc.) where you already have the text, IDs, metadata, and relations, sent as the `app_knowledge` multipart field (JSON string)
...
<div className="api-after-endpoint" />
<Note>
...
| `kind` | string | App object kind: `email`, `message`, `ticket`, `knowledge_base`, `comment`, or `custom`. |
...
| `comments` | array | Embedded snapshot comments connected to this app source. For incremental ticket comments, prefer a new `kind: "comment"` source. |

### Incremental app-source ingestion

Use this same endpoint for live webhook updates:

| Event | Send |
|---|---|
| Ticket title, status, assignee, or description changed | The same `ticket` source with the same `id` and `external_id`, using `upsert: true` |
| New Linear/Jira/Zendesk ticket comment | A new `comment` source with its own `id` and `external_id`, plus `fields.parent_id` set to the parent ticket `external_id` |
| Edited ticket comment | The same `comment` source with the same `id` and `external_id`, using `upsert: true` |
| New Slack/Teams message | A new `message` source with `thread_id` and optional `parent_id` |
| New Gmail/Outlook reply | A new `email` source with `thread_id` and optional `reply_to_id` |
| New Notion/Confluence page | A new `knowledge_base` source, with the space/project stored in `metadata` |

For a first-class comment, the top-level `kind` and `fields.kind` must both be `comment`:
...
```json
{
  ...
  "fields": {
    "kind": "comment",
    "body": "Confirmed this still reproduces on staging.",
