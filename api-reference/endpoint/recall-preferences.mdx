...
| `sub_tenant_id` | string | No | primary tenant ID | **Critical for B2C apps** – pass the end user's sub-tenant to scope the search to that user's memories. If omitted, defaults to the resolved primary tenant ID. |
...
<Info>
**Pass `sub_tenant_id` explicitly for B2C apps.** Personalized recall only works when memories are scoped to the right user. Omitting `sub_tenant_id` defaults to the resolved primary tenant ID.
</Info>
