...
1. Declare the field on the database **using the real request wrappers** (not a bare field object):

**At create** — [`POST /databases`](/api-reference/v2/endpoint/create-tenant) puts fields in `database_metadata_schema`:
...
**Additive update** — [`PATCH /databases/{database}/metadata-schema`](/api-reference/v2/endpoint/update-metadata-schema) puts fields in `add_fields`:
