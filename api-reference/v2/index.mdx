...
Rate limits apply per API key. For production deployments, build retry logic with exponential backoff against the `429` response. Contact [founders@hydradb.com](mailto:founders@hydradb.com) for current limit values. See [Rate Limits and Backoff](/essentials/v2/rate-limits) for how to design bulk ingestion and query traffic around the limit.
