...
<Accordion title="Knowledge documents - upload PDFs, DOCX, CSV, markdown, or text">
  ...
  <Note>
  Those are the only keys `document_metadata` accepts, alongside the `source_id`/`file_id` aliases for `id` and an explicit `infer`. Anything else  -  including `title`, `type`, `url` and `timestamp`  -  is rejected with a `400` that names the unsupported key.

  Source title and type are **derived**, not settable: the title defaults to the filename and the type is inferred from the upload. If you need your own display fields, put them in `additional_metadata`. To set a title explicitly, use the `app_knowledge` item shape shown in the next section, which has its own `title` field.
  </Note>
</Accordion>
