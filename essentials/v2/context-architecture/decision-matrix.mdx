...
| **Slack** | Knowledge [App Sources](/essentials/v2/app-sources) or [Connectors](/essentials/v2/connectors), graph | Company DB; **required** channel/workspace `collection` or visibility `metadata` from caller ACLs | `type: "knowledge"`, `query_apps: true`, **plus** ACL `metadata_filters` / allowed `collections` | Never unscoped company-wide query over private channels |
