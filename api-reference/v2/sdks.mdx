...
Successful endpoint-handler responses use a consistent envelope:
...
The SDKs return the full success envelope. Read the payload from `data` (for example, `response.data`); on HTTP failure they raise an exception whose body follows the documented `detail` error shape.
