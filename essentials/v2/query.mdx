...
### Store and retrieval method

| Parameter | Type / values | Purpose |
| --- | --- | --- |
| `type` | `"knowledge"`, `"memory"`, or `"all"` | Which store to query. `"all"` runs Knowledge and Memories in parallel and merges by `relevancy_score`. Default: `"knowledge"`. |
| `query_by` | `"hybrid"` or `"text"` | Retrieval method. `"hybrid"` blends semantic and BM25 signals; `"text"` is pure BM25 keyword matching. Default: `"hybrid"`. |
| `mode` | `"fast"`, `"thinking"`, or `"auto"` | Retrieval pipeline. Applies to `hybrid` only; ignored for `text`. `"auto"` scores the query and resolves to `"fast"` or `"thinking"`, and overrides whatever `graph_context` you sent to match the resolved mode. Default: `"auto"`. |
| `alpha` | float `0.0`–`1.0` or `"auto"` | Hybrid weight: `1.0` is pure semantic, `0.0` is pure BM25. Applies to `query_by: "hybrid"` only. Default: `0.8`. |
| `operator` | `"or"`, `"and"`, or `"phrase"` | BM25 term operator for `query_by: "text"`. Ignored for `hybrid`. Default: `"or"`. |
...
| `query_forceful_relations` | boolean | Whether to fetch author-declared related sources (see [`relations` on ingest](/api-reference/v2/endpoint/ingest-context)) into `additional_context`. **Only takes effect when `mode` resolves to `"thinking"`** - silently ignored in `fast` mode. Default: `true`. |
