...
| <Field name="collection" type="string" /> | Collection that contains the source. Defaults to the default collection. (deprecated alias: `sub_tenant_id`) |
...
<Warning>
  This PATCH body uses `database_metadata` for schema-backed source metadata. Ingestion and list responses use the shorter `metadata` name. Deprecated `tenant_metadata` is still accepted, but do not use it in new integrations. `document_metadata` is also deprecated here; use `additional_metadata`.
</Warning>
