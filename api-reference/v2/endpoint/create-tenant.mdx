...
api: "POST https://api.hydradb.com/databases"
playground: "none"
...
| Name | Description |
| --- | --- |
| <Field name="database" type="string" required /> | Database name, unique within your organization. Up to 255 characters, using only lowercase letters, digits, `-` and `_`. Formerly `tenant_id`; the `tenant_id` alias is still accepted (deprecated). |
