...
Pass one or more IDs in `ids` to delete that context, whether you ingested it or a connector synced it. Send `database`, `collection`, and `ids` as top-level fields in the request body. Include the same `collection` you used when ingesting; omitting it targets the default collection.
...
- **`data.success` is deprecated:** do not use it to decide anything. Whether the request succeeded is the HTTP status (or the envelope's top-level `success`); whether anything was removed is `deleted_count` and `results[]`.
- **Retrieval drops deleted context immediately:** Even before background cleanup finishes, deleted IDs disappear from `/query` and `/context/list` responses.
