...
For a two-chunk knowledge query with `graph_context: true` in `mode: "thinking"` (forceful-relation expansion only runs in thinking mode) and a declared [forceful relation](/essentials/v2/forceful-relations), `build_string` returns a string like:

```text
=== ENTITY PATHS ===
[Acme Corp] -> acquired -> [Beta Labs]
  ↳ [Beta Labs] -> develops -> [RoutePlanner]

=== CONTEXT ===
Chunk 1
Source: Q3 Board Update.pdf
Acme completed the acquisition of Beta Labs in July. Integration of the
RoutePlanner team starts next quarter…
Graph Relations:
  [Acme Corp] -> acquired -> [Beta Labs]: announced in the Q3 board update [Time: 2026-07]
Extra Context:
  Related Context (Acquisition FAQ.md): Timeline and org changes for the Beta Labs integration…
---

Chunk 2
Source: Q3 Board Update.pdf
Headcount grows by 34 with the Beta Labs team joining…
---
```

Sections appear only when their data is present — a `fast`-mode query with no graph context is just the `=== CONTEXT ===` block. This is what makes the result LLM-ready without post-processing: every chunk is source-attributed, supporting graph triplets sit next to the chunk they support, and declared related sources arrive as labelled extra context instead of being interleaved with primary evidence.

<Note>
Illustrative sample — exact labels and ordering may evolve as we optimize the format. Treat the helper, not this sample, as canonical.
</Note>
