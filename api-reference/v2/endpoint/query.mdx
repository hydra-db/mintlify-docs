...
| <Field name="sub_tenant_id" type="string or null" /> | Sub-tenant scope. Required for per-user memory queries. If omitted, defaults to the resolved primary tenant ID. |
...
- **Common mistakes.** Check [Ingestion Status](/api-reference/v2/endpoint/source-status) for recently ingested documents before querying. If you omit `sub_tenant_id`, HydraDB defaults to the resolved primary tenant ID; use [List Sub-Tenants](/api-reference/v2/endpoint/list-sub-tenants) to discover available IDs.
</Warning>
