...
Successful v2 endpoint-handler responses use this envelope:
...
- Parse successful handler-envelope payloads from `data`, including webhook management responses.
...
Do not invent app-kind enums or undocumented nested app fields. For ingestion, use the documented `app_knowledge` content shape and place provider-specific attributes in `metadata` or `additional_metadata`.
