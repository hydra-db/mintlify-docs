...
<Note>
Endpoint pages list the most common codes for that operation. New codes may be added over time, so clients should handle unknown `error.code` values gracefully. For the **complete** catalog — every transport code plus the ingestion/status `error_code` values, with canonical names and deprecated aliases — see the [Error Code Reference](/api-reference/v2/error-codes).
</Note>
