...
```cypher Upsert a node on your own key
MERGE (p:Person {ext_id: $id})
SET p += $props
RETURN p.ext_id AS id
```

```cypher Connect two existing nodes
MATCH (a:Person {ext_id: $from}), (b:Company {ext_id: $to})
MERGE (a)-[w:WORKS_AT]->(b)
SET w.since = $since
```

```cypher Expand a node's neighborhood
...
```cypher Multi-hop relationship expansion
...
```cypher Shortest path between two nodes
...
```cypher Filtered, typed traversal
...
```cypher Index a property you filter on
CREATE INDEX FOR (n:Person) ON (n.ext_id)
```

```cypher Delete a node and its edges
