...
# Create tenant with metadata schema (matches ingestion metadata fields below)
result = client.tenant.create(
    tenant_id=TENANT_ID,
...
def verify_all_indexed(file_ids: list[str], poll_interval: int = 3, max_polls: int = 20) -> bool:
    """
    Poll client.upload.verify_processing() until all file_ids report 'completed'.
    ...
    for attempt in range(max_polls):
        ...
        for fid in list(pending):
            data = client.upload.verify_processing(
                tenant_id=TENANT_ID,
                file_ids=[fid],
            )
            statuses = data.statuses
            status = statuses[0].indexing_status if statuses else "unknown"
...
def query_trend(
    ...
    chunks_sorted = sorted(
        chunks,
        key=lambda c: (c.source_upload_time or "") if hasattr(c, "source_upload_time") else "",
    )
    ...
    prefs_result  = client.recall.recall_preferences(
        tenant_id=TENANT_ID,
        sub_tenant_id=f"analyst-{user_id}",
        mode="thinking",
    ...
    ) or "No analyst profile available."
...
def cross_source_reconciliation(
    ...
    def recall(sub_tenant_id: str, doc_type_filter: str) -> list:
...
def track_narrative_shift(
    ...
    context_text = "\n\n".join(
        f"[{(c.additional_metadata or {}).get('period_label', '?')}]\n{c.chunk_content}"
        for c in chunks
    )
...
def financial_recall(
    ...
    raw = client.recall.full_recall(**payload)
    return {
        "chunks":        raw.chunks or [],
        "sources":       getattr(raw, "sources", []),
...
print(f"Chunks: {len(r2.get('chunks', []))}")  # r2 is a dict returned by financial_recall()
...
def generate_quarterly_briefing(
    ...
    current_context = "\n\n".join(
        f"[{(c.additional_metadata or {}).get('period_label','?')} | "
        f"{(c.additional_metadata or {}).get('doc_type','?')}]\n{c.chunk_content}"
