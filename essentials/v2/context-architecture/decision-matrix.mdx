...
| **Slack** | Knowledge [App Sources](/essentials/v2/app-sources) or [Connectors](/essentials/v2/connectors), graph | Company DB; **`collection = channel_id` per channel** | `type: "knowledge"`, `query_apps: true`, `collections: [allowed channel ids]` | Do not use metadata arrays as OR/IN; see [multi-channel ACL](/essentials/v2/context-architecture/reference-architectures#multi-channel-acl-implementable-pattern) |
| **Notion** | Knowledge app sources / connectors | Company or workspace collection; visibility metadata when pages are restricted | `type: "knowledge"`, `query_apps: true`, ACL filters when needed | Re-ingest on page update |
...
| **Personalization** | Memories + Knowledge together | User collection for Memories; shared/default (or team) collection for Knowledge | Dual query, or `type: "all"` with `collections` listing **both** partitions | A user-only `collection` on `type: "all"` misses shared Knowledge |
...
| Put prefs in Knowledge "so one query works" | Memories + dual query (or `type: "all"` with both collections) |
