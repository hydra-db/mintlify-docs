...
| --- | --- | --- |
| [`/connector-catalog`](/api-reference/v2/endpoint/connector-catalog) | `GET` | List catalog metadata for supported providers |
| [`/connector-discovery`](/api-reference/v2/endpoint/connector-discovery) | `POST` | Preview resources from credentials before create |
...
```text
GET  /connector-catalog              -> browse provider availability
GET  /connectors/providers?id=...    -> confirm credential schema
POST /connector-discovery            -> preview resources with temp credentials (optional)
