...
HydraDB publishes official SDKs for Python and TypeScript/Node. They provide typed methods and IDE autocomplete for the published v2 surface; check the installed release's method list when an endpoint was added recently.
...
| [`/databases/{database}/metadata-schema`](/api-reference/v2/endpoint/update-metadata-schema) | `PATCH` | `databases.update_metadata_schema` / `databases.updateMetadataSchema` | Add metadata schema fields | You need to add filterable metadata fields after database creation. |
...
| [`/context/{id}/metadata`](/api-reference/v2/endpoint/update-source-metadata) | `PATCH` | `context.update_source_metadata` / `context.updateSourceMetadata` | Update source metadata | You need to merge `database_metadata` or `additional_metadata` onto one existing source without re-ingesting. |
...
**Response envelope:** Many core v2 endpoint handlers use a consistent envelope. For those operations, the resource-specific payload, including webhook management responses, lives under `data`. Some connector catalog, discovery, and resource endpoints, plus database metadata-schema updates, return direct objects instead. Follow each endpoint's 2xx schema. Authentication or gateway middleware may reject a request earlier with a `detail` error shape, so branch on HTTP status before parsing a success response.
...
Handler errors use the endpoint's documented `detail` shape; middleware may return `detail` as a string. Branch on HTTP status before parsing either a success envelope or a direct success object.
...
- **Parameter casing.** Raw REST requests and the Python SDK use snake_case (`query_by`). The TypeScript SDK uses camelCase request and response fields (`queryBy`) and serializes them to snake_case on the wire.
