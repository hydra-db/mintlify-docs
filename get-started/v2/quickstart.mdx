...
id = next((item.id for item in results if item.id and not item.error), None)
if not id:
...
const id = ingest.data?.results?.find((item) => item.id && !item.error)?.id;
if (!id) throw new Error("ingestion returned no context ID");
...
while (true) {
  ...
  if (["errored", "failed"].includes(status.indexingStatus ?? "")) {
    throw new Error(status.errorMessage ?? "indexing failed");
  }
...
until curl -s "https://api.hydradb.com/databases/status?database=${DATABASE}" \
  -H "Authorization: Bearer $HYDRA_DB_API_KEY" \
  -H "API-Version: 2" \
  | jq -e '.data.infra as $i | ($i.scheduler_status == true and $i.graph_status == true and $i.vectorstore_status.knowledge == true and $i.vectorstore_status.memories == true)' > /dev/null; do
  sleep 5
done
...
while true; do
  ...
  if [ "$INDEXING_STATUS" = "errored" ] || [ "$INDEXING_STATUS" = "failed" ]; then
