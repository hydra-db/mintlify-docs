...
### Success envelope and HTTP error shape

Many core v2 handlers use the `{ success, data, error, meta }` envelope. This includes webhook management endpoints (`/webhooks/indexing*`); their resource-specific response object is under `data`. Do not assume the shape for every endpoint: connector catalog, discovery, and resource operations, plus database metadata-schema updates, can return direct objects. Follow the endpoint's documented 2xx schema.
...
Failed raw HTTP calls use the endpoint's documented error response. V2 handler errors use `detail`; authentication or gateway middleware may return `detail` as a string instead of the structured object:
...
Field | Description |
|---|---|
| `detail.error_code` | Machine-readable code for structured handler errors. |
| `detail.message` | Human-readable explanation for structured handler errors. |
| `detail.success` | `false` for structured handler errors. |
| `detail` string | Authentication, gateway, or other middleware rejection message. |

<Info>
Branch on the HTTP status before parsing the body. When `detail` is structured, use `detail.error_code` for finer handling. Log a request or correlation ID when the response provides one.
</Info>
...
| `502` | Upstream provider or dependency returned a bad gateway response | Yes, with backoff for idempotent operations |
...
<Note>
Endpoint pages list the most common codes for that operation. New codes may be added over time, so clients should handle unknown `detail.error_code` values gracefully.
</Note>
...
Retry only transient failures: `429` and server-side `5xx` responses such as `500`, `502`, and `503`. Retry writes only when they are idempotent (for example, stable IDs with explicit upsert behavior). Use exponential backoff with jitter and keep retries bounded.
...
try {
  ...
  if (error instanceof HydraDBError) {
    ...
    if (code === "DATABASE_NOT_FOUND") {
      ...
      console.error({ code, statusCode: error.statusCode });
      throw error;
    }
  } else {
    throw error;
...
def get_error_code(exc: ApiError):
    detail = (
        exc.body.get("detail")
        if isinstance(exc.body, dict)
        else getattr(exc.body, "detail", None)
    )
    return (
        detail.get("error_code")
        if isinstance(detail, dict)
        else getattr(detail, "error_code", None)
    )

with open("policy.pdf", "rb") as policy:
    try:
        ...
        if get_error_code(exc) == "DATABASE_NOT_FOUND":
            # Create or select a valid database before ingesting.
            pass
