...
curl -X POST "$BASE/byog/query" \
  ...
  -d '{
    "database": "crm",
    "collection": "sample",
    "query": "CREATE (a:Person), (b:Person), (c:Person), (d:Person), (e:Person), (acme:Company), (globex:Company) SET a = $alice, b = $bob, c = $carol, d = $dan, e = $eve, acme = $acme, globex = $globex CREATE (a)-[:KNOWS {since: 2020}]->(b), (b)-[:KNOWS {since: 2021}]->(c), (c)-[:KNOWS {since: 2022}]->(d), (a)-[:KNOWS {since: 2023}]->(e), (b)-[:WORKS_AT]->(acme), (c)-[:WORKS_AT]->(acme), (d)-[:WORKS_AT]->(globex) RETURN count(*) AS created",
    "params": {
      "alice": {"ext_id": "u1", "name": "Alice", "role": "admin"},
      "bob": {"ext_id": "u2", "name": "Bob", "role": "analyst"},
      "carol": {"ext_id": "u3", "name": "Carol", "role": "engineer"},
      "dan": {"ext_id": "u4", "name": "Dan", "role": "engineer"},
      "eve": {"ext_id": "u5", "name": "Eve", "role": "designer"},
      "acme": {"ext_id": "c1", "name": "Acme", "sector": "fintech"},
      "globex": {"ext_id": "c2", "name": "Globex", "sector": "retail"}
    }
  }'
```
...
curl -X POST "$BASE/byog/query" \
  ...
  -d '{
    "database": "crm",
    "collection": "sample",
    "query": "MERGE (p:Person {ext_id: $id}) SET p += $props RETURN p.ext_id AS id, p.name AS name, p.role AS role",
...
curl -X POST "$BASE/byog/query" \
  ...
  -d '{
    "database": "crm",
    "collection": "sample",
    "query": "MATCH (a:Person {ext_id: $from}), (b:Company {ext_id: $to}) MERGE (a)-[w:WORKS_AT]->(b) SET w.since = $since RETURN a.name AS person, type(w) AS rel, b.name AS company, w.since AS since",
...
curl -X POST "$BASE/byog/query" \
  ...
  -d '{
    "database": "crm",
    "collection": "sample",
    "query": "MATCH (p:Person {name: $name})-[r]-(nbr) RETURN type(r) AS rel, labels(nbr) AS kind, nbr.name AS neighbor ORDER BY rel, neighbor",
    "params": {"name": "Bob"}
  }'
```
...
curl -X POST "$BASE/byog/query" \
  ...
  -d '{
    "database": "crm",
    "collection": "sample",
    "query": "MATCH (a:Person {name: $name})-[:KNOWS*1..3]->(reach:Person) WHERE reach.name <> $name RETURN DISTINCT reach.name AS name ORDER BY name",
    "params": {"name": "Alice"}
  }'
```
...
curl -X POST "$BASE/byog/query" \
  ...
  -d '{
    "database": "crm",
    "collection": "sample",
    "query": "MATCH (a:Person {name: $from}), (b:Person {name: $to}) RETURN shortestPath((a)-[:KNOWS*..8]->(b)) AS path",
...
curl -X POST "$BASE/byog/query" \
  ...
  -d '{
    "database": "crm",
    "collection": "sample",
    "query": "MATCH (a:Person {name: $name})-[:KNOWS]->(f:Person)-[:WORKS_AT]->(c:Company) WHERE c.sector = $sector RETURN f.name AS person, c.name AS company ORDER BY person",
    "params": {"name": "Alice", "sector": "fintech"}
  }'
```
...
curl -X POST "$BASE/byog/query" \
  ...
  -d '{
    "database": "crm",
    "collection": "sample",
    "query": "MATCH (p:Person)-[:WORKS_AT]->(c:Company) RETURN c.name AS company, count(p) AS headcount ORDER BY headcount DESC"
  }'
```
...
curl -X POST "$BASE/byog/query" \
  ...
  -d '{
    "database": "crm",
    "collection": "sample",
    "query": "CREATE INDEX FOR (n:Person) ON (n.ext_id)"
  }'
```
...
curl -X POST "$BASE/byog/query" \
  ...
  -d '{
    "database": "crm",
    "collection": "sample",
    "query": "MATCH (p:Person {ext_id: $id}) DETACH DELETE p RETURN count(*) AS deleted",
    "params": {"id": "u5"}
  }'
```
