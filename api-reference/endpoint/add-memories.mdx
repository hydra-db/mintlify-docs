...
For any form of memories, the identifier is `source_id`. When that identifier already exists, `upsert` (boolean, default `true`) controls the behavior:
...
<Warning>
  **Tenant not provisioned (HTTP 404)**: If you receive a `404 TENANT_NOT_FOUND` error, the tenant's vector store collection has not finished provisioning. Poll `GET /tenants/infra/status?tenant_id=<your_tenant_id>` until the status is `ready` before retrying the request. See [Monitor & infra status](/api-reference/endpoint/monitor-tenant#check-tenant-infra-status) for details.
</Warning>
