...
def extract_pdf_text(file_path: str) -> list[str]:
    ...
    while start < len(encoded):
        end = min(start + MAX_TEXT_BYTES, len(encoded))
        # prefer a blank-line boundary; otherwise back off to a UTF-8
        # character boundary (continuation bytes have the form 10xxxxxx)
