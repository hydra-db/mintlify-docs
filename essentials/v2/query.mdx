...
| `received_at` | string | When HydraDB received the item this chunk came from, as an RFC 3339 timestamp (for example `2026-07-02T09:14:05Z`). This is the ingest time, not the item's `happened_at`, which is not returned here. Omitted when no receipt time is recorded for the chunk, as on older items; it is never sent empty. |
