...
A split database returns the old response exactly as before. A unified database returns four keys: `chunks`, `graph`, `relations` and `llm_prompt`, described on [Query](/essentials/v2/query#3-response). Field by field:
...
| `graph_context.query_paths[]` and `graph_context.chunk_relations[]` | `graph[]`, one flat list: query paths first, then chunk expansions, deduplicated |
| `graph_context.chunk_id_to_group_ids` | removed; each `triplets[].relation.chunk_id` names its evidence chunk |
...
| `additional_context` and `forceful_relations.declared` | `relations[]`, each `{ via: { from, to }, chunk }` |
| `temporal_facts` | `chunks[].temporal[]` |
| (none) | `llm_prompt`, a server-built string with `[1]`, `[R1]` and `[P1]` citation labels |

The SDK `build_string` / `buildString` helper formats the split response for a model. On a unified database it is not needed: inject `llm_prompt`. A parser tells the two shapes apart by their keys: `llm_prompt` and a `graph` array mean unified, `graph_context` or `chunk_content` mean split.
