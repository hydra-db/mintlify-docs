...
<Warning>
`database` can be empty for items that were already being processed when this
field was introduced. `tenant_id` is always set, so fall back to it if you need a
guaranteed non-empty scope.
</Warning>
