  ...
  "sources": [
    {
      ...
      "url": "https://docs.example.com/my-document"
    }
  ],
  "graph_context": {
    "query_paths": [
...
Five things matter for prompt construction:

- **`chunks`** - the primary retrieval output. Ranked by relevance; preserve the order HydraDB returns.
- **`sources`** - deduplicated source metadata for the returned chunks. For citations, treat `chunk_uuid` as an opaque evidence identifier and join `chunks[].id` to `sources[].id` for the source title and URL.
...
- **`additional_context`** - a map keyed by related chunk ID. When a primary chunk includes `extra_context_ids`, use those IDs to look up related chunks here and treat the matching map key as that related chunk's evidence ID.

For an ordinary prompt, the raw object has too much noise for an LLM. Section 2 shows how to convert it into a clean string. If your model returns citations, retain the original response separately and validate each generated evidence ID against it before rendering. See [Validate Citations in RAG Answers](/cookbooks/v2/validated-citations) for a complete pattern.
...
- **Give the model a grounding instruction, then validate citations.** A system prompt like "answer only from the provided context" reduces unsupported answers, but it is not an enforcement boundary. Validate generated evidence IDs before rendering user-facing citations.
...
| Missing grounding and validation | Unsupported answers and invented citation IDs can reach users | Add a context-only grounding prompt, then validate claim evidence IDs against the associated response. |
| Treating a grounding prompt as citation validation | The model can still invent an ID, omit evidence, or return source metadata you did not retrieve | Require claim-level `chunk_uuid` values and validate them against the associated query response. |
...
- [Validate Citations in RAG Answers](/cookbooks/v2/validated-citations) - enforce same-response evidence membership before rendering citations
