...
HydraDB v2 renamed its core nouns. The API accepts the old names as deprecated aliases, and some page URLs still carry legacy slugs, so you will encounter both. This table is the decoder:

| Deprecated (v1) name | Canonical v2 name | Where the old name still appears |
|---|---|---|
| `tenant_id` | `database` | Accepted as a request-body alias everywhere; plugin config keys (`HYDRADB_TENANT_ID`) |
| `sub_tenant_id` | `collection` | Accepted as a request-body alias everywhere; plugin config keys |
| `tenant_metadata` | `database_metadata` | Accepted as an alias on [`PATCH /context/{id}/metadata`](/api-reference/v2/endpoint/update-source-metadata) |
| `document_metadata` (per-item) | `additional_metadata` | Accepted as a per-item alias; still the canonical name of the multipart array field on [file ingestion](/api-reference/v2/endpoint/ingest-context) |
| `POST /recall/*` endpoints | [`POST /query`](/api-reference/v2/endpoint/query) | v1 docs and older plugin descriptions |
| "tenant" / "source" in URLs | "database" / "context" | Reference page slugs keep the legacy names for link stability - e.g. **Create Database** lives at `…/endpoint/create-tenant` and **Delete Context** at `…/endpoint/delete-source`. The sidebar titles are canonical. |

If you are reading older sample code, translate the request fields with this table; the requests will still work unchanged because every alias remains accepted.
