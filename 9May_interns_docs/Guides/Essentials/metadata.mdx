...
Tenant metadata keys are immutable once set during tenant creation, so plan filterable fields before ingestion. If a field needs to be used in `metadata_filters`, define it up front and enable matching for it.
...
### Tenant metadata vs document metadata

HydraDB exposes multiple metadata fields across write APIs. The important distinction is:

- **Filterable metadata** should be declared in the tenant metadata schema and match-enabled.
- **Flexible metadata** can be attached for display, bookkeeping, or downstream use, but should not be assumed filterable unless the corresponding field is configured for matching.

Some endpoints represent metadata differently. For example, memory item fields such as `tenant_metadata` and `document_metadata` are JSON strings, while other metadata fields may be objects depending on the endpoint. Follow the API reference for the endpoint you are using.
...
Tenant metadata keys are immutable once set during tenant creation. If you need to filter on a field later, plan it early and declare it before ingestion.
