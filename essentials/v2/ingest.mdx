...
- An unrecognised field is either refused with a `400` or dropped without an error, so check spelling against the tables above.
...
An unrecognised field is refused or dropped, never guessed at. Use the names in [Item fields](#3-item-fields).
</Accordion>
