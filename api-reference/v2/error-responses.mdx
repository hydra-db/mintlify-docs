...
Successful v2 endpoint-handler responses use the `{ success, data, error, meta }` envelope. This includes webhook management endpoints (`/webhooks/indexing*`); their resource-specific response object is under `data`.
...
async function withRetry<T>(
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      const retryable =
        error.statusCode !== undefined && [429, 500, 503].includes(error.statusCode);
