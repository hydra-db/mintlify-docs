...
- `POST /webhooks/indexing` with `{"generate_signing_secret": true}` registers and enables signing in one call, returning the plaintext once. Mutually exclusive with `signing_secret`; sending both is a `422`.
- `POST /webhooks/indexing/signing-secret` with no body generates one and returns the plaintext once. Send `{"signing_secret": "..."}` to supply your own (minimum 16 characters).
