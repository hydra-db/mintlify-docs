...
import { HydraGraph } from "/snippets/hydra-graph.jsx";

## 1. What it is

**Memories** are per-user facts and preferences (dietary needs, UI settings, tools someone uses). **Knowledge** is shared workspace content (guides, runbooks, menus) that any user or agent in the database can query. They live in separate stores but can be retrieved together.

The interactive demo below starts empty — only a User anchor. Load demo data (or add your own nodes), toggle **infer** to draw keyword-linked edges between memories and knowledge, then ask a question to watch **recall**: matching memories highlight first, then related knowledge, before a canned answer is assembled from the retrieved text. Everything runs in the browser with no network calls.

<HydraGraph />

After you try the graph, remember the mapping to the API: storing memories can optionally set `infer` so HydraDB extracts insights and links; answering a question maps to [`POST /query`](/api-reference/v2/endpoint/query) with `type: "all"` so recall pulls from both stores and can expand through the context graph. The panels under the demo show the request and response shape for that recall path.
