...
flowchart TD
    ...
    C4 -- "Yes" --> F4(["Re-run without filters;<br/>check the value matches<br/>a source exactly"])
...
**Check:** Re-run the query with no `metadata_filters`. If results come back, a filter was the culprit - the value you filtered on doesn't match any source exactly.

**Fix:** Confirm the filter value matches a value actually present on your sources (exact match, correct case and type). A declared, correctly-placed filter that simply names a value no source carries is the metadata cause of an empty result.

<Warning>
  **A silently-ignored filter causes *wrong* results, not empty ones.** Two filters are dropped silently rather than applied - an **undeclared** `metadata` key (one not declared in the schema with `enable_match: true`), and an `additional_metadata` key passed at the **top level** instead of nested as `metadata_filters: { additional_metadata: { author: "alice" } }`. In both cases the query runs *without* that filter and returns **unfiltered** (often too many) results. If you're seeing the wrong results rather than none, an ignored filter key is the likely cause. See [Metadata](/essentials/v2/metadata).
</Warning>
...
| Empty only when filtering | Filter matched zero sources (exact-match failure) | Test with no filters; confirm the value matches a source exactly (correct case and type) |
| Too many / wrong results when filtering | Filter silently ignored - undeclared key or `additional_metadata` at top level | Declare the key with `enable_match: true`; nest as `metadata_filters: { additional_metadata: { ... } }` |
